...
</code></pre></div><h2 id="定数の定義" tabindex="-1">定数の定義 <a class="header-anchor" href="#定数の定義" aria-label="Permalink to “定数の定義”">​</a></h2><p>いよいよコードを記述します。 まず、CPU内で何度も使用する定数や型を書いておくためのパッケージを作成します。</p><p><code>src/eei.veryl</code>を作成し、次のように記述します(リスト3)。</p><p><span class="caption">▼リスト3.3: eei.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/acd4130e6b9c63f2432bbf25a10f96db6faa18dc~1..acd4130e6b9c63f2432bbf25a10f96db6faa18dc#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">package</span> eei {
...
</code></pre></div><p>それでは、最上位のモジュールを作成します。 <code>src/top.veryl</code>を作成し、次のように記述します(リスト8)。</p><p><span class="caption">▼リスト3.8: 最上位モジュールの定義 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/4d25fdcba026a878fcc49357e9a67f526790b2b0~1..4d25fdcba026a878fcc49357e9a67f526790b2b0#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>addr_to_memaddr関数は、 <code>MEM_DATA_WIDTH</code>(=32)をバイトに変換した値(=4)のlog2をとった値(=2)を使って、 <code>addr[17:2]</code>を切り取っています。 範囲の選択には<code>+:</code>を利用しています。</p><p>次に、coreモジュール用のmembus_ifインターフェースを作成します(リスト11)。 ジェネリックパラメータには、 coreモジュールのインターフェースのジェネリックパラメータと同じく、 ILENとXLENを割り当てます。</p><p><span class="caption">▼リスト3.11: coreモジュール用のmembus_ifインターフェースをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/928b0dedc8fe270685fa2423bf0ee95f43dac0ad~1..928b0dedc8fe270685fa2423bf0ee95f43dac0ad#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus     : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>最後にcoreモジュールをインスタンス化します (リスト13)。 メモリとCPUが接続されました。</p><p><span class="caption">▼リスト3.13: coreモジュールをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/928b0dedc8fe270685fa2423bf0ee95f43dac0ad~1..928b0dedc8fe270685fa2423bf0ee95f43dac0ad#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk                ,
    rst                ,
    membus: membus_core,
);
...
</code></pre></div><h3 id="フェッチした命令をfifoに格納する" tabindex="-1">フェッチした命令をFIFOに格納する <a class="header-anchor" href="#フェッチした命令をfifoに格納する" aria-label="Permalink to “フェッチした命令をFIFOに格納する”">​</a></h3><p><img src="`+t+`" alt="FIFO"></p><p class="caption">▲図2: FIFO</p> フェッチした命令は次々に実行されますが、 その命令が何クロックで実行されるかは分かりません。 命令が常に1クロックで実行される場合は、 現状の常にフェッチし続けるようなコードで問題ありませんが、 例えばメモリにアクセスする命令は実行に何クロックかかるか分かりません。 <p>複数クロックかかる命令に対応するために、 命令の処理が終わってから次の命令をフェッチするように変更する場合、 命令の実行の流れは次のようになります。</p><ol><li>命令の処理が終わる</li><li>次の命令のフェッチ要求をメモリに送る</li><li>命令がフェッチされ、命令の処理を開始する</li></ol><p>このとき、 命令の処理が終わってから次の命令をフェッチするため、 次々にフェッチするよりも多くのクロック数が必要です。 これはCPUの性能を露骨に悪化させるので許容できません。</p><h4 id="fifoの作成" tabindex="-1">FIFOの作成 <a class="header-anchor" href="#fifoの作成" aria-label="Permalink to “FIFOの作成”">​</a></h4><p>そこで、 <strong>FIFO</strong>(First In First Out, ファイフォ)を作成して、 フェッチした命令を格納します。 FIFOとは、先に入れたデータが先に出されるデータ構造のことです(図2)。 命令をフェッチしたらFIFOに格納(enqueue)し、 命令を処理するときにFIFOから取り出し(dequeue)ます。</p><p>Verylの標準ライブラリ<sup class="footnote-ref"><a href="#fn3" id="fnref3">[3]</a></sup>にはFIFOが用意されていますが、 FIFOは簡単なデータ構造なので自分で作ってみましょう。 <code>src/fifo.veryl</code>を作成し、次のように記述します(リスト21)。</p><p><span class="caption">▼リスト3.21: FIFOモジュールの実装 (fifo.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cd3ad4137acc172347ac5c715fb720edb6e5b86b~1..cd3ad4137acc172347ac5c715fb720edb6e5b86b#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> fifo #(
...
</code></pre></div><p>fifoモジュールをインスタンス化したので、 メモリへデータを要求する処理を変更します(リスト25)。</p><p><span class="caption">▼リスト3.25: フェッチ処理の変更 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cd3ad4137acc172347ac5c715fb720edb6e5b86b~1..cd3ad4137acc172347ac5c715fb720edb6e5b86b#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令フェッチ処理</span>
<span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>リスト25では、 メモリに命令フェッチを要求する条件を FIFOに2つ以上空きがあるという条件に変更しています<sup class="footnote-ref"><a href="#fn4" id="fnref4">[4]</a></sup>。 これにより、FIFOがあふれてしまうことがなくなります。 また、FIFOから常にデータを取り出すようにしています。</p><p>命令をフェッチできたらFIFOに格納する処理をalways_ffブロックの中に追加します(リスト26)。</p><p><span class="caption">▼リスト3.26: FIFOへのデータの格納 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cd3ad4137acc172347ac5c715fb720edb6e5b86b~1..cd3ad4137acc172347ac5c715fb720edb6e5b86b#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// IFのFIFOの制御</span>
...
</code></pre></div><p><code>if_fifo_wvalid</code>と<code>if_fifo_wdata</code>を<code>0</code>に初期化します(リスト27)。</p><p><span class="caption">▼リスト3.27: 変数の初期化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cd3ad4137acc172347ac5c715fb720edb6e5b86b~1..cd3ad4137acc172347ac5c715fb720edb6e5b86b#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    if_pc           = <span class="hljs-number">0</span>;
    if_is_requested = <span class="hljs-number">0</span>;
    if_pc_requested = <span class="hljs-number">0</span>;
...
</code></pre></div><p>これらの値とそれぞれの命令の対応は、仕様書を確認してください。</p><h3 id="制御フラグと即値を生成する" tabindex="-1">制御フラグと即値を生成する <a class="header-anchor" href="#制御フラグと即値を生成する" aria-label="Permalink to “制御フラグと即値を生成する”">​</a></h3><p>デコード処理を書く準備が整いました。 <code>src/inst_decoder.veryl</code>を作成し、 次のように記述します(リスト32)。</p><p><span class="caption">▼リスト3.32: inst_decoder.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/e4927e3db280938846b2ab2b6eedf887d598bd55~1..e4927e3db280938846b2ab2b6eedf887d598bd55#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>ifを使うことで、 0番目のレジスタが指定されたときは、 値が常に<code>0</code>になるようにします。</p><p>レジスタの値を読み込めていることを確認するために、 デバッグ表示にソースレジスタの値を追加します(リスト38)。 <code>$display</code>システムタスクで、 命令のレジスタ番号と値をデバッグ表示します。</p><p><span class="caption">▼リスト3.38: レジスタの値をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/22011dbba42021cef415fb101bb094caf7a1e702~1..22011dbba42021cef415fb101bb094caf7a1e702#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>早速動作のテストをしたいところですが、 今のままだとレジスタの値が初期化されておらず、 0番目のレジスタの値以外は不定値<sup class="footnote-ref"><a href="#fn5" id="fnref5">[5]</a></sup>になってしまいます。</p><p>これではテストする意味がないため、 レジスタの値を適当な値に初期化します。 always_ffブロックのif_resetで、 <code>i</code>番目(0 &lt; <code>i</code> &lt; 32)のレジスタの値を<code>i</code>で初期化します(リスト39)。</p><p><span class="caption">▼リスト3.39: レジスタを適当な値で初期化する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/22011dbba42021cef415fb101bb094caf7a1e702~1..22011dbba42021cef415fb101bb094caf7a1e702#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタの初期化</span>
<span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p><code>src/sample.hex</code>を、次のように書き換えます(リスト46)。</p><p><span class="caption">▼リスト3.46: sample.hexを書き換える</span> <a href="https://github.com/nananapo/bluecore/compare/e87a1512af8d45d29963d2c48a11aa3deddf9def~1..e87a1512af8d45d29963d2c48a11aa3deddf9def#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02000093 // addi x1, x0, 32
...
00000008 : 002081b3
  ...
  op2     : 00000002
  alu res : 00000003
</code></pre></div><p>まだ、結果をディスティネーションレジスタに格納する処理を作成していません。 そのため、命令を実行してもレジスタの値は変わらないことに注意してください</p><dl><dt>addi x1, x0, 32</dt><dd> \`op1\`は0番目のレジスタの値です。 0番目のレジスタの値は常に\`0\`であるため、\`32&#39;h00000000\`と表示されています。 \`op2\`は即値です。 即値は32であるため、\`32&#39;h00000020\`と表示されています。 ALUの計算結果として、0と32を足した結果\`32&#39;h00000020\`が表示されています。 </dd><dt>auipc x2, 256</dt><dd> \`op1\`はPCです。 \`op1\`には、命令のアドレス\`0x00000004\`が表示されています。 \`op2\`は即値です。 256を12bit左にシフトした値\`32&#39;h00100000\`が表示されています。 ALUの計算結果として、これを足した結果\`32&#39;h00100004\`が表示されています。 </dd><dt>add x3, x1, x2</dt><dd> \`op1\`、\`op2\`は1、2番目のレジスタの値です。 ALUの計算結果として、それぞれの初期値\`1\`と\`2\`を足した結果\`32&#39;h00000003\`が表示されています。 </dd></dl><h2 id="レジスタに結果を書き込む" tabindex="-1">レジスタに結果を書き込む <a class="header-anchor" href="#レジスタに結果を書き込む" aria-label="Permalink to “レジスタに結果を書き込む”">​</a></h2><p>CPUはレジスタから値を読み込み、計算して、 レジスタに結果の値を書き戻します。 レジスタに値を書き戻すことを、 値を<strong>ライトバック</strong>(write-back)すると呼びます。</p><p>ライトバックする値は、計算やメモリアクセスの結果です。 まだメモリにアクセスする処理を実装していませんが、 先にライトバック処理を実装します。</p><h3 id="ライトバック処理を実装する" tabindex="-1">ライトバック処理を実装する <a class="header-anchor" href="#ライトバック処理を実装する" aria-label="Permalink to “ライトバック処理を実装する”">​</a></h3><p>書き込む対象のレジスタ(デスティネーションレジスタ)は、 命令のrdフィールドによって番号で指定されます。 デコード時に、 レジスタに結果を書き込む命令かどうかを<code>InstCtrl.rwb_en</code>に格納しています(リスト32)。</p><p>LUI命令のときは即値をそのまま、 それ以外の命令のときはALUの結果をライトバックします(リスト48)。</p><p><span class="caption">▼リスト3.48: ライトバック処理の実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/aa2034a556d6063bf561911c8b8c1650e9883e2c~1..aa2034a556d6063bf561911c8b8c1650e9883e2c#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];</span>
...
</code></pre></div><h3 id="ライトバック処理をテストする" tabindex="-1">ライトバック処理をテストする <a class="header-anchor" href="#ライトバック処理をテストする" aria-label="Permalink to “ライトバック処理をテストする”">​</a></h3><p>デバッグ表示用のalways_ffブロックで、 ライトバック処理の概要をデバッグ表示します(リスト49)。 処理している命令がライトバックする命令のときにのみ、 <code>$display</code>システムタスクを呼び出します。</p><p><span class="caption">▼リスト3.49: ライトバックのデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/aa2034a556d6063bf561911c8b8c1650e9883e2c~1..aa2034a556d6063bf561911c8b8c1650e9883e2c#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.rwb_en {
...
</code></pre></div><p>次に、<code>inst_is_new</code>の値を更新します(リスト54)。 命令が現在のクロックで供給されたかどうかは、 FIFOの<code>rvalid</code>と<code>rready</code>を観測することでわかります。 <code>rvalid</code>が<code>1</code>のとき、 <code>rready</code>が<code>1</code>なら、 次のクロックで供給される命令は新しく供給される命令です。 <code>rready</code>が<code>0</code>なら、 次のクロックで供給されている命令は現在のクロックと同じ命令になります。 <code>rvalid</code>が<code>0</code>のとき、 次のクロックで供給される命令は常に新しく供給される命令になります (次のクロックで<code>rvalid</code>が<code>1</code>かどうかは考えません)。</p><p><span class="caption">▼リスト3.54: inst_is_newの実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>memunitモジュールをインスタンス化する前に、 メモリとの接続方法を考える必要があります。</p><p>coreモジュールには、メモリとの接続点としてmembusポートが存在します。 しかし、これは命令フェッチに使用されているため、 memunitモジュールのために使用できません。 また、memoryモジュールは同時に2つの操作を受け付けられません。</p><p>この問題を、 coreモジュールにメモリとのインターフェースを2つ用意してtopモジュールで調停することにより回避します。</p><p>まず、coreモジュールに命令フェッチ用のポート<code>i_membus</code>と、 ロードストア命令用のポート<code>d_membus</code>の2つのポートを用意します(リスト55)。</p><p><span class="caption">▼リスト3.55: coreモジュールのポート定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> core (
    clk     : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">clock</span>                                    ,
    rst     : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">reset</span>                                    ,
...
</code></pre></div><p>命令フェッチ用のポートが<code>membus</code>から<code>i_membus</code>に変更されるため、 既存の<code>membus</code>を<code>i_membus</code>に置き換えてください(リスト56)。</p><p><span class="caption">▼リスト3.56: membusをi_membusに置き換える (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// FIFOに2個以上空きがあるとき、命令をフェッチする</span>
...
</code></pre></div><p>次に、topモジュールでの調停を実装します(リスト57)。 新しく<code>i_membus</code>と<code>d_membus</code>をインスタンス化し、 それを<code>membus</code>と接続します。</p><p><span class="caption">▼リスト3.57: メモリへのアクセス要求の調停 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus  : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>調停の仕組みは次のとおりです。</p><ul><li><code>i_membus</code>と<code>d_membus</code>の両方の<code>valid</code>が<code>1</code>のとき、<code>d_membus</code>を優先する</li><li><code>memarb_last_i</code>レジスタに、受け入れた要求が<code>i_membus</code>からのものだったかを記録する</li><li>メモリが要求の結果を返すとき、<code>memarb_last_i</code>を見て、<code>i_membus</code>と<code>d_membus</code>のどちらか片方の<code>rvalid</code>を<code>1</code>にする</li></ul><p>命令フェッチを優先しているとロードストア命令の処理が進まないため、 <code>i_membus</code>よりも<code>d_membus</code>を優先します。</p><p>coreモジュールとの接続を次のように変更します(リスト58)。</p><p><span class="caption">▼リスト3.58: membusを2つに分けて接続する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk       ,
    rst       ,
    i_membus  ,
    d_membus  ,
);
</code></pre></div><p>memoryモジュールとmemunitモジュールを接続する準備が整ったので、memunitモジュールをインスタンス化します(リスト59)。</p><p><span class="caption">▼リスト3.59: memunitモジュールのインスタンス化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> memu_rdata: UIntX;
...
</code></pre></div><p>ところで、現在のコードではmemunitの処理が終了していないときも値をライトバックし続けています。 レジスタへのライトバックは命令の実行が終了したときのみで良いため、 次のようにコードを変更します(リスト62)。</p><p><span class="caption">▼リスト3.62: 命令の実行が終了したときにのみライトバックする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> inst_valid <span class="custom-hl-bold">&amp;&amp; if_fifo_rready</span> &amp;&amp; inst_ctrl.rwb_en {
        regfile[rd_addr] = wb_data;
    }
}
</code></pre></div><p>デバッグ表示も同様で、 ライトバックするときにのみデバッグ表示します(リスト63)。</p><p><span class="caption">▼リスト3.63: ライトバックするときにのみデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> <span class="custom-hl-bold">if_fifo_rready &amp;&amp;</span> inst_ctrl.rwb_en {
    $display(<span class="hljs-string">&quot;  reg[%d] &lt;= %h&quot;</span>, rd_addr, wb_data);
}
...
</code></pre></div><p>ここからのテストは実行するクロック数が多くなります。 そこで、ログに何クロック目かを表示することでログを読みやすくします(リスト65)。</p><p><span class="caption">▼リスト3.65: 何クロック目かを出力する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">var</span> clock_count: <span class="hljs-keyword">u64</span>;</span>
...
<span class="hljs-keyword">always_ff</span> {
    ...
    <span class="custom-hl-bold">} <span class="hljs-keyword">else</span> {</span>
        ...
        <span class="hljs-keyword">if</span> inst_valid {
            ...
            $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
            $display(<span class="hljs-string">&quot;  itype     : %b&quot;</span>, inst_ctrl.itype);
</code></pre></div><p>LW、SW命令のテストのために、 <code>src/sample.hex</code>を次のように変更します(リスト66)。</p><p><span class="caption">▼リスト3.66: テスト用のプログラムを記述する (sample.hex)</span> <a href="https://github.com/nananapo/bluecore/compare/e6d22c39ec88fe07dd4458e664904df8716026d0~1..e6d22c39ec88fe07dd4458e664904df8716026d0#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02002503 // lw x10, 0x20(x0)
...
</code></pre></div><p>funct3をcase文で分岐し、 アドレスの下位ビットを見ることで、 命令とアドレスに応じた値をrdataに設定します(リスト69)。</p><p><span class="caption">▼リスト3.69: rdataをアドレスと読み込みサイズに応じて変更する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/15bf256ef4a3bf6e13efc4e115d689f193ef9f5c~1..15bf256ef4a3bf6e13efc4e115d689f193ef9f5c#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// loadの結果</span>
...
</code></pre></div><p><code>wmask</code>、wmask_expand関数をmodportに追加します ( リスト72 )。</p><p><span class="caption">▼リスト3.72: modport masterとslaveにwmask、wmask_expand関数を追加する (membus_if.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/15bf256ef4a3bf6e13efc4e115d689f193ef9f5c~1..15bf256ef4a3bf6e13efc4e115d689f193ef9f5c#diff-5b8ff8b1b43eb415d367759fb46872980d16065c43c0a71a75fec85e987ad460">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">modport</span> master {
    valid       : <span class="hljs-keyword">output</span>,
    ready       : <span class="hljs-keyword">input</span> ,
    addr        : <span class="hljs-keyword">output</span>,
    wen         : <span class="hljs-keyword">output</span>,
...
</code></pre></div><p><code>wmask</code>には、書き込む部分を<code>1</code>、書き込まない部分を<code>0</code>で指定します。 このような挙動をする値を、書き込みマスクと呼びます。 バイト単位で指定するため、<code>wmask</code>の幅は<code>DATA_WIDTH / 8</code>ビットです。</p><p>次に、memoryモジュールで書き込みマスクをサポートします(リスト73)。</p><p><span class="caption">▼リスト3.73: 書き込みマスクをサポートするmemoryモジュール (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/15bf256ef4a3bf6e13efc4e115d689f193ef9f5c~1..15bf256ef4a3bf6e13efc4e115d689f193ef9f5c#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
    <span class="hljs-keyword">param</span> FILEPATH_IS_ENV: <span class="hljs-keyword">logic</span>  = <span class="hljs-number">0</span> , <span class="hljs-comment">// FILEPATHが環境変数名かどうか</span>
    <span class="hljs-keyword">param</span> FILEPATH       : <span class="hljs-keyword">string</span> = <span class="hljs-string">&quot;&quot;</span>, <span class="hljs-comment">// メモリの初期化用ファイルのパス, または環境変数名</span>
) (
    clk   : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">clock</span>                                     ,
...
</code></pre></div><p><span class="caption">▼リスト3.78: メモリにアクセスする命令のとき、wmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/15bf256ef4a3bf6e13efc4e115d689f193ef9f5c~1..15bf256ef4a3bf6e13efc4e115d689f193ef9f5c#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>req_wmask = <span class="hljs-keyword">case</span> ctrl.funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
`)])])],-1),e('<p>ここで、新しく命令をフェッチしなおすようにしても、 ジャンプ命令によって実行されることがなくなった命令がFIFOに残っていることがあることに注意する必要があります(図4)。</p><p><img src="'+r+`" alt="ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている"></p><p class="caption">▲図4: ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている</p> 実行するべきではない命令を実行しないようにするために、 ジャンプ命令を実行するときに、FIFOをリセットします。 <p>FIFOに、中身をリセットするための信号<code>flush</code>を実装します(リスト86)。</p><p><span class="caption">▼リスト3.86: ポートにflushを追加する (fifo.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e~1..3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> fifo #(
    <span class="hljs-keyword">param</span> DATA_TYPE: <span class="hljs-keyword">type</span> = <span class="hljs-keyword">logic</span>,
    <span class="hljs-keyword">param</span> WIDTH    : <span class="hljs-keyword">u32</span>  = <span class="hljs-number">2</span>    ,
) (
    clk       : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>        ,
    ...
    wready    : <span class="hljs-keyword">output</span> <span class="hljs-keyword">logic</span>        ,
</code></pre></div><p><code>flush</code>が<code>1</code>のとき、 <code>head</code>と<code>tail</code>を<code>0</code>に初期化することでFIFOを空にします(リスト87、リスト88)。</p><p><span class="caption">▼リスト3.87: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH==1)</span> <a href="https://github.com/nananapo/bluecore/compare/3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e~1..3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        rdata  = <span class="hljs-number">0</span>;
        rvalid = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p><span class="caption">▼リスト3.88: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH!=1)</span> <a href="https://github.com/nananapo/bluecore/compare/3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e~1..3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        head = <span class="hljs-number">0</span>;
        tail = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p>coreモジュールで、 <code>control_hazard</code>と<code>flush</code>を接続し、 FIFOをリセットします(リスト89)。</p><p><span class="caption">▼リスト3.89: ジャンプ命令のとき、FIFOをリセットする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e~1..3c044407ef6cb5a6b8a620dee9ab8a247f2bc88e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> if_fifo: fifo #(
    DATA_TYPE: if_fifo_type,
    WIDTH    : <span class="hljs-number">3</span>           ,
) (
    clk                           ,
