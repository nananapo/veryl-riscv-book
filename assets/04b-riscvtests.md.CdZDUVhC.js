...
</code></pre></div><p><code>test_success</code>はポートとして定義します (リスト10)。</p><p><span class="caption">▼リスト5.10: テスト結果を報告するためのポートを宣言する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/98655d98fd2c51ed735083d0c4e4c167adefad70~1..98655d98fd2c51ed735083d0c4e4c167adefad70#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> top #(
...
</code></pre></div><p>このPythonプログラムは、 第2引数で指定したディレクトリに存在する、 第3引数で指定した文字列を名前に含むファイルを、 第1引数で指定したシミュレータで実行し、 その結果を報告します。</p><p>次のオプションの引数が存在します。</p><dl><dt>-r</dt><dd> 第2引数で指定されたディレクトリの中にあるディレクトリも走査します。 デフォルトでは走査しません。 </dd><dt>-e 拡張子</dt><dd> 指定した拡張子のファイルのみを対象にテストします。 HEXファイルをテストしたい場合は、\`-e hex\`にします。 デフォルトでは\`hex\`が指定されています。 </dd><dt>-o ディレクトリ</dt><dd> 指定したディレクトリにテスト結果を格納します。 デフォルトでは\`result\`ディレクトリに格納します。 </dd><dt>-t 時間</dt><dd> テストに時間制限を設けます。 0を指定すると時間制限はなくなります。 デフォルト値は10(秒)です。 </dd></dl><p>テストが成功したか失敗したかの判定には、 シミュレータの終了コードを利用しています。 テストが失敗したときに終了コードが<code>1</code>になるように、 Verilatorに渡しているC++プログラムを変更します (リスト13)。</p><p><span class="caption">▼リスト5.13: tb_verilator.cpp</span> <a href="https://github.com/nananapo/bluecore/compare/98655d98fd2c51ed735083d0c4e4c167adefad70~1..98655d98fd2c51ed735083d0c4e4c167adefad70#diff-cb1447535f2400768c9115df1ccdcc7be37fd2670577cc465b330f5968ee3013">差分をみる</a></p><div class="language-cpp"><button title="Copy Code" class="copy"></button><span class="lang">cpp</span><pre class="hljs"><code><span class="hljs-meta">#<span class="hljs-keyword">ifdef</span> TEST_MODE</span>
