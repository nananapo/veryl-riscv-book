...
</code></pre></div><h3 id="mtvalレジスタを実装する" tabindex="-1">mtvalレジスタを実装する <a class="header-anchor" href="#mtvalレジスタを実装する" aria-label="Permalink to “mtvalレジスタを実装する”">​</a></h3><p>例外が発生すると、CPUはトラップベクタにジャンプして例外を処理します。 mcauseレジスタを読むことでどの例外が発生したかを判別できますが、 その例外の詳しい情報を知りたいことがあります。</p><p><img src="`+d+`" alt="mtvalレジスタ"></p><p class="caption">▲図1: mtvalレジスタ</p> RISC-Vには、例外が発生したときのソフトウェアによるハンドリングを補助するために、 MXLENビットのmtvalレジスタが定義されています(図1)。 例外が発生したとき、CPUはmtvalレジスタに例外に固有の情報を書き込みます。 これ以降、例外に固有の情報のことをtvalと呼びます。 <p><code>ExceptionInfo</code>構造体に例外に固有の情報を示す<code>value</code>を追加します (リスト11)。</p><p><span class="caption">▼リスト10.11: tvalをExceptionInfoに追加する (corectrl.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/634ecdaad9df9ac4228c619d4b313082765e3b13~1..634ecdaad9df9ac4228c619d4b313082765e3b13#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">struct</span> ExceptionInfo {
    valid: <span class="hljs-keyword">logic</span>   ,
    cause: CsrCause,
