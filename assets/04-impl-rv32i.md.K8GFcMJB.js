...
</code></pre></div><h2 id="定数の定義" tabindex="-1">定数の定義 <a class="header-anchor" href="#定数の定義" aria-label="Permalink to “定数の定義”">​</a></h2><p>いよいよコードを記述します。 まず、CPU内で何度も使用する定数や型を書いておくためのパッケージを作成します。</p><p><code>src/eei.veryl</code>を作成し、次のように記述します(リスト3)。</p><p><span class="caption">▼リスト3.3: eei.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/10c1101df3793ff38192190951d57e8e0859dabe~1..10c1101df3793ff38192190951d57e8e0859dabe#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">package</span> eei {
...
</code></pre></div><p>membus_ifはジェネリックインターフェースです。 ジェネリックパラメータとして、 <code>ADDR_WIDTH</code>と<code>DATA_WIDTH</code>が定義されています。 <code>ADDR_WIDTH</code>はアドレスの幅、 <code>DATA_WIDTH</code>は1つのデータの幅です。</p><p>interfaceを利用することで変数の定義が不要になり、 ポートの相互接続を簡潔にできます。</p><h3 id="メモリモジュールを実装する" tabindex="-1">メモリモジュールを実装する <a class="header-anchor" href="#メモリモジュールを実装する" aria-label="Permalink to “メモリモジュールを実装する”">​</a></h3><p>メモリを作る準備が整いました。 <code>src/memory.veryl</code>を作成し、次のように記述します(リスト5)。</p><p><span class="caption">▼リスト3.5: メモリモジュールの定義 (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/4f1bc794938e0855d5b0b09f334b574ae3c26d04~1..4f1bc794938e0855d5b0b09f334b574ae3c26d04#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
...
<span class="hljs-keyword">const</span> MEM_ADDR_WIDTH: <span class="hljs-keyword">u32</span> = <span class="hljs-number">20</span>;
</code></pre></div><p>それでは、最上位のモジュールを作成します。 <code>src/top.veryl</code>を作成し、次のように記述します(リスト8)。</p><p><span class="caption">▼リスト3.8: 最上位モジュールの定義 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/4f1bc794938e0855d5b0b09f334b574ae3c26d04~1..4f1bc794938e0855d5b0b09f334b574ae3c26d04#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>addr_to_memaddr関数は、 <code>MEM_DATA_WIDTH</code>(=32)をバイトに変換した値(=4)のlog2をとった値(=2)を使って、 <code>addr[17:2]</code>を切り取っています。 範囲の選択には<code>+:</code>を利用しています。</p><p>次に、coreモジュール用のmembus_ifインターフェースを作成します(リスト11)。 ジェネリックパラメータには、 coreモジュールのインターフェースのジェネリックパラメータと同じく、 ILENとXLENを割り当てます。</p><p><span class="caption">▼リスト3.11: coreモジュール用のmembus_ifインターフェースをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/81da624cb605e314831b3b286b66960f52509c8e~1..81da624cb605e314831b3b286b66960f52509c8e#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus     : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>最後にcoreモジュールをインスタンス化します (リスト13)。 メモリとCPUが接続されました。</p><p><span class="caption">▼リスト3.13: coreモジュールをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/81da624cb605e314831b3b286b66960f52509c8e~1..81da624cb605e314831b3b286b66960f52509c8e#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk                ,
    rst                ,
    membus: membus_core,
);
...
</code></pre></div><p>fifoモジュールをインスタンス化したので、 メモリへデータを要求する処理を変更します(リスト25)。</p><p><span class="caption">▼リスト3.25: フェッチ処理の変更 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/9d1cf1054539568c7b9655b806d0a8a9c52c59b9~1..9d1cf1054539568c7b9655b806d0a8a9c52c59b9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令フェッチ処理</span>
<span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>リスト25では、 メモリに命令フェッチを要求する条件を FIFOに2つ以上空きがあるという条件に変更しています<sup class="footnote-ref"><a href="#fn4" id="fnref4">[4]</a></sup>。 これにより、FIFOがあふれてしまうことがなくなります。 また、FIFOから常にデータを取り出すようにしています。</p><p>命令をフェッチできたらFIFOに格納する処理をalways_ffブロックの中に追加します(リスト26)。</p><p><span class="caption">▼リスト3.26: FIFOへのデータの格納 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/9d1cf1054539568c7b9655b806d0a8a9c52c59b9~1..9d1cf1054539568c7b9655b806d0a8a9c52c59b9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// IFのFIFOの制御</span>
...
</code></pre></div><p><code>if_fifo_wvalid</code>と<code>if_fifo_wdata</code>を<code>0</code>に初期化します(リスト27)。</p><p><span class="caption">▼リスト3.27: 変数の初期化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/9d1cf1054539568c7b9655b806d0a8a9c52c59b9~1..9d1cf1054539568c7b9655b806d0a8a9c52c59b9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    if_pc           = <span class="hljs-number">0</span>;
    if_is_requested = <span class="hljs-number">0</span>;
    if_pc_requested = <span class="hljs-number">0</span>;
...
</code></pre></div><p>これらの値とそれぞれの命令の対応は、仕様書を確認してください。</p><h3 id="制御フラグと即値を生成する" tabindex="-1">制御フラグと即値を生成する <a class="header-anchor" href="#制御フラグと即値を生成する" aria-label="Permalink to “制御フラグと即値を生成する”">​</a></h3><p>デコード処理を書く準備が整いました。 <code>src/inst_decoder.veryl</code>を作成し、 次のように記述します(リスト32)。</p><p><span class="caption">▼リスト3.32: inst_decoder.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/18660c7ff863989d69ca86c7f74b508c7cbdb8f5~1..18660c7ff863989d69ca86c7f74b508c7cbdb8f5#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>まず、デコーダとcoreモジュールを接続するために<code>inst_ctrl</code>と<code>inst_imm</code>を定義します。 次に、inst_decoderモジュールをインスタンス化します。 <code>bits</code>ポートに<code>inst_bits</code>を渡すことでフェッチした命令をデコードします。</p><p>デバッグ用のalways_ffブロックに、 デコードした結果をデバッグ表示するコードを記述します(リスト34)。</p><p><span class="caption">▼リスト3.34: デコード結果のデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/18660c7ff863989d69ca86c7f74b508c7cbdb8f5~1..18660c7ff863989d69ca86c7f74b508c7cbdb8f5#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>ifを使うことで、 0番目のレジスタが指定されたときは、 値が常に<code>0</code>になるようにします。</p><p>レジスタの値を読み込めていることを確認するために、 デバッグ表示にソースレジスタの値を追加します(リスト38)。 <code>$display</code>システムタスクで、 命令のレジスタ番号と値をデバッグ表示します。</p><p><span class="caption">▼リスト3.38: レジスタの値をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/caa43f90d9aeabb889352cbebf9cddd815ec2e26~1..caa43f90d9aeabb889352cbebf9cddd815ec2e26#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>早速動作のテストをしたいところですが、 今のままだとレジスタの値が初期化されておらず、 0番目のレジスタの値以外は不定値<sup class="footnote-ref"><a href="#fn5" id="fnref5">[5]</a></sup>になってしまいます。</p><p>これではテストする意味がないため、 レジスタの値を適当な値に初期化します。 always_ffブロックのif_resetで、 <code>i</code>番目(0 &lt; <code>i</code> &lt; 32)のレジスタの値を<code>i</code>で初期化します(リスト39)。</p><p><span class="caption">▼リスト3.39: レジスタを適当な値で初期化する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/caa43f90d9aeabb889352cbebf9cddd815ec2e26~1..caa43f90d9aeabb889352cbebf9cddd815ec2e26#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタの初期化</span>
<span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>次に、<code>src/alu.veryl</code>を作成し、次のように記述します(リスト42)。</p><p><span class="caption">▼リスト3.42: alu.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/ddc58faa558f3b25287a96c3b9ed459571f31184~1..ddc58faa558f3b25287a96c3b9ed459571f31184#diff-b2410559b76182b1bc742cc3c956eb08e8bc1b4179316a0cebacd77cf94a4d7a">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
<span class="hljs-keyword">import</span> corectrl::*;
...
</code></pre></div><p><code>src/sample.hex</code>を、次のように書き換えます(リスト46)。</p><p><span class="caption">▼リスト3.46: sample.hexを書き換える</span> <a href="https://github.com/nananapo/bluecore/compare/ddc58faa558f3b25287a96c3b9ed459571f31184~1..ddc58faa558f3b25287a96c3b9ed459571f31184#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02000093 // addi x1, x0, 32
...
00000008 : 002081b3
  ...
  op2     : 00000002
  alu res : 00000003
</code></pre></div><p>まだ、結果をディスティネーションレジスタに格納する処理を作成していません。 そのため、命令を実行してもレジスタの値は変わらないことに注意してください</p><dl><dt>addi x1, x0, 32</dt><dd> \`op1\`は0番目のレジスタの値です。 0番目のレジスタの値は常に\`0\`であるため、\`32&#39;h00000000\`と表示されています。 \`op2\`は即値です。 即値は32であるため、\`32&#39;h00000020\`と表示されています。 ALUの計算結果として、0と32を足した結果\`32&#39;h00000020\`が表示されています。 </dd><dt>auipc x2, 256</dt><dd> \`op1\`はPCです。 \`op1\`には、命令のアドレス\`0x00000004\`が表示されています。 \`op2\`は即値です。 256を12bit左にシフトした値\`32&#39;h00100000\`が表示されています。 ALUの計算結果として、これを足した結果\`32&#39;h00100004\`が表示されています。 </dd><dt>add x3, x1, x2</dt><dd> \`op1\`、\`op2\`は1、2番目のレジスタの値です。 ALUの計算結果として、それぞれの初期値\`1\`と\`2\`を足した結果\`32&#39;h00000003\`が表示されています。 </dd></dl><h2 id="レジスタに結果を書き込む" tabindex="-1">レジスタに結果を書き込む <a class="header-anchor" href="#レジスタに結果を書き込む" aria-label="Permalink to “レジスタに結果を書き込む”">​</a></h2><p>CPUはレジスタから値を読み込み、計算して、 レジスタに結果の値を書き戻します。 レジスタに値を書き戻すことを、 値を<strong>ライトバック</strong>(write-back)すると呼びます。</p><p>ライトバックする値は、計算やメモリアクセスの結果です。 まだメモリにアクセスする処理を実装していませんが、 先にライトバック処理を実装します。</p><h3 id="ライトバック処理を実装する" tabindex="-1">ライトバック処理を実装する <a class="header-anchor" href="#ライトバック処理を実装する" aria-label="Permalink to “ライトバック処理を実装する”">​</a></h3><p>書き込む対象のレジスタ(デスティネーションレジスタ)は、 命令のrdフィールドによって番号で指定されます。 デコード時に、 レジスタに結果を書き込む命令かどうかを<code>InstCtrl.rwb_en</code>に格納しています(リスト32)。</p><p>LUI命令のときは即値をそのまま、 それ以外の命令のときはALUの結果をライトバックします(リスト48)。</p><p><span class="caption">▼リスト3.48: ライトバック処理の実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/efb6d7b2f08e7b3e03b7d08128fb477649d38b80~1..efb6d7b2f08e7b3e03b7d08128fb477649d38b80#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];</span>
...
</code></pre></div><h3 id="ライトバック処理をテストする" tabindex="-1">ライトバック処理をテストする <a class="header-anchor" href="#ライトバック処理をテストする" aria-label="Permalink to “ライトバック処理をテストする”">​</a></h3><p>デバッグ表示用のalways_ffブロックで、 ライトバック処理の概要をデバッグ表示します(リスト49)。 処理している命令がライトバックする命令のときにのみ、 <code>$display</code>システムタスクを呼び出します。</p><p><span class="caption">▼リスト3.49: ライトバックのデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/efb6d7b2f08e7b3e03b7d08128fb477649d38b80~1..efb6d7b2f08e7b3e03b7d08128fb477649d38b80#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.rwb_en {
...
</code></pre></div><p>次に、<code>inst_is_new</code>の値を更新します(リスト54)。 命令が現在のクロックで供給されたかどうかは、 FIFOの<code>rvalid</code>と<code>rready</code>を観測することでわかります。 <code>rvalid</code>が<code>1</code>のとき、 <code>rready</code>が<code>1</code>なら、 次のクロックで供給される命令は新しく供給される命令です。 <code>rready</code>が<code>0</code>なら、 次のクロックで供給されている命令は現在のクロックと同じ命令になります。 <code>rvalid</code>が<code>0</code>のとき、 次のクロックで供給される命令は常に新しく供給される命令になります (次のクロックで<code>rvalid</code>が<code>1</code>かどうかは考えません)。</p><p><span class="caption">▼リスト3.54: inst_is_newの実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>命令フェッチ用のポートが<code>membus</code>から<code>i_membus</code>に変更されるため、 既存の<code>membus</code>を<code>i_membus</code>に置き換えてください(リスト56)。</p><p><span class="caption">▼リスト3.56: membusをi_membusに置き換える (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// FIFOに2個以上空きがあるとき、命令をフェッチする</span>
...
</code></pre></div><p>次に、topモジュールでの調停を実装します(リスト57)。 新しく<code>i_membus</code>と<code>d_membus</code>をインスタンス化し、 それを<code>membus</code>と接続します。</p><p><span class="caption">▼リスト3.57: メモリへのアクセス要求の調停 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus  : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>調停の仕組みは次のとおりです。</p><ul><li><code>i_membus</code>と<code>d_membus</code>の両方の<code>valid</code>が<code>1</code>のとき、<code>d_membus</code>を優先する</li><li><code>memarb_last_i</code>レジスタに、受け入れた要求が<code>i_membus</code>からのものだったかを記録する</li><li>メモリが要求の結果を返すとき、<code>memarb_last_i</code>を見て、<code>i_membus</code>と<code>d_membus</code>のどちらか片方の<code>rvalid</code>を<code>1</code>にする</li></ul><p>命令フェッチを優先しているとロードストア命令の処理が進まないため、 <code>i_membus</code>よりも<code>d_membus</code>を優先します。</p><p>coreモジュールとの接続を次のように変更します(リスト58)。</p><p><span class="caption">▼リスト3.58: membusを2つに分けて接続する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk       ,
    rst       ,
    i_membus  ,
    d_membus  ,
);
</code></pre></div><p>memoryモジュールとmemunitモジュールを接続する準備が整ったので、memunitモジュールをインスタンス化します ( リスト59、 リスト60 )。</p><p><span class="caption">▼リスト3.59: memunitモジュールで使うワイヤの定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> memu_rdata: UIntX;
<span class="hljs-keyword">var</span> memu_stall: <span class="hljs-keyword">logic</span>;
</code></pre></div><p><span class="caption">▼リスト3.60: memunitモジュールのインスタンス化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> memu: memunit (
    clk                ,
    rst                ,
    valid : inst_valid ,
    is_new: inst_is_new,
...
</code></pre></div><h4 id="memunitモジュールの処理待ちとライトバック" tabindex="-1">memunitモジュールの処理待ちとライトバック <a class="header-anchor" href="#memunitモジュールの処理待ちとライトバック" aria-label="Permalink to “memunitモジュールの処理待ちとライトバック”">​</a></h4><p>memunitモジュールが処理中のときは命令をFIFOから取り出すのを止める処理と、 ロード命令で読み込んだデータをレジスタにライトバックする処理を実装します。</p><p>memunitモジュールが処理中のとき、 FIFOから命令を取り出すのを止めます(リスト61)。</p><p><span class="caption">▼リスト3.61: memunitモジュールの処理が終わるのを待つ (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// memunitが処理中ではないとき、FIFOから命令を取り出していい</span>
if_fifo_rready = <span class="custom-hl-bold">!memu_stall</span>;
</code></pre></div><p>memunitモジュールが処理中のとき、<code>memu_stall</code>が<code>1</code>になっています。 そのため、<code>memu_stall</code>が<code>1</code>のときは<code>if_fifo_rready</code>を<code>0</code>にすることで、 FIFOからの命令の取り出しを停止します。</p><p>次に、ロード命令の結果をレジスタにライトバックします(リスト62)。 ライトバック処理では、命令がロード命令のとき(<code>inst_ctrl.is_load</code>)、 <code>memu_rdata</code>を<code>wb_data</code>に設定します。</p><p><span class="caption">▼リスト3.62: memunitモジュールの結果をライトバックする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];
...
</code></pre></div><p>ところで、現在のコードではmemunitの処理が終了していないときも値をライトバックし続けています。 レジスタへのライトバックは命令の実行が終了したときのみで良いため、 次のようにコードを変更します(リスト63)。</p><p><span class="caption">▼リスト3.63: 命令の実行が終了したときにのみライトバックする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> inst_valid <span class="custom-hl-bold">&amp;&amp; if_fifo_rready</span> &amp;&amp; inst_ctrl.rwb_en {
        regfile[rd_addr] = wb_data;
    }
}
</code></pre></div><p>デバッグ表示も同様で、 ライトバックするときにのみデバッグ表示します(リスト64)。</p><p><span class="caption">▼リスト3.64: ライトバックするときにのみデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> <span class="custom-hl-bold">if_fifo_rready &amp;&amp;</span> inst_ctrl.rwb_en {
    $display(<span class="hljs-string">&quot;  reg[%d] &lt;= %h&quot;</span>, rd_addr, wb_data);
}
</code></pre></div><h4 id="lw、sw命令のテスト" tabindex="-1">LW、SW命令のテスト <a class="header-anchor" href="#lw、sw命令のテスト" aria-label="Permalink to “LW、SW命令のテスト”">​</a></h4><p>LW命令とSW命令が正しく動作していることを確認するために、 デバッグ表示に次のコードを追加します(リスト65)。</p><p><span class="caption">▼リスト3.65: メモリモジュールの状態をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>$display(<span class="hljs-string">&quot;  mem stall : %b&quot;</span>, memu_stall);
$display(<span class="hljs-string">&quot;  mem rdata : %h&quot;</span>, memu_rdata);
</code></pre></div><p>ここからのテストは実行するクロック数が多くなります。 そこで、ログに何クロック目かを表示することでログを読みやすくします(リスト66)。</p><p><span class="caption">▼リスト3.66: 何クロック目かを出力する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">var</span> clock_count: <span class="hljs-keyword">u64</span>;</span>
...
<span class="hljs-keyword">always_ff</span> {
    ...
    <span class="custom-hl-bold">} <span class="hljs-keyword">else</span> {</span>
        ...
        <span class="hljs-keyword">if</span> inst_valid {
            ...
            $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
            $display(<span class="hljs-string">&quot;  itype     : %b&quot;</span>, inst_ctrl.itype);
</code></pre></div><p>LW、SW命令のテストのために、 <code>src/sample.hex</code>を次のように変更します(リスト67)。</p><p><span class="caption">▼リスト3.67: テスト用のプログラムを記述する (sample.hex)</span> <a href="https://github.com/nananapo/bluecore/compare/11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d~1..11a61c8f4c21d071f822dfe16d9bbdcd1f78c63d#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02002503 // lw x10, 0x20(x0)
...
00000000
deadbeef // 0x20
</code></pre></div><p>プログラムは次のようになっています(表8)。</p><div id="sample.hex.table.lwsw-range" class="table"><p class="caption">表3.8: メモリに格納する命令</p><table><tr class="hline"><th>アドレス</th><th>命令</th><th>意味</th></tr><tr class="hline"><td>0x00000000</td><td>lw x10, 0x20(x0)</td><td>x10に、アドレスが0x20のデータを読み込む</td></tr><tr class="hline"><td>0x00000004</td><td>addi x11, x0, 0x400</td><td>x11 = 0x400</td></tr><tr class="hline"><td>0x00000008</td><td>sw x11, 0x20(x0)</td><td>アドレス0x20にx11の値を書き込む</td></tr><tr class="hline"><td>0x0000000c</td><td>lw x12, 0x20(x0)</td><td>x12に、アドレスが0x20のデータを読み込む</td></tr></table></div> アドレス\`0x00000020\`には、データ\`32&#39;hdeadbeef\`を格納しています。 1つ目の命令で\`32&#39;hdeadbeef\`が読み込まれ、 3つ目の命令で\`32&#39;h00000400\`を書き込み、 4つ目の命令で\`32&#39;h00000400\`が読み込まれます。 <p>シミュレータを実行し、結果を確かめます(リスト68)。</p><p><span class="caption">▼リスト3.68: LW、SW命令のテスト</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
...
</code></pre></div><p>funct3をcase文で分岐し、 アドレスの下位ビットを見ることで、 命令とアドレスに応じた値をrdataに設定します(リスト70)。</p><p><span class="caption">▼リスト3.70: rdataをアドレスと読み込みサイズに応じて変更する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// loadの結果</span>
...
</code></pre></div><p><code>wmask</code>、wmask_expand関数をmodportに追加します ( リスト73 )。</p><p><span class="caption">▼リスト3.73: modport masterとslaveにwmask、wmask_expand関数を追加する (membus_if.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-5b8ff8b1b43eb415d367759fb46872980d16065c43c0a71a75fec85e987ad460">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">modport</span> master {
    valid       : <span class="hljs-keyword">output</span>,
    ready       : <span class="hljs-keyword">input</span> ,
    addr        : <span class="hljs-keyword">output</span>,
    wen         : <span class="hljs-keyword">output</span>,
...
<span class="hljs-keyword">modport</span> slave {
...
</code></pre></div><p><code>wmask</code>には、書き込む部分を<code>1</code>、書き込まない部分を<code>0</code>で指定します。 このような挙動をする値を、書き込みマスクと呼びます。 バイト単位で指定するため、<code>wmask</code>の幅は<code>DATA_WIDTH / 8</code>ビットです。</p><p>次に、memoryモジュールで書き込みマスクをサポートします(リスト74)。</p><p><span class="caption">▼リスト3.74: 書き込みマスクをサポートするmemoryモジュール (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
    <span class="hljs-keyword">param</span> FILEPATH_IS_ENV: <span class="hljs-keyword">logic</span>  = <span class="hljs-number">0</span> , <span class="hljs-comment">// FILEPATHが環境変数名かどうか</span>
    <span class="hljs-keyword">param</span> FILEPATH       : <span class="hljs-keyword">string</span> = <span class="hljs-string">&quot;&quot;</span>, <span class="hljs-comment">// メモリの初期化用ファイルのパス, または環境変数名</span>
) (
    clk   : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">clock</span>                                     ,
    ...
    <span class="hljs-keyword">var</span> wmask_saved: <span class="hljs-keyword">logic</span>   &lt;DATA_WIDTH&gt;;
    ...
    <span class="hljs-keyword">always_ff</span> {
        <span class="hljs-keyword">if</span> state == State::WriteValid {
            mem[addr_saved[ADDR_WIDTH - <span class="hljs-number">1</span>:<span class="hljs-number">0</span>]] = wdata_saved &amp; wmask_saved | rdata_saved &amp; ~wmask_saved;
        }
    }
...
</code></pre></div><h4 id="memunitモジュールの実装" tabindex="-1">memunitモジュールの実装 <a class="header-anchor" href="#memunitモジュールの実装" aria-label="Permalink to “memunitモジュールの実装”">​</a></h4><p>memoryモジュールが書き込みマスクをサポートしたので、 memunitモジュールで<code>wmask</code>を設定します。</p><p><code>req_wmask</code>レジスタを作成し、 <code>membus.wmask</code>と接続します ( リスト76、 リスト77 )。</p><p><span class="caption">▼リスト3.76: req_wmaskの定義 (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> req_wmask: <span class="hljs-keyword">logic</span>&lt;MEM_DATA_WIDTH / <span class="hljs-number">8</span>&gt;;
</code></pre></div><p><span class="caption">▼リスト3.77: membusにwmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// メモリアクセス</span>
membus.valid = state == State::WaitReady;
membus.addr  = req_addr;
membus.wen   = req_wen;
membus.wdata = req_wdata;
<span class="custom-hl-bold">membus.wmask = req_wmask;</span>
</code></pre></div><p>always_ffの中で、<code>req_wmask</code>の値を設定します。 それぞれの命令のとき、<code>wmask</code>がどうなるかを確認してください( リスト78、 リスト79 )。</p><p><span class="caption">▼リスト3.78: if_resetでreq_wmaskを初期化する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    state     = State::Init;
    req_wen   = <span class="hljs-number">0</span>;
    req_addr  = <span class="hljs-number">0</span>;
    req_wdata = <span class="hljs-number">0</span>;
...
</code></pre></div><p><span class="caption">▼リスト3.79: メモリにアクセスする命令のとき、wmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/662fab4a061ba80f2a188bb3c17bc886f15e11c6~1..662fab4a061ba80f2a188bb3c17bc886f15e11c6#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>req_wmask = <span class="hljs-keyword">case</span> ctrl.funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
</code></pre></div><p>次に、次にフェッチする命令をジャンプ先の命令に変更します。 フェッチ先の変更が発生を示す信号<code>control_hazard</code>と、 新しいフェッチ先を示す信号<code>control_hazard_pc_next</code>を作成します ( リスト84、 リスト85 )。</p><p><span class="caption">▼リスト3.84: control_hazardとcontrol_hazard_pc_nextの定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> control_hazard        : <span class="hljs-keyword">logic</span>;
<span class="hljs-keyword">var</span> control_hazard_pc_next: Addr ;
</code></pre></div><p><span class="caption">▼リスト3.85: control_hazardとcontrol_hazard_pc_nextの割り当て (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = inst_valid &amp;&amp; inst_ctrl.is_jump;
<span class="hljs-keyword">assign</span> control_hazard_pc_next = alu_result &amp; ~<span class="hljs-number">1</span>;
</code></pre></div><p><code>control_hazard</code>を利用して<code>if_pc</code>を更新し、 新しく命令をフェッチしなおすようにします(リスト86)。</p><p><span class="caption">▼リスト3.86: PCをジャンプ先に変更する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p>`,480),s("div",{class:"language-veryl"},[s("button",{title:"Copy Code",class:"copy"}),s("span",{class:"lang"},"veryl"),s("pre",{class:"hljs"},[s("code",null,[s("span",{class:"hljs-keyword"},"always_ff"),a(` {
...
`)])])],-1),e('<p>ここで、新しく命令をフェッチしなおすようにしても、 ジャンプ命令によって実行されることがなくなった命令がFIFOに残っていることがあることに注意する必要があります(図4)。</p><p><img src="'+r+`" alt="ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図4: ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている</p><p>実行するべきではない命令を実行しないようにするために、 ジャンプ命令を実行するときに、FIFOをリセットします。</p><p>FIFOに、中身をリセットするための信号<code>flush</code>を実装します(リスト87)。</p><p><span class="caption">▼リスト3.87: ポートにflushを追加する (fifo.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> fifo #(
    <span class="hljs-keyword">param</span> DATA_TYPE: <span class="hljs-keyword">type</span> = <span class="hljs-keyword">logic</span>,
    <span class="hljs-keyword">param</span> WIDTH    : <span class="hljs-keyword">u32</span>  = <span class="hljs-number">2</span>    ,
) (
    clk       : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>        ,
    ...
    wready    : <span class="hljs-keyword">output</span> <span class="hljs-keyword">logic</span>        ,
</code></pre></div><p><code>flush</code>が<code>1</code>のとき、 <code>head</code>と<code>tail</code>を<code>0</code>に初期化することでFIFOを空にします(リスト88、リスト89)。</p><p><span class="caption">▼リスト3.88: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH==1)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        rdata  = <span class="hljs-number">0</span>;
        rvalid = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p><span class="caption">▼リスト3.89: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH!=1)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        head = <span class="hljs-number">0</span>;
        tail = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p>coreモジュールで、 <code>control_hazard</code>と<code>flush</code>を接続し、 FIFOをリセットします(リスト90)。</p><p><span class="caption">▼リスト3.90: ジャンプ命令のとき、FIFOをリセットする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> if_fifo: fifo #(
    DATA_TYPE: if_fifo_type,
    WIDTH    : <span class="hljs-number">3</span>           ,
) (
    clk                           ,
...
</code></pre></div><h4 id="無条件ジャンプのテスト" tabindex="-1">無条件ジャンプのテスト <a class="header-anchor" href="#無条件ジャンプのテスト" aria-label="Permalink to “無条件ジャンプのテスト”">​</a></h4><p>簡単なテストを作成し、動作をテストします(リスト91、リスト92)。</p><p><span class="caption">▼リスト3.91: sample_jump.hex</span> <a href="https://github.com/nananapo/bluecore/compare/79ab1d8034f77714266f15d9ba4a909a848b7f65~1..79ab1d8034f77714266f15d9ba4a909a848b7f65#diff-4b870828046f4065482146d5dd02dccb5d0523eabe5af669e953963404e810ac">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>0100006f //  0: jal x0, 0x10 : 0x10にジャンプする
deadbeef //  4:
deadbeef //  8:
deadbeef //  c:
...
</code></pre></div><p><span class="caption">▼リスト3.92: テストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
...
</code></pre></div><p>brunitモジュールは、 <code>funct3</code>に応じて<code>take</code>の条件を切り替えます。 分岐が成立するときに<code>take</code>が<code>1</code>になります。</p><p>brunitモジュールを、 coreモジュールでインスタンス化します(リスト94)。 命令がB形式のとき、 <code>op1</code>は<code>rs1_data</code>、 <code>op2</code>は<code>rs2_data</code>になっていることを確認してください(リスト43)。</p><p><span class="caption">▼リスト3.94: brunitモジュールのインスタンス化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/41ea1578d04a9ccf531e712e9708fb4094b9b7ac~1..41ea1578d04a9ccf531e712e9708fb4094b9b7ac#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> brunit_take: <span class="hljs-keyword">logic</span>;
...
</code></pre></div><p>命令が条件分岐命令で<code>brunit_take</code>が<code>1</code>のとき、 次のPCをPC + 即値にします ( リスト95、 リスト96 )。</p><p><span class="caption">▼リスト3.95: 命令が条件分岐命令か判定する関数 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/41ea1578d04a9ccf531e712e9708fb4094b9b7ac~1..41ea1578d04a9ccf531e712e9708fb4094b9b7ac#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令が分岐命令かどうかを判定する</span>
...
</code></pre></div><p><span class="caption">▼リスト3.96: 分岐成立時のPCの設定 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/41ea1578d04a9ccf531e712e9708fb4094b9b7ac~1..41ea1578d04a9ccf531e712e9708fb4094b9b7ac#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = inst_valid &amp;&amp; (inst_ctrl.is_jump || inst_is_br(inst_ctrl) &amp;&amp; brunit_take);
<span class="hljs-keyword">assign</span> control_hazard_pc_next = <span class="custom-hl-bold"><span class="hljs-keyword">if</span> inst_is_br(inst_ctrl) ? inst_pc + inst_imm :</span> alu_result &amp; ~<span class="hljs-number">1</span>;
</code></pre></div><p><code>control_hazard</code>は、命令が無条件ジャンプ命令か、命令が条件分岐命令かつ分岐が成立するときに<code>1</code>になります。 <code>control_hazard_pc_next</code>は、無条件ジャンプ命令のときは<code>alu_result</code>、条件分岐命令のときはPC + 即値になります。</p><h4 id="条件分岐命令のテスト" tabindex="-1">条件分岐命令のテスト <a class="header-anchor" href="#条件分岐命令のテスト" aria-label="Permalink to “条件分岐命令のテスト”">​</a></h4><p>条件分岐命令を実行するとき、分岐の成否をデバッグ表示します。 デバッグ表示を行っているalways_ffブロック内に、次のコードを追加します(リスト97)。</p><p><span class="caption">▼リスト3.97: 分岐判定のデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/41ea1578d04a9ccf531e712e9708fb4094b9b7ac~1..41ea1578d04a9ccf531e712e9708fb4094b9b7ac#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-very"><button title="Copy Code" class="copy"></button><span class="lang">very</span><pre class="hljs"><code>if inst_is_br(inst_ctrl) {
    $display(&quot;  br take   : %b&quot;, brunit_take);
}
</code></pre></div><p>簡単なテストを作成し、動作をテストします(リスト98, リスト99)。</p><p><span class="caption">▼リスト3.98: sample_br.hex</span> <a href="https://github.com/nananapo/bluecore/compare/41ea1578d04a9ccf531e712e9708fb4094b9b7ac~1..41ea1578d04a9ccf531e712e9708fb4094b9b7ac#diff-ed70829e581bd3b76a3741500395b0a2d55950437f287a0db0b81ae8b589e4e0">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>00100093 //  0: addi x1, x0, 1
...
</code></pre></div><p><span class="caption">▼リスト3.99: テストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
