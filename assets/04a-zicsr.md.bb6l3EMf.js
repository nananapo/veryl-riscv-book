import{_ as a,c,o as n,ah as e,at as p,au as l,av as t}from"./chunks/framework.BNheOMQd.js";const u=JSON.parse('{"title":"Zicsr拡張の実装","description":"","frontmatter":{},"headers":[],"relativePath":"04a-zicsr.md","filePath":"04a-zicsr.md"}'),d={name:"04a-zicsr.md"};function r(o,s,i,b,h,f){return n(),c("div",null,[...s[0]||(s[0]=[e(`<h1 id="zicsr拡張の実装" tabindex="-1">Zicsr拡張の実装 <a class="header-anchor" href="#zicsr拡張の実装" aria-label="Permalink to “Zicsr拡張の実装”">​</a></h1><h2 id="csrとは何か" tabindex="-1">CSRとは何か? <a class="header-anchor" href="#csrとは何か" aria-label="Permalink to “CSRとは何か?”">​</a></h2><p>前の章では、RISC-Vの基本整数命令セットであるRV32Iを実装しました。 既に簡単なプログラムを動かせますが、 例外や割り込み、ページングなどの機能がありません<sup class="footnote-ref"><a href="#fn1" id="fnref1">[1]</a></sup>。 このような機能はCSRを介して提供されます。</p><p>RISC-Vには、CSR(Control and Status Register)というレジスタが4096個存在しています。 例えば<code>mtvec</code>というレジスタは、例外や割り込みが発生したときのジャンプ先のアドレスを格納しています。 RISC-VのCPUは、CSRの読み書きによって制御(Control)や状態(Status)の読み取りを行います。</p><p>CSRの読み書きを行う命令は、Zicsr拡張によって定義されています(表1)。 本章では、Zicsrに定義されている命令、 RV32Iに定義されているECALL命令、 MRET命令、 mtvecレジスタ、 mepcレジスタ、 mcauseレジスタを実装します。</p><div id="zicsr.insts" class="table"><p class="caption">表4.1: Zicsr拡張に定義されている命令</p><table><tr class="hline"><th>命令</th><th>作用</th></tr><tr class="hline"><td>CSRRW</td><td>CSRにrs1を書き込み、元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRWI</td><td>CSRRWのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr><tr class="hline"><td>CSRRS</td><td>CSRとrs1をビットORした値をCSRに書き込み、元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRSI</td><td>CSRRSのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr><tr class="hline"><td>CSRRC</td><td>CSRと~rs1(rs1のビットNOT)をビットANDした値をCSRに書き込み、<br>元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRCI</td><td>CSRRCのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr></table></div><h2 id="csr命令のデコード" tabindex="-1">CSR命令のデコード <a class="header-anchor" href="#csr命令のデコード" aria-label="Permalink to “CSR命令のデコード”">​</a></h2><p>まず、Zicsrに定義されている命令(表1)をデコードします。</p><p>これらの命令のopcodeは<code>SYSTEM</code>(<code>7&#39;b1110011</code>)です。 この値をeeiパッケージに定義します(リスト1)。</p><p><span class="caption">▼リスト4.1: opcode用の定数の定義 (eei.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3fc242f4aa33aeda960e3a4c71b90b77585de130~1..3fc242f4aa33aeda960e3a4c71b90b77585de130#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">const</span> OP_SYSTEM: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">7</span>&gt; = <span class="hljs-number">7&#39;b1110011</span>;
</code></pre></div><p>次に、<code>InstCtrl</code>構造体に、 CSRを制御する命令であることを示す<code>is_csr</code>フラグを追加します(リスト2)。</p><p><span class="caption">▼リスト4.2: is_csrを追加する (corectrl.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3fc242f4aa33aeda960e3a4c71b90b77585de130~1..3fc242f4aa33aeda960e3a4c71b90b77585de130#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 制御に使うフラグ用の構造体</span>
...
</code></pre></div><p>これでデコード処理を書く準備が整いました。 inst_decoderモジュールの<code>InstCtrl</code>を生成している部分を変更します(リスト3)。</p><p><span class="caption">▼リスト4.3: OP_SYSTEMとis_csrを追加する (inst_decoder.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3fc242f4aa33aeda960e3a4c71b90b77585de130~1..3fc242f4aa33aeda960e3a4c71b90b77585de130#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>                                        ↓ is_csrを追加
ctrl = {
...
</code></pre></div><p>最後に、デバッグ用の表示を追加します。 デバッグ表示用のalways_ffブロックに、 次のコードを追加してください(リスト7)。</p><p><span class="caption">▼リスト4.7: rdataをデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/3fc242f4aa33aeda960e3a4c71b90b77585de130~1..3fc242f4aa33aeda960e3a4c71b90b77585de130#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.is_csr {
    $display(<span class="hljs-string">&quot;  csr rdata : %h&quot;</span>, csru_rdata);
}
</code></pre></div><p>これらのテストは、 csrunitモジュールにCSRを追加してから行います。</p><h3 id="mtvecレジスタを実装する" tabindex="-1">mtvecレジスタを実装する <a class="header-anchor" href="#mtvecレジスタを実装する" aria-label="Permalink to “mtvecレジスタを実装する”">​</a></h3><p>csrunitモジュールには、まだCSRが定義されていません。 1つ目のCSRとして、mtvecレジスタを実装します。</p><h4 id="mtvecレジスタ、トラップ" tabindex="-1">mtvecレジスタ、トラップ <a class="header-anchor" href="#mtvecレジスタ、トラップ" aria-label="Permalink to “mtvecレジスタ、トラップ”">​</a></h4><p><img src="`+p+`" alt="mtvecのエンコーディング"> mtvecは、MXLENビットのWARLなレジスタです。 mtvecのアドレスは<code>12&#39;h305</code>です。</p><p>MXLENはmisaレジスタに定義されていますが、 今のところはXLENと等しいという認識で問題ありません。 WARLはWrite Any Values, Reads Legal Valuesの略です。 その名の通り、好きな値を書き込めますが 読み出すときには合法な値<sup class="footnote-ref"><a href="#fn2" id="fnref2">[2]</a></sup>になっているという認識で問題ありません。</p><p>mtvecは、トラップ(Trap)が発生したときのジャンプ先(Trap-Vector)の基準となるアドレスを格納するレジスタです。 <strong>トラップ</strong>とは、例外(Exception)、または割り込み(Interrupt)により、 CPUの制御を変更することです<sup class="footnote-ref"><a href="#fn3" id="fnref3">[3]</a></sup>。 トラップが発生するとき、CPUはCSRを変更した後、 mtvecに格納されたアドレスにジャンプします。</p><p><strong>例外</strong>とは、命令の実行によって引き起こされる異常な状態(unusual condition)のことです。 例えば、不正な命令を実行しようとしたときにはIllegal Instruction例外が発生します。 CPUは、例外が発生したときのジャンプ先(対処方法)を決めておくことで、 CPUが異常な状態に陥ったままにならないようにしています。</p><p>mtvecはBASEとMODEの2つのフィールドで構成されています。 MODEはジャンプ先の決め方を指定するためのフィールドですが、 簡単のために常に<code>2&#39;b00</code>(Directモード)になるようにします。 Directモードのとき、トラップ時のジャンプ先は<code>BASE &lt;&lt; 2</code>になります。</p><h4 id="mtvecレジスタの実装" tabindex="-1">mtvecレジスタの実装 <a class="header-anchor" href="#mtvecレジスタの実装" aria-label="Permalink to “mtvecレジスタの実装”">​</a></h4><p>それでは、mtvecレジスタを実装します。 まず、CSRのアドレスを表す列挙型を定義します (リスト8)。</p><p><span class="caption">▼リスト4.8: CsrAddr型を定義する (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/259a8236483109c159bf9cce28e0ee05cbfa071a~1..259a8236483109c159bf9cce28e0ee05cbfa071a#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// CSRのアドレス</span>
...
</code></pre></div><p>次に、mtvecレジスタを作成します。 MXLEN=XLENとしているので、 型は<code>UIntX</code>にします (リスト9)。</p><p><span class="caption">▼リスト4.9: mtvecレジスタの定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/259a8236483109c159bf9cce28e0ee05cbfa071a~1..259a8236483109c159bf9cce28e0ee05cbfa071a#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// CSR</span>
<span class="hljs-keyword">var</span> mtvec: UIntX;
</code></pre></div><p>MODEはDirectモード(<code>2&#39;b00</code>)しか対応していません。 mtvecはWARLなレジスタなので、 MODEフィールドには書き込めないようにする必要があります。 これを制御するためにmtvecレジスタの書き込みマスク用の定数を定義します (リスト10)。</p><p><span class="caption">▼リスト4.10: mtvecレジスタの書き込みマスクの定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/259a8236483109c159bf9cce28e0ee05cbfa071a~1..259a8236483109c159bf9cce28e0ee05cbfa071a#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// wmasks</span>
<span class="hljs-keyword">const</span> MTVEC_WMASK: UIntX = <span class="hljs-number">&#39;hffff_fffc</span>;
</code></pre></div><p>次に、書き込むデータ<code>wdata</code>の生成と、 mtvecレジスタの読み込みを実装します(リスト11)。</p><p><span class="caption">▼リスト4.11: レジスタの読み込みと書き込むデータの作成 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/259a8236483109c159bf9cce28e0ee05cbfa071a~1..259a8236483109c159bf9cce28e0ee05cbfa071a#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> wmask: UIntX; <span class="hljs-comment">// write mask</span>
...
</code></pre></div><p>mtvecの初期値は<code>0</code>です。 mtvecに<code>wdata</code>を書き込むとき、 MODEが常に<code>2&#39;b00</code>になります。</p><h3 id="csrunitモジュールをテストする" tabindex="-1">csrunitモジュールをテストする <a class="header-anchor" href="#csrunitモジュールをテストする" aria-label="Permalink to “csrunitモジュールをテストする”">​</a></h3><p>mtvecレジスタの書き込み、 読み込みができることを確認します。</p><p><code>test/sample_csr.hex</code>を作成し、 次のように記述します(リスト13)。</p><p><span class="caption">▼リスト4.13: sample_csr.hex</span> <a href="https://github.com/nananapo/bluecore/compare/259a8236483109c159bf9cce28e0ee05cbfa071a~1..259a8236483109c159bf9cce28e0ee05cbfa071a#diff-47d9e90d4876f4a6f796e2e1837db907c2838c0df7f630a81f65fc98cbed5c76">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>305bd0f3 // 0: csrrwi x1, mtvec, 0b10111
...
</code></pre></div><p>次に、トラップの発生原因を表現する型<code>CsrCause</code>を定義します。 今のところ、発生原因はECALL命令によるEnvironment Call From M-mode例外しかありません (リスト16)。</p><p><span class="caption">▼リスト4.16: CsrCause型の定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">enum</span> CsrCause: UIntX {
    ENVIRONMENT_CALL_FROM_M_MODE = <span class="hljs-number">11</span>,
}
</code></pre></div><p>最後に、mepcとmcauseの書き込みマスクを定義します (リスト17)。 mepcに格納されるのは例外が発生した時の命令のアドレスです。 命令は4バイトに整列して配置されているため、 mepcの下位2ビットは常に<code>2&#39;b00</code>になるようにします。</p><p><span class="caption">▼リスト4.17: mepcとmcauseの書き込みマスクの定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">const</span> MTVEC_WMASK : UIntX = <span class="hljs-number">&#39;hffff_fffc</span>;
...
</code></pre></div><p>次に、mepcとmcauseの読み込み処理と、書き込みマスクの割り当てを実装します。 どちらもcase文にアドレスと値のペアを追加するだけです ( リスト19、 リスト20 )。</p><p><span class="caption">▼リスト4.19: mepcとmcauseの読み込み (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>rdata = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : mtvec,
    <span class="custom-hl-bold">CsrAddr::MEPC  : mepc,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: mcause,</span>
    <span class="hljs-keyword">default</span>        : &#39;x,
};
</code></pre></div><p><span class="caption">▼リスト4.20: mepcとmcauseの書き込みマスクの設定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>wmask = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : MTVEC_WMASK,
    <span class="custom-hl-bold">CsrAddr::MEPC  : MEPC_WMASK,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: MCAUSE_WMASK,</span>
    <span class="hljs-keyword">default</span>        : <span class="hljs-number">0</span>,
};
</code></pre></div><p>最後に、mepcとmcauseの書き込みを実装します。 if_resetで値を<code>0</code>に初期化し、case文にmepcとmcauseの場合を実装します (リスト21)。</p><p><span class="caption">▼リスト4.21: mepcとmcauseの書き込み (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        mtvec  = <span class="hljs-number">0</span>;
        <span class="custom-hl-bold">mepc   = <span class="hljs-number">0</span>;</span>
        ...
        <span class="hljs-keyword">if</span> valid {
            <span class="hljs-keyword">if</span> is_wsc {
                <span class="hljs-keyword">case</span> csr_addr {
                    CsrAddr::MTVEC : mtvec  = wdata;
                    <span class="custom-hl-bold">CsrAddr::MEPC  : mepc   = wdata;</span>
...
</code></pre></div><h4 id="例外の実装" tabindex="-1">例外の実装 <a class="header-anchor" href="#例外の実装" aria-label="Permalink to “例外の実装”">​</a></h4><p>ECALL命令と、それによって発生するトラップを実装します。 まず、csrunitモジュールにポートを追加します (リスト22)。</p><p><span class="caption">▼リスト4.22: csrunitモジュールにポートを追加する (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> csrunit (
    clk        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>       ,
    rst        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">reset</span>       ,
    valid      : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">logic</span>       ,
...
</code></pre></div><p>それぞれの用途は次の通りです。</p><dl><dt>pc</dt><dd> 現在処理している命令のアドレスを受け取ります。<br> 例外が発生するとき、mepcにPCを格納するために使います。 </dd><dt>rd_addr</dt><dd> 現在処理している命令のrdの番号を受け取ります。<br> 命令がECALL命令かどうかを判定するために使います。 </dd><dt>raise_trap</dt><dd> 例外が発生するとき、値を\`1\`にします。 </dd><dt>trap_vector</dt><dd> 例外が発生するとき、ジャンプ先のアドレスを出力します。 </dd></dl><p>csrunitモジュールの中身を実装する前に、 coreモジュールに例外発生時の動作を実装します。</p><p>csrunitモジュールと接続するための変数を定義してcsrunitモジュールと接続します ( リスト23、 リスト24 )。</p><p><span class="caption">▼リスト4.23: csrunitモジュールのポートの定義を変更する ① (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> csru_rdata      : UIntX;
...
</code></pre></div><p><span class="caption">▼リスト4.24: csrunitモジュールのポートの定義を変更する ② (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> csru: csrunit (
    clk                       ,
    rst                       ,
    valid   : inst_valid      ,
...
</code></pre></div><p><img src="`+l+`" alt="ECALL命令のエンコーディング"> それでは、csrunitモジュールにトラップの処理を実装します。</p><p>ECALL命令は、 I形式、 即値は<code>0</code>、 rs1とrdは<code>0</code>、 funct3は<code>0</code>、 opcodeは<code>SYSTEM</code>な命令です(図2)。 これを判定するための変数を作成します(リスト26)。</p><p><span class="caption">▼リスト4.26: ECALL命令かどうかの判定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// ECALL命令かどうか</span>
...
</code></pre></div><p>次に、例外が発生するかどうかを示す<code>raise_expt</code>と、 例外の発生の原因を示す<code>expt_cause</code>を作成します。 今のところ、例外はECALL命令によってのみ発生するため、 <code>expt_cause</code>は実質的に定数になっています (リスト27)。</p><p><span class="caption">▼リスト4.27: 例外とトラップの判定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// Exception</span>
...
</code></pre></div><p>トラップが発生するかどうかを示す<code>raise_trap</code>には、 例外が発生するかどうかを割り当てます。 トラップの原因を示す<code>trap_cause</code>には、 例外の発生原因を割り当てます。 また、トラップ先には<code>mtvec</code>を割り当てます。</p><p>最後に、トラップに伴うCSRの変更を実装します。 トラップが発生するとき、 mepcレジスタにPC、 mcauseレジスタにトラップの発生原因を格納します (リスト28)。</p><p><span class="caption">▼リスト4.28: トラップが発生したらCSRを変更する (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        ...
    } <span class="hljs-keyword">else</span> {
        <span class="hljs-keyword">if</span> valid {
            <span class="custom-hl-bold"><span class="hljs-keyword">if</span> raise_trap {</span> ← トラップ時の動作
                <span class="custom-hl-bold"><span class="hljs-keyword">if</span> raise_expt {</span> ← 例外時の動作
            ...
            <span class="custom-hl-bold">} <span class="hljs-keyword">else</span> {</span>
                <span class="hljs-keyword">if</span> is_wsc {
                    ...
</code></pre></div><h3 id="ecall命令をテストする" tabindex="-1">ECALL命令をテストする <a class="header-anchor" href="#ecall命令をテストする" aria-label="Permalink to “ECALL命令をテストする”">​</a></h3><p>ECALL命令をテストする前に、 デバッグのために<code>$display</code>システムタスクで、 例外が発生したかどうかと、 トラップ先を表示します (リスト29)。</p><p><span class="caption">▼リスト4.29: トラップの情報をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.is_csr {
    $display(<span class="hljs-string">&quot;  csr rdata : %h&quot;</span>, csru_rdata);
...
</code></pre></div><p><code>test/sample_ecall.hex</code>を作成し、 次のように記述します (リスト30)。</p><p><span class="caption">▼リスト4.30: sample_ecall.hex</span> <a href="https://github.com/nananapo/bluecore/compare/61c47783f63c27d5af9c3c3de10adf512ebbca69~1..61c47783f63c27d5af9c3c3de10adf512ebbca69#diff-818919fa8011230661860f6c1f57b536be1be4559c0ce404f6fd655e4616735b">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>30585073 //  0: csrrwi x0, mtvec, 0x10
00000073 //  4: ecall
00000000 //  8:
00000000 //  c:
342020f3 // 10: csrrs x1, mcause, x0
34102173 // 14: csrrs x2, mepc, x0
</code></pre></div><p>CSRRWI命令でmtvecレジスタに値を書き込み、 ECALL命令で例外を発生させてジャンプします。 ジャンプ先では、 mcauseレジスタとmepcレジスタの値を読み取ります。</p><p>シミュレータを実行し、結果を確かめます(リスト31)。</p><p><span class="caption">▼リスト4.31: ECALL命令のテストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
...
</code></pre></div><p>ECALL命令によって例外が発生し、 mcauseとmepcに書き込みが行われてからmtvecにジャンプしていることを確認できます。</p><p>ECALL命令の実行時にレジスタに値がライトバックされてしまっていますが、 ECALL命令のrdは常に0番目のレジスタであり、 0番目のレジスタは常に値が<code>0</code>になるため問題ありません。</p><h2 id="mret命令の実装" tabindex="-1">MRET命令の実装 <a class="header-anchor" href="#mret命令の実装" aria-label="Permalink to “MRET命令の実装”">​</a></h2><p>MRET命令<sup class="footnote-ref"><a href="#fn4" id="fnref4">[4]</a></sup>は、 トラップ先からトラップ元に戻るための命令です。 MRET命令を実行すると、 mepcレジスタに格納されたアドレスにジャンプします<sup class="footnote-ref"><a href="#fn5" id="fnref5">[5]</a></sup>。 例えば、権限のあるOSから権限のないユーザー空間に戻るために利用します。</p><h3 id="mret命令を実装する" tabindex="-1">MRET命令を実装する <a class="header-anchor" href="#mret命令を実装する" aria-label="Permalink to “MRET命令を実装する”">​</a></h3><p><img src="`+t+`" alt="MRET命令のエンコーディング"> まず、 csrunitモジュールに供給されている命令がMRET命令かどうかを判定する変数<code>is_mret</code>を作成します (リスト32)。 MRET命令は、上位12ビットは<code>12&#39;b001100000010</code>、 rs1は<code>0</code>、funct3は<code>0</code>、rdは<code>0</code>です(図3)。</p><p><span class="caption">▼リスト4.32: MRET命令の判定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba~1..909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// MRET命令かどうか</span>
...
</code></pre></div><p>次に、 csrunitモジュールにMRET命令が供給されているときにmepcにジャンプする仕組みを実装します。 ジャンプするための仕組みには、トラップによってジャンプする仕組みを利用します (リスト33)。 <code>raise_trap</code>に<code>is_mret</code>を追加し、トラップ先も変更します。</p><p><span class="caption">▼リスト4.33: MRET命令によってジャンプさせる (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba~1..909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// Trap</span>
...
</code></pre></div><div class="info custom-block"><p class="custom-block-title"><b>例外が優先</b></p><p>trap_vectorには、 <code>is_mret</code>のときに<code>mepc</code>を割り当てるのではなく、 <code>raise_expt</code>のときに<code>mtvec</code>を割り当てています。 これは、MRET命令によって発生する例外があるからです。 MRET命令の判定を優先すると、例外が発生するのにmepcにジャンプしてしまいます。</p></div><h3 id="mret命令をテストする" tabindex="-1">MRET命令をテストする <a class="header-anchor" href="#mret命令をテストする" aria-label="Permalink to “MRET命令をテストする”">​</a></h3><p>mepcに値を設定してからMRET命令を実行することで mepcにジャンプするようなテストを作成します (リスト34)。</p><p><span class="caption">▼リスト4.34: sample_mret.hex</span> <a href="https://github.com/nananapo/bluecore/compare/909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba~1..909f1a6fd3b7d03ab8fdf5574c98f9464729d3ba#diff-32b144559a214c9ebf3ea88eeeb575aa9aaec2d41f76a6e875c34e8726a6be17">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>34185073 //  0: csrrwi x0, mepc, 0x10
30200073 //  4: mret
00000000 //  8:
00000000 //  c:
00000013 // 10: addi x0, x0, 0
</code></pre></div><p>シミュレータを実行し、結果を確かめます(リスト35)。</p><p><span class="caption">▼リスト4.35: MRET命令のテストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
