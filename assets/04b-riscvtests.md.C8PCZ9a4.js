...
</code></pre></div><p><code>test_success</code>はポートとして定義します (リスト10)。</p><p><span class="caption">▼リスト5.10: テスト結果を報告するためのポートを宣言する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7ef221face7584354a0e0b1d0f4f166fc3e595be~1..7ef221face7584354a0e0b1d0f4f166fc3e595be#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> top #(
...
</code></pre></div><p><code>riscv-tests success!</code>と表示され、テストが正常終了しました<sup class="footnote-ref"><a href="#fn2" id="fnref2">[2]</a></sup>。</p><h2 id="複数のテストの自動実行" tabindex="-1">複数のテストの自動実行 <a class="header-anchor" href="#複数のテストの自動実行" aria-label="Permalink to “複数のテストの自動実行”">​</a></h2><p>ADD命令以外の命令もテストしたいですが、わざわざコマンドを手打ちしたくありません。 自動でテストを実行して、その結果を報告するプログラムを作成しましょう。</p><p><code>test/test.py</code>を作成し、次のように記述します(リスト12)。</p><p><span class="caption">▼リスト5.12: test.py</span> <a href="https://github.com/nananapo/bluecore/compare/0a2826b3b2b19b515eda2450c1aed8c43d6db65d~1..0a2826b3b2b19b515eda2450c1aed8c43d6db65d#diff-354b34bf98c7e65e1214431540698436073d5e2caf243680551ce1014ba0afde">差分をみる</a></p><div class="language-py"><button title="Copy Code" class="copy"></button><span class="lang">py</span><pre class="hljs"><code><span class="hljs-keyword">import</span> argparse
...
</code></pre></div><p>このPythonプログラムは、 第2引数で指定したディレクトリに存在する、 第3引数で指定した文字列を名前に含むファイルを、 第1引数で指定したシミュレータで実行し、 その結果を報告します。</p><p>次のオプションの引数が存在します。</p><dl><dt>-r</dt><dd> 第2引数で指定されたディレクトリの中にあるディレクトリも走査します。 デフォルトでは走査しません。 </dd><dt>-e 拡張子</dt><dd> 指定した拡張子のファイルのみを対象にテストします。 HEXファイルをテストしたい場合は、\`-e hex\`にします。 デフォルトでは\`hex\`が指定されています。 </dd><dt>-o ディレクトリ</dt><dd> 指定したディレクトリにテスト結果を格納します。 デフォルトでは\`result\`ディレクトリに格納します。 </dd><dt>-t 時間</dt><dd> テストに時間制限を設けます。 0を指定すると時間制限はなくなります。 デフォルト値は10(秒)です。 </dd></dl><p>テストが成功したか失敗したかの判定には、 シミュレータの終了コードを利用しています。 テストが失敗したときに終了コードが<code>1</code>になるように、 Verilatorに渡しているC++プログラムを変更します (リスト13)。</p><p><span class="caption">▼リスト5.13: tb_verilator.cpp</span> <a href="https://github.com/nananapo/bluecore/compare/7ef221face7584354a0e0b1d0f4f166fc3e595be~1..7ef221face7584354a0e0b1d0f4f166fc3e595be#diff-cb1447535f2400768c9115df1ccdcc7be37fd2670577cc465b330f5968ee3013">差分をみる</a></p><div class="language-cpp"><button title="Copy Code" class="copy"></button><span class="lang">cpp</span><pre class="hljs"><code><span class="hljs-meta">#<span class="hljs-keyword">ifdef</span> TEST_MODE</span>
...
Test Result : 39 / 40
</code></pre></div><p><code>rv32ui-p-</code>から始まる40個のテストの内、39個のテストに成功しました。 テストの詳細な結果はresultsディレクトリに格納されています。</p><p><code>rv32ui-p-ma_data</code>は、 ロードストアするサイズに整列されていないアドレスへのロードストア命令のテストです。 これは後の章で例外として対処するため、今は無視します。</p><hr class="footnotes-sep"><section class="footnotes"><ol class="footnotes-list"><li id="fn1" class="footnote-item"><p>ELF(Executable and Linkable Format)とは実行可能ファイルの形式です <a href="#fnref1" class="footnote-backref">↩︎</a></p></li><li id="fn2" class="footnote-item"><p>実行が終了しない場合はどこかしらにバグがあります。rv32ui-p-add.dumpと実行ログを見比べて、頑張って原因を探してください <a href="#fnref2" class="footnote-backref">↩︎</a></p></li></ol></section>`,74)])])}const b=a(p,[["render",l]]);export{u as __pageData,b as default};
