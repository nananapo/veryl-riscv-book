...
</code></pre></div><p>inst_decoderモジュールの<code>InstCtrl</code>を生成している部分を変更します。 opcodeが<code>OP</code>か<code>OP-32</code>の場合はfunct7の値によって<code>is_muldiv</code>を設定します(リスト3)。 その他のopcodeの<code>is_muldiv</code>は<code>F</code>に設定してください。</p><p><span class="caption">▼リスト9.3: is_muldivを設定する (inst_decoder.veryl) (一部)</span> <a href="https://github.com/nananapo/bluecore/compare/dbe55b8af409ba7f5908c3c598f0c07c169274ae~1..dbe55b8af409ba7f5908c3c598f0c07c169274ae#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>OP_OP: {
...
<span class="hljs-keyword">var</span> exs_muldiv_stall       : <span class="hljs-keyword">logic</span>;
...
</code></pre></div><p>muldivunitモジュールで計算を開始するのは、 EXステージに命令が存在し(<code>exs_valid</code>)、 命令がM拡張の命令であり(<code>exs_ctrl.is_muldiv</code>)、 データハザードが発生しておらず(<code>!exs_data_hazard</code>)、 まだ計算を要求していない場合です。 これらの条件を<code>exs_muldiv_valid</code>に設定します(リスト6)。</p><p><span class="caption">▼リスト9.6: 計算の要求、ストールの条件の設定 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/dbe55b8af409ba7f5908c3c598f0c07c169274ae~1..dbe55b8af409ba7f5908c3c598f0c07c169274ae#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
    exs_muldiv_valid = <span class="hljs-number">0</span>;
    exs_muldiv_stall = <span class="hljs-number">0</span>;
    <span class="hljs-keyword">if</span> (exs_valid &amp;&amp; exs_ctrl.is_muldiv) {
...
</code></pre></div><p>muldivunitモジュールはALUモジュールのように1クロックの間に入力から出力を生成しないため、 計算中はEXステージをストールさせる必要があります。 ストールするかを示す<code>exs_muldiv_stall</code>変数を定義して、EXステージがストールする条件に追加します( リスト6、 リスト8 )。</p><p><code>exs_muldiv_is_requested</code>変数を定義し、 ステージの遷移条件とmuldivunitに計算を要求したかによって値を更新します(リスト7)。 また、すでに計算が完了しているかを示す<code>exs_muldiv_rvalided</code>変数を定義し、 muldivunitモジュールの<code>rvalid</code>が<code>1</code>になったかを観測します。</p><p><span class="caption">▼リスト9.7: 計算の要求、結果の状態の管理 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/dbe55b8af409ba7f5908c3c598f0c07c169274ae~1..dbe55b8af409ba7f5908c3c598f0c07c169274ae#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        ...
        <span class="hljs-keyword">if</span> exq_rvalid &amp;&amp; exq_rready {
            ...
            <span class="hljs-keyword">if</span> exs_muldiv_is_requested {
...
</code></pre></div><p>M拡張の命令のとき、MEMステージに渡す<code>alu_result</code>の値をmuldivunitモジュールの結果に設定します(リスト8)。</p><p><span class="caption">▼リスト9.8: EXステージのストール条件の変更とM拡張の命令の結果の設定 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/dbe55b8af409ba7f5908c3c598f0c07c169274ae~1..dbe55b8af409ba7f5908c3c598f0c07c169274ae#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">let</span> exs_stall: <span class="hljs-keyword">logic</span> = exs_data_hazard || exs_muldiv_stall;</span>
...
</code></pre></div><p>mulunitモジュールは<code>op1 * op2</code>を計算するモジュールです。 <code>valid</code>が<code>1</code>になったら計算を開始し、 計算が完了したら<code>rvalid</code>を<code>1</code>、<code>result</code>を<code>WIDTH * 2</code>ビットの計算結果に設定します。</p><p>積は<code>WIDTH</code>回の足し算を<code>WIDTH</code>クロックかけて行って求めています(図1)。 計算を開始すると入力をゼロで<code>WIDTH * 2</code>ビットに拡張し、 <code>result</code>を<code>0</code>でリセットします。</p><p><code>State::AddLoop</code>では、次の操作を<code>WIDTH</code>回行います。 <code>i</code>回目では次の操作を行います。</p><ol><li><code>op2[i-1]</code>が<code>1</code>なら<code>result</code>に<code>op1</code>を足す</li><li><code>op1</code>を1ビット左シフトする</li><li>カウンタをインクリメントする</li></ol><p><img src="`+d+`" alt="符号無し4ビットの乗算"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図1: 符号無し4ビットの乗算</p><h3 id="mulunitモジュールをインスタンス化する" tabindex="-1">mulunitモジュールをインスタンス化する <a class="header-anchor" href="#mulunitモジュールをインスタンス化する" aria-label="Permalink to “mulunitモジュールをインスタンス化する”">​</a></h3><p>mulunitモジュールをmuldivunitモジュールでインスタンス化します (リスト10)。 まだ結果は利用しません。</p><p><span class="caption">▼リスト9.10: mulunitモジュールをインスタンス化する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7672ebd17718187c6ce898f8a2ce405f1e262e51~1..7672ebd17718187c6ce898f8a2ce405f1e262e51#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// multiply unit</span>
...
</code></pre></div><h2 id="mulhu命令の実装" tabindex="-1">MULHU命令の実装 <a class="header-anchor" href="#mulhu命令の実装" aria-label="Permalink to “MULHU命令の実装”">​</a></h2><p>MULHU命令は、2つの符号無しのXLENビットの値の乗算を実行し、 デスティネーションレジスタに結果(XLEN * 2ビット)の上位XLENビットを書き込む命令です。 funct3の下位2ビットによってmulunitモジュールの結果を選択するようにします (リスト11)。</p><p><span class="caption">▼リスト9.11: MULHUモジュールの結果を取得する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6c6678f9536fa9a3460952fa457a1e193d087e97~1..6c6678f9536fa9a3460952fa457a1e193d087e97#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::WaitValid: <span class="custom-hl-bold"><span class="hljs-keyword">if</span> is_mul &amp;&amp; mu_rvalid {</span>
    state  = State::Finish;
...
</code></pre></div><p>abs関数を利用して、MUL、MULH命令のときにmulunitに渡す値を絶対値に設定します (リスト13、リスト14)。</p><p><span class="caption">▼リスト9.13: op1とop2を生成する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> mu_op1: <span class="hljs-keyword">logic</span>&lt;MUL_OP_WIDTH&gt; = <span class="hljs-keyword">case</span> funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
</code></pre></div><p><span class="caption">▼リスト9.14: mulunitに渡す値を変更する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> mu: mulunit #(
    WIDTH: MUL_OP_WIDTH,
) (
    clk                             ,
    rst                             ,
...
</code></pre></div><p>計算結果の符号は<code>op1</code>と<code>op2</code>の符号が異なる場合に負になります。 後で符号の情報を利用するために、muldivunitモジュールが要求を受け入れる時に符号を保存します ( リスト15、 リスト16、 リスト17 )。</p><p><span class="caption">▼リスト9.15: 符号を保存する変数を作成する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// saved_data</span>
<span class="hljs-keyword">var</span> funct3_saved : <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">3</span>&gt;;
...
</code></pre></div><p><span class="caption">▼リスト9.16: 変数のリセット (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        state         = State::Idle;
        result        = <span class="hljs-number">0</span>;
        funct3_saved  = <span class="hljs-number">0</span>;
...
</code></pre></div><p><span class="caption">▼リスト9.17: 符号を変数に保存する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">case</span> state {
    State::Idle: <span class="hljs-keyword">if</span> ready &amp;&amp; valid {
        state         = State::WaitValid;
        funct3_saved  = funct3;
...
</code></pre></div><p>保存した符号を利用して計算結果の符号を復元します (リスト18)。</p><p><span class="caption">▼リスト9.18: 計算結果の符号を復元する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/573f4d922ccbee831afaa30e3d8861dfb2f61c5e~1..573f4d922ccbee831afaa30e3d8861dfb2f61c5e#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::WaitValid: <span class="hljs-keyword">if</span> is_mul &amp;&amp; mu_rvalid {
...
</code></pre></div><p>計算結果は<code>op1</code>の符号にします (リスト20)。</p><p><span class="caption">▼リスト9.20: 計算結果の符号を復元する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/9a0e3f92371ecae7b350446e7b5ffb0fee1b2887~1..9a0e3f92371ecae7b350446e7b5ffb0fee1b2887#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::WaitValid: <span class="hljs-keyword">if</span> is_mul &amp;&amp; mu_rvalid {
...
</code></pre></div><p>riscv-testsの<code>rv64um-p-mulhsu</code>を実行し、成功することを確認してください。</p><h3 id="mulw命令の実装" tabindex="-1">MULW命令の実装 <a class="header-anchor" href="#mulw命令の実装" aria-label="Permalink to “MULW命令の実装”">​</a></h3><p>MULW命令は、2つの符号付きの32ビットの値の乗算を実行し、 デスティネーションレジスタに結果の下位32ビットを符号拡張した値を書き込む命令です。</p><p>32ビット演算の命令であることを判定するために、 muldivunitモジュールに<code>is_op32</code>ポートを作成します ( リスト21、 リスト22 )。</p><p><span class="caption">▼リスト9.21: is_op32ポートを追加する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> muldivunit (
    clk    : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>   ,
    rst    : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">reset</span>   ,
    ready  : <span class="hljs-keyword">output</span> <span class="hljs-keyword">logic</span>   ,
    valid  : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">logic</span>   ,
...
</code></pre></div><p><span class="caption">▼リスト9.22: is_op32ポートに値を割り当てる (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> mdu: muldivunit (
    clk                       ,
    rst                       ,
    valid  : exs_muldiv_valid ,
    ready  : exs_muldiv_ready ,
...
</code></pre></div><p>muldivunitモジュールが要求を受け入れる時に<code>is_op32</code>を保存します。 また、符号ビットを<code>is_op32</code>に応じて設定します ( リスト23、 リスト24、 リスト25 )。</p><p><span class="caption">▼リスト9.23: is_op32を保存する変数を作成する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// saved_data</span>
<span class="hljs-keyword">var</span> funct3_saved : <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">3</span>&gt;;
...
</code></pre></div><p><span class="caption">▼リスト9.24: 変数のリセット (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        state         = State::Idle;
        result        = <span class="hljs-number">0</span>;
        funct3_saved  = <span class="hljs-number">0</span>;
...
</code></pre></div><p><span class="caption">▼リスト9.25: is_op32を変数に保存する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::Idle: <span class="hljs-keyword">if</span> ready &amp;&amp; valid {
    state         = State::WaitValid;
    funct3_saved  = funct3;
...
</code></pre></div><p><span class="caption">▼リスト9.27: MULW命令用にop1、op2を設定する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> mu_op1: <span class="hljs-keyword">logic</span>&lt;MUL_OP_WIDTH&gt; = <span class="hljs-keyword">case</span> funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
</code></pre></div><p>最後に、計算結果を符号拡張した値に設定します (リスト28)。</p><p><span class="caption">▼リスト9.28: 計算結果を符号拡張する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0691652ab090f1f48a2fa7b56544883d8666e07f~1..0691652ab090f1f48a2fa7b56544883d8666e07f#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::WaitValid: <span class="hljs-keyword">if</span> is_mul &amp;&amp; mu_rvalid {
    <span class="hljs-keyword">let</span> res_signed: <span class="hljs-keyword">logic</span>&lt;MUL_RES_WIDTH&gt; = <span class="hljs-keyword">if</span> op1sign_saved != op2sign_saved ? ~mu_result + <span class="hljs-number">1</span> : mu_result;
...
</code></pre></div><p><span class="caption">▼リスト9.33: divunitに渡す値を変更する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0af5063d84907c122eb21c936bcb6f93b0d00349~1..0af5063d84907c122eb21c936bcb6f93b0d00349#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> du: divunit #(
    WIDTH: DIV_WIDTH,
) (
    clk                                                     ,
    rst                                                     ,
...
</code></pre></div><p><span class="caption">▼リスト9.35: 符号付き除算の例外的な結果を処理する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0af5063d84907c122eb21c936bcb6f93b0d00349~1..0af5063d84907c122eb21c936bcb6f93b0d00349#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>State::Idle: <span class="hljs-keyword">if</span> ready &amp;&amp; valid {
    funct3_saved  = funct3;
...
</code></pre></div><p>計算が終了したら、商と剰余の符号を復元します。 商の符号は除数と被除数の符号が異なる場合に負になります。 剰余の符号は被除数の符号にします (リスト36)。</p><p><span class="caption">▼リスト9.36: 計算結果の符号を復元する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0af5063d84907c122eb21c936bcb6f93b0d00349~1..0af5063d84907c122eb21c936bcb6f93b0d00349#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>} <span class="hljs-keyword">else</span> <span class="hljs-keyword">if</span> !is_mul &amp;&amp; du_rvalid {
...
</code></pre></div><p>符号付き除算のオーバーフローとゼロ除算の判定を<code>is_op32</code>で変更します (リスト38)。</p><p><span class="caption">▼リスト9.38: 32ビット演算のときの例外的な処理に対応する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/961992f59ad0a1a93d4c7970c7af9dace3f3f9f0~1..961992f59ad0a1a93d4c7970c7af9dace3f3f9f0#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>ゼロ除算のときの結果を<code>is_op32</code>に応じて変更します (リスト39)。</p><p><span class="caption">▼リスト9.39: 32ビット演算のときの例外的な処理に対応する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/961992f59ad0a1a93d4c7970c7af9dace3f3f9f0~1..961992f59ad0a1a93d4c7970c7af9dace3f3f9f0#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p>`,30),s("div",{class:"language-veryl"},[s("button",{title:"Copy Code",class:"copy"}),s("span",{class:"lang"},"veryl"),s("pre",{class:"hljs"},[s("code",null,[a("State::Idle: "),s("span",{class:"hljs-keyword"},"if"),a(` ready && valid {
    funct3_saved  = funct3;
...
`)])])],-1),n(`<p>最後に、32ビットの結果をXLENビットに符号拡張します (リスト40)。 符号付き、符号無し演算のどちらも32ビットの結果を符号拡張したものが結果になります。</p><p><span class="caption">▼リスト9.40: 32ビット演算のとき、結果を符号拡張する (muldivunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/961992f59ad0a1a93d4c7970c7af9dace3f3f9f0~1..961992f59ad0a1a93d4c7970c7af9dace3f3f9f0#diff-a0b1c500872f0d9547a64ef2feee3ca7e167d9aa98f02cb06578c0f51eea47b0">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>} <span class="hljs-keyword">else</span> <span class="hljs-keyword">if</span> !is_mul &amp;&amp; du_rvalid {
