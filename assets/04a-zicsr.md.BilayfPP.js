import{_ as a,c as e,o as n,ah as c,as as p,at as l,au as t}from"./chunks/framework.HhScKIQu.js";const u=JSON.parse('{"title":"Zicsr拡張の実装","description":"","frontmatter":{},"headers":[],"relativePath":"04a-zicsr.md","filePath":"04a-zicsr.md"}'),d={name:"04a-zicsr.md"};function r(o,s,f,i,h,b){return n(),e("div",null,[...s[0]||(s[0]=[c(`<h1 id="zicsr拡張の実装" tabindex="-1">Zicsr拡張の実装 <a class="header-anchor" href="#zicsr拡張の実装" aria-label="Permalink to “Zicsr拡張の実装”">​</a></h1><h2 id="csrとは何か" tabindex="-1">CSRとは何か? <a class="header-anchor" href="#csrとは何か" aria-label="Permalink to “CSRとは何か?”">​</a></h2><p>前の章では、RISC-Vの基本整数命令セットであるRV32Iを実装しました。 既に簡単なプログラムを動かせますが、 例外や割り込み、ページングなどの機能がありません<sup class="footnote-ref"><a href="#fn1" id="fnref1">[1]</a></sup>。 このような機能はCSRを介して提供されます。</p><p>RISC-Vには、CSR(Control and Status Register)というレジスタが4096個存在しています。 例えば<code>mtvec</code>というレジスタは、例外や割り込みが発生したときのジャンプ先のアドレスを格納しています。 RISC-VのCPUは、CSRの読み書きによって制御(Control)や状態(Status)の読み取りを行います。</p><p>CSRの読み書きを行う命令は、Zicsr拡張によって定義されています(表1)。 本章では、Zicsrに定義されている命令、 RV32Iに定義されているECALL命令、 MRET命令、 mtvecレジスタ、 mepcレジスタ、 mcauseレジスタを実装します。</p><div id="zicsr.insts" class="table"><p class="caption">表4.1: Zicsr拡張に定義されている命令</p><table><tr class="hline"><th>命令</th><th>作用</th></tr><tr class="hline"><td>CSRRW</td><td>CSRにrs1を書き込み、元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRWI</td><td>CSRRWのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr><tr class="hline"><td>CSRRS</td><td>CSRとrs1をビットORした値をCSRに書き込み、元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRSI</td><td>CSRRSのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr><tr class="hline"><td>CSRRC</td><td>CSRと~rs1(rs1のビットNOT)をビットANDした値をCSRに書き込み、<br>元のCSRの値をrdに書き込む</td></tr><tr class="hline"><td>CSRRCI</td><td>CSRRCのrs1を、即値をゼロ拡張した値に置き換えた動作</td></tr></table></div><h2 id="csr命令のデコード" tabindex="-1">CSR命令のデコード <a class="header-anchor" href="#csr命令のデコード" aria-label="Permalink to “CSR命令のデコード”">​</a></h2><p>まず、Zicsrに定義されている命令(表1)をデコードします。</p><p>これらの命令のopcodeは<code>SYSTEM</code>(<code>7&#39;b1110011</code>)です。 この値をeeiパッケージに定義します(リスト1)。</p><p><span class="caption">▼リスト4.1: opcode用の定数の定義 (eei.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4~1..ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">const</span> OP_SYSTEM: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">7</span>&gt; = <span class="hljs-number">7&#39;b1110011</span>;
</code></pre></div><p>次に、<code>InstCtrl</code>構造体に、 CSRを制御する命令であることを示す<code>is_csr</code>フラグを追加します(リスト2)。</p><p><span class="caption">▼リスト4.2: is_csrを追加する (corectrl.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4~1..ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 制御に使うフラグ用の構造体</span>
...
</code></pre></div><p>これでデコード処理を書く準備が整いました。 inst_decoderモジュールの<code>InstCtrl</code>を生成している部分を変更します(リスト3)。</p><p><span class="caption">▼リスト4.3: OP_SYSTEMとis_csrを追加する (inst_decoder.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4~1..ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>ctrl = {
...
</code></pre></div><p>CSR命令の結果の受け取りのために変数<code>csru_rdata</code>を作成し、 csrunitモジュールをインスタンス化しています。</p><p><code>csr_addr</code>ポートには命令の上位12ビットを設定しています。 <code>rs1</code>ポートには、 即値を利用する命令(CSRR(W|S|C)I)の場合はrs1_addrを<code>0</code>で拡張した値を、 それ以外の命令の場合はrs1のデータを設定しています。</p><p>次に、CSRを読み込んだデータをレジスタにライトバックします。 具体的には、 <code>InstCtrl.is_csr</code>が<code>1</code>のとき、 <code>wb_data</code>が<code>csru_rdata</code>になるようにします (リスト6)。</p><p><span class="caption">▼リスト4.6: CSR命令の結果がライトバックされるようにする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4~1..ae9d20b4a140c4e1ced90cdf1f327f1e1fea96f4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];
...
</code></pre></div><p>mtvecの初期値は<code>0</code>です。 mtvecに<code>wdata</code>を書き込むとき、 MODEが常に<code>2&#39;b00</code>になります。</p><h3 id="csrunitモジュールをテストする" tabindex="-1">csrunitモジュールをテストする <a class="header-anchor" href="#csrunitモジュールをテストする" aria-label="Permalink to “csrunitモジュールをテストする”">​</a></h3><p>mtvecレジスタの書き込み、 読み込みができることを確認します。</p><p><code>test/sample_csr.hex</code>を作成し、 次のように記述します(リスト13)。</p><p><span class="caption">▼リスト4.13: sample_csr.hex</span> <a href="https://github.com/nananapo/bluecore/compare/1f557c3f77fab08ba75f3c4f24148df2aa3ea20d~1..1f557c3f77fab08ba75f3c4f24148df2aa3ea20d#diff-47d9e90d4876f4a6f796e2e1837db907c2838c0df7f630a81f65fc98cbed5c76">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>305bd0f3 // 0: csrrwi x1, mtvec, 0b10111
...
</code></pre></div><p>次に、トラップの発生原因を表現する型<code>CsrCause</code>を定義します。 今のところ、発生原因はECALL命令によるEnvironment Call From M-mode例外しかありません (リスト16)。</p><p><span class="caption">▼リスト4.16: CsrCause型の定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">enum</span> CsrCause: UIntX {
    ENVIRONMENT_CALL_FROM_M_MODE = <span class="hljs-number">11</span>,
}
</code></pre></div><p>最後に、mepcとmcauseの書き込みマスクを定義します (リスト17)。 mepcに格納されるのは例外が発生した時の命令のアドレスです。 命令は4バイトに整列して配置されているため、 mepcの下位2ビットは常に<code>2&#39;b00</code>になるようにします。</p><p><span class="caption">▼リスト4.17: mepcとmcauseの書き込みマスクの定義 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">const</span> MTVEC_WMASK : UIntX = <span class="hljs-number">&#39;hffff_fffc</span>;
...
</code></pre></div><p>次に、mepcとmcauseの読み込み処理と、書き込みマスクの割り当てを実装します。 どちらもcase文にアドレスと値のペアを追加するだけです ( リスト19、 リスト20 )。</p><p><span class="caption">▼リスト4.19: mepcとmcauseの読み込み (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>rdata = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : mtvec,
    <span class="custom-hl-bold">CsrAddr::MEPC  : mepc,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: mcause,</span>
    <span class="hljs-keyword">default</span>        : &#39;x,
};
</code></pre></div><p><span class="caption">▼リスト4.20: mepcとmcauseの書き込みマスクの設定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>wmask = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : MTVEC_WMASK,
    <span class="custom-hl-bold">CsrAddr::MEPC  : MEPC_WMASK,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: MCAUSE_WMASK,</span>
    <span class="hljs-keyword">default</span>        : <span class="hljs-number">0</span>,
};
</code></pre></div><p>最後に、mepcとmcauseの書き込みを実装します。 if_resetで値を<code>0</code>に初期化し、case文にmepcとmcauseの場合を実装します (リスト27)。</p><h4 id="例外の実装" tabindex="-1">例外の実装 <a class="header-anchor" href="#例外の実装" aria-label="Permalink to “例外の実装”">​</a></h4><p>ECALL命令と、それによって発生するトラップを実装します。 まず、csrunitモジュールにポートを追加します (リスト21)。</p><p><span class="caption">▼リスト4.21: csrunitモジュールにポートを追加する (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> csrunit (
    clk        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>       ,
    rst        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">reset</span>       ,
    valid      : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">logic</span>       ,
...
</code></pre></div><p><img src="`+l+`" alt="ECALL命令のエンコーディング"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図2: ECALL命令のエンコーディング</p><p>それでは、csrunitモジュールにトラップの処理を実装します。</p><p>ECALL命令は、 I形式、 即値は<code>0</code>、 rs1とrdは<code>0</code>、 funct3は<code>0</code>、 opcodeは<code>SYSTEM</code>な命令です(図2)。 これを判定するための変数を作成します(リスト25)。</p><p><span class="caption">▼リスト4.25: ECALL命令かどうかの判定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// ECALL命令かどうか</span>
...
</code></pre></div><p>次に、例外が発生するかどうかを示す<code>raise_expt</code>と、 例外の発生の原因を示す<code>expt_cause</code>を作成します。 今のところ、例外はECALL命令によってのみ発生するため、 <code>expt_cause</code>は実質的に定数になっています (リスト26)。</p><p><span class="caption">▼リスト4.26: 例外とトラップの判定 (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// Exception</span>
...
</code></pre></div><p>トラップが発生するかどうかを示す<code>raise_trap</code>には、 例外が発生するかどうかを割り当てます。 トラップの原因を示す<code>trap_cause</code>には、 例外の発生原因を割り当てます。 また、トラップ先には<code>mtvec</code>を割り当てます。</p><p>最後に、トラップに伴うCSRの変更を実装します。 トラップが発生するとき、 mepcレジスタにPC、 mcauseレジスタにトラップの発生原因を格納します (リスト27)。</p><p><span class="caption">▼リスト4.27: CSR、トラップ発生時の処理を実装する (csrunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-44dd9efb5b2797bbd5248f96206e5f13442629b6dd3cda87990e383f99c2aeec">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        mtvec  = <span class="hljs-number">0</span>;
...
</code></pre></div><p><code>test/sample_ecall.hex</code>を作成し、 次のように記述します (リスト29)。</p><p><span class="caption">▼リスト4.29: sample_ecall.hex</span> <a href="https://github.com/nananapo/bluecore/compare/7e5713fe59303d531e335c748579dd35f7fe4e5f~1..7e5713fe59303d531e335c748579dd35f7fe4e5f#diff-818919fa8011230661860f6c1f57b536be1be4559c0ce404f6fd655e4616735b">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>30585073 //  0: csrrwi x0, mtvec, 0x10
00000073 //  4: ecall
00000000 //  8:
00000000 //  c:
