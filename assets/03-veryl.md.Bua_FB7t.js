...
</code></pre></div><p><img src="`+i+`" alt="の型の要素は連続した領域に並ぶ (例 : v[1][0]とv[0][3]が隣り合う)"></p><p class="caption">▲図5: &lt;&gt;の型の要素は連続した領域に並ぶ (例 : v[1][0]とv[0][3]が隣り合う)</p> \`[]\`を使用することでも、多次元の型を定義できます (リスト28)。 ただし、\`[]\`を使用して構成される型の要素は、 連続した領域に並ぶことが保証されません。 <p><span class="caption">▼リスト2.28: 配列型</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 型名[個数] で、&quot;型名&quot;型が&quot;個数&quot;個の配列になる</span>
...
</code></pre></div><h3 id="式、文、宣言" tabindex="-1">式、文、宣言 <a class="header-anchor" href="#式、文、宣言" aria-label="Permalink to “式、文、宣言”">​</a></h3><h4 id="ビット選択" tabindex="-1">ビット選択 <a class="header-anchor" href="#ビット選択" aria-label="Permalink to “ビット選択”">​</a></h4><p><img src="`+m+'" alt="ビット選択"></p><p class="caption">▲図6: ビット選択</p> 変数の任意のビットを切り出すには`[]`を使用します(図6)。 範囲の選択には`[:]`を使用します。 最上位ビット(most significant bit, MSB)は**msb**キーワード、 最下位ビット(least significant bit, LSB)は**lsb**キーワードで指定できます。 選択する場所の指定には式を使えます。 <p>よく使われる範囲の選択には、別の書き方が用意されています (リスト30)。</p><p><span class="caption">▼リスト2.30: 範囲の選択の別の記法</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>v[s +: w]   <span class="hljs-comment">// = v[s+w-1   : s    ]</span>\nv[s -: w]   <span class="hljs-comment">// = v[s       : s-w+1]</span>\nv[i <span class="hljs-keyword">step</span> w] <span class="hljs-comment">// = v[i*(w+1) : i*w  ] = v[i*w +: w]</span>\n</code></pre></div><h4 id="演算子" tabindex="-1">演算子 <a class="header-anchor" href="#演算子" aria-label="Permalink to “演算子”">​</a></h4><p>Verylでは、表5の演算子を使用できます。 ほとんどの演算子と優先度は通常のプログラミング言語と同じですが、 ビット演算の種類が多かったり、 <code>x</code>と<code>z</code>を考慮した演算があるなどの違いがあります。</p>',171)),n("p",null,[s[0]||(s[0]=a("SystemVerilogとの差異を説明すると、 <code>++</code>、<code>--</code>、<code>:=</code>、<code>:/</code>、<code>&lt;=</code>(代入)、 <code>&lt;</code>と<code>&gt;</code>がそれぞれ<code>&lt;:</code>と<code>&gt;:</code>に変更されています。 また、<code>inside</code>と",21)),n("code",null,o(),1),s[1]||(s[1]=l("、",-1)),s[2]||(s[2]=n("code",null,"?:",-1)),s[3]||(s[3]=l("(三項演算子)の形式が変更され、case式、switch式が追加されています。",-1))]),s[5]||(s[5]=a(`<p>単項、二項演算子の使用例は次の通りです(リスト31)。</p><p><span class="caption">▼リスト2.31: 単項、二項演算子 (Verylのドキュメントの例を改変)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 単項算術演算</span>
