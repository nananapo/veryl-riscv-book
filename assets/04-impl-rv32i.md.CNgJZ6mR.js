...
</code></pre></div><p>すると、プロジェクト名のディレクトリと、その中に<code>Veryl.toml</code>、srcディレクトリが作成されます。 Verylのソースファイルはsrcディレクトリに作成します。</p><p><code>Veryl.toml</code>にはプロジェクトの設定を記述します。 デフォルトの状態だとソースマップファイルが生成されますが、使用しない場合は<code>Veryl.toml</code>を次のように変更してください(リスト2)。</p><p><span class="caption">▼リスト3.2: Veryl.toml</span> <a href="https://github.com/nananapo/bluecore/compare/fc3f93a6b6032781f67e8d0f56cedbe850ab276d~1..fc3f93a6b6032781f67e8d0f56cedbe850ab276d#diff-64bb648d464e211c90aeeb225a89aa505b27a33b890617e6303c9f92b1f43cf9">差分をみる</a></p><div class="language-toml"><button title="Copy Code" class="copy"></button><span class="lang">toml</span><pre class="hljs"><code><span class="hljs-section">[project]</span>
...
</code></pre></div><h2 id="定数の定義" tabindex="-1">定数の定義 <a class="header-anchor" href="#定数の定義" aria-label="Permalink to “定数の定義”">​</a></h2><p>いよいよコードを記述します。 まず、CPU内で何度も使用する定数や型を書いておくためのパッケージを作成します。</p><p><code>src/eei.veryl</code>を作成し、次のように記述します(リスト3)。</p><p><span class="caption">▼リスト3.3: eei.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/48b0669d389eb6d31ae6a866d7122ca77f646072~1..48b0669d389eb6d31ae6a866d7122ca77f646072#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">package</span> eei {
...
</code></pre></div><p>membus_ifはジェネリックインターフェースです。 ジェネリックパラメータとして、 <code>ADDR_WIDTH</code>と<code>DATA_WIDTH</code>が定義されています。 <code>ADDR_WIDTH</code>はアドレスの幅、 <code>DATA_WIDTH</code>は1つのデータの幅です。</p><p>interfaceを利用することで変数の定義が不要になり、 ポートの相互接続を簡潔にできます。</p><h3 id="メモリモジュールを実装する" tabindex="-1">メモリモジュールを実装する <a class="header-anchor" href="#メモリモジュールを実装する" aria-label="Permalink to “メモリモジュールを実装する”">​</a></h3><p>メモリを作る準備が整いました。 <code>src/memory.veryl</code>を作成し、次のように記述します(リスト5)。</p><p><span class="caption">▼リスト3.5: メモリモジュールの定義 (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/4bb29fdad0ff19d1285afad6f09ac2589481020f~1..4bb29fdad0ff19d1285afad6f09ac2589481020f#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
...
</code></pre></div><p>それでは、最上位のモジュールを作成します。 <code>src/top.veryl</code>を作成し、次のように記述します(リスト8)。</p><p><span class="caption">▼リスト3.8: 最上位モジュールの定義 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/4bb29fdad0ff19d1285afad6f09ac2589481020f~1..4bb29fdad0ff19d1285afad6f09ac2589481020f#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>addr_to_memaddr関数は、 <code>MEM_DATA_WIDTH</code>(=32)をバイトに変換した値(=4)のlog2をとった値(=2)を使って、 <code>addr[17:2]</code>を切り取っています。 範囲の選択には<code>+:</code>を利用しています。</p><p>次に、coreモジュール用のmembus_ifインターフェースを作成します(リスト11)。 ジェネリックパラメータには、 coreモジュールのインターフェースのジェネリックパラメータと同じく、 ILENとXLENを割り当てます。</p><p><span class="caption">▼リスト3.11: coreモジュール用のmembus_ifインターフェースをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d0814be9b264784dbf7764c5e2c7dda545803f0e~1..d0814be9b264784dbf7764c5e2c7dda545803f0e#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus     : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>最後にcoreモジュールをインスタンス化します (リスト13)。 メモリとCPUが接続されました。</p><p><span class="caption">▼リスト3.13: coreモジュールをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d0814be9b264784dbf7764c5e2c7dda545803f0e~1..d0814be9b264784dbf7764c5e2c7dda545803f0e#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk                ,
    rst                ,
    membus: membus_core,
);
...
</code></pre></div><p>fifoモジュールをインスタンス化したので、 メモリへデータを要求する処理を変更します(リスト25)。</p><p><span class="caption">▼リスト3.25: フェッチ処理の変更 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/38bece061a7168d5045393706c4aaee749940f42~1..38bece061a7168d5045393706c4aaee749940f42#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令フェッチ処理</span>
<span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>リスト25では、 メモリに命令フェッチを要求する条件を FIFOに2つ以上空きがあるという条件に変更しています<sup class="footnote-ref"><a href="#fn4" id="fnref4">[4]</a></sup>。 これにより、FIFOがあふれてしまうことがなくなります。 また、FIFOから常にデータを取り出すようにしています。</p><p>命令をフェッチできたらFIFOに格納する処理をalways_ffブロックの中に追加します(リスト26)。</p><p><span class="caption">▼リスト3.26: FIFOへのデータの格納 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/38bece061a7168d5045393706c4aaee749940f42~1..38bece061a7168d5045393706c4aaee749940f42#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// IFのFIFOの制御</span>
...
</code></pre></div><p><code>if_fifo_wvalid</code>と<code>if_fifo_wdata</code>を<code>0</code>に初期化します(リスト27)。</p><p><span class="caption">▼リスト3.27: 変数の初期化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/38bece061a7168d5045393706c4aaee749940f42~1..38bece061a7168d5045393706c4aaee749940f42#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    if_pc           = <span class="hljs-number">0</span>;
    if_is_requested = <span class="hljs-number">0</span>;
    if_pc_requested = <span class="hljs-number">0</span>;
...
</code></pre></div><p>これらの値とそれぞれの命令の対応は、仕様書を確認してください。</p><h3 id="制御フラグと即値を生成する" tabindex="-1">制御フラグと即値を生成する <a class="header-anchor" href="#制御フラグと即値を生成する" aria-label="Permalink to “制御フラグと即値を生成する”">​</a></h3><p>デコード処理を書く準備が整いました。 <code>src/inst_decoder.veryl</code>を作成し、 次のように記述します(リスト32)。</p><p><span class="caption">▼リスト3.32: inst_decoder.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/b6eabb828d04abaf167330bf5958531285a8bf48~1..b6eabb828d04abaf167330bf5958531285a8bf48#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>まず、デコーダとcoreモジュールを接続するために<code>inst_ctrl</code>と<code>inst_imm</code>を定義します。 次に、inst_decoderモジュールをインスタンス化します。 <code>bits</code>ポートに<code>inst_bits</code>を渡すことでフェッチした命令をデコードします。</p><p>デバッグ用のalways_ffブロックに、 デコードした結果をデバッグ表示するコードを記述します(リスト34)。</p><p><span class="caption">▼リスト3.34: デコード結果のデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/b6eabb828d04abaf167330bf5958531285a8bf48~1..b6eabb828d04abaf167330bf5958531285a8bf48#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>ifを使うことで、 0番目のレジスタが指定されたときは、 値が常に<code>0</code>になるようにします。</p><p>レジスタの値を読み込めていることを確認するために、 デバッグ表示にソースレジスタの値を追加します(リスト38)。 <code>$display</code>システムタスクで、 命令のレジスタ番号と値をデバッグ表示します。</p><p><span class="caption">▼リスト3.38: レジスタの値をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7f217fc3ad7cb8a11d96ba6b9354bdf699e4a453~1..7f217fc3ad7cb8a11d96ba6b9354bdf699e4a453#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>早速動作のテストをしたいところですが、 今のままだとレジスタの値が初期化されておらず、 0番目のレジスタの値以外は不定値<sup class="footnote-ref"><a href="#fn5" id="fnref5">[5]</a></sup>になってしまいます。</p><p>これではテストする意味がないため、 レジスタの値を適当な値に初期化します。 always_ffブロックのif_resetで、 <code>i</code>番目(0 &lt; <code>i</code> &lt; 32)のレジスタの値を<code>i</code>で初期化します(リスト39)。</p><p><span class="caption">▼リスト3.39: レジスタを適当な値で初期化する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/7f217fc3ad7cb8a11d96ba6b9354bdf699e4a453~1..7f217fc3ad7cb8a11d96ba6b9354bdf699e4a453#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタの初期化</span>
<span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>次に、<code>src/alu.veryl</code>を作成し、次のように記述します(リスト42)。</p><p><span class="caption">▼リスト3.42: alu.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/49895e4cd107c3b6aff6200985da498c6a077e8f~1..49895e4cd107c3b6aff6200985da498c6a077e8f#diff-b2410559b76182b1bc742cc3c956eb08e8bc1b4179316a0cebacd77cf94a4d7a">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
<span class="hljs-keyword">import</span> corectrl::*;
...
</code></pre></div><p><code>src/sample.hex</code>を、次のように書き換えます(リスト46)。</p><p><span class="caption">▼リスト3.46: sample.hexを書き換える</span> <a href="https://github.com/nananapo/bluecore/compare/49895e4cd107c3b6aff6200985da498c6a077e8f~1..49895e4cd107c3b6aff6200985da498c6a077e8f#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02000093 // addi x1, x0, 32
...
00000008 : 002081b3
  ...
  op2     : 00000002
  alu res : 00000003
</code></pre></div><p>まだ、結果をディスティネーションレジスタに格納する処理を作成していません。 そのため、命令を実行してもレジスタの値は変わらないことに注意してください</p><dl><dt>addi x1, x0, 32</dt><dd> \`op1\`は0番目のレジスタの値です。 0番目のレジスタの値は常に\`0\`であるため、\`32&#39;h00000000\`と表示されています。 \`op2\`は即値です。 即値は32であるため、\`32&#39;h00000020\`と表示されています。 ALUの計算結果として、0と32を足した結果\`32&#39;h00000020\`が表示されています。 </dd><dt>auipc x2, 256</dt><dd> \`op1\`はPCです。 \`op1\`には、命令のアドレス\`0x00000004\`が表示されています。 \`op2\`は即値です。 256を12bit左にシフトした値\`32&#39;h00100000\`が表示されています。 ALUの計算結果として、これを足した結果\`32&#39;h00100004\`が表示されています。 </dd><dt>add x3, x1, x2</dt><dd> \`op1\`、\`op2\`は1、2番目のレジスタの値です。 ALUの計算結果として、それぞれの初期値\`1\`と\`2\`を足した結果\`32&#39;h00000003\`が表示されています。 </dd></dl><h2 id="レジスタに結果を書き込む" tabindex="-1">レジスタに結果を書き込む <a class="header-anchor" href="#レジスタに結果を書き込む" aria-label="Permalink to “レジスタに結果を書き込む”">​</a></h2><p>CPUはレジスタから値を読み込み、計算して、 レジスタに結果の値を書き戻します。 レジスタに値を書き戻すことを、 値を<strong>ライトバック</strong>(write-back)すると呼びます。</p><p>ライトバックする値は、計算やメモリアクセスの結果です。 まだメモリにアクセスする処理を実装していませんが、 先にライトバック処理を実装します。</p><h3 id="ライトバック処理を実装する" tabindex="-1">ライトバック処理を実装する <a class="header-anchor" href="#ライトバック処理を実装する" aria-label="Permalink to “ライトバック処理を実装する”">​</a></h3><p>書き込む対象のレジスタ(デスティネーションレジスタ)は、 命令のrdフィールドによって番号で指定されます。 デコード時に、 レジスタに結果を書き込む命令かどうかを<code>InstCtrl.rwb_en</code>に格納しています(リスト32)。</p><p>LUI命令のときは即値をそのまま、 それ以外の命令のときはALUの結果をライトバックします(リスト48)。</p><p><span class="caption">▼リスト3.48: ライトバック処理の実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/844c80aac0ca76520b4bfabc28b6695c3a2745df~1..844c80aac0ca76520b4bfabc28b6695c3a2745df#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];</span>
...
</code></pre></div><h3 id="ライトバック処理をテストする" tabindex="-1">ライトバック処理をテストする <a class="header-anchor" href="#ライトバック処理をテストする" aria-label="Permalink to “ライトバック処理をテストする”">​</a></h3><p>デバッグ表示用のalways_ffブロックで、 ライトバック処理の概要をデバッグ表示します(リスト49)。 処理している命令がライトバックする命令のときにのみ、 <code>$display</code>システムタスクを呼び出します。</p><p><span class="caption">▼リスト3.49: ライトバックのデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/844c80aac0ca76520b4bfabc28b6695c3a2745df~1..844c80aac0ca76520b4bfabc28b6695c3a2745df#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.rwb_en {
...
</code></pre></div><p>次に、<code>inst_is_new</code>の値を更新します(リスト54)。 命令が現在のクロックで供給されたかどうかは、 FIFOの<code>rvalid</code>と<code>rready</code>を観測することでわかります。 <code>rvalid</code>が<code>1</code>のとき、 <code>rready</code>が<code>1</code>なら、 次のクロックで供給される命令は新しく供給される命令です。 <code>rready</code>が<code>0</code>なら、 次のクロックで供給されている命令は現在のクロックと同じ命令になります。 <code>rvalid</code>が<code>0</code>のとき、 次のクロックで供給される命令は常に新しく供給される命令になります (次のクロックで<code>rvalid</code>が<code>1</code>かどうかは考えません)。</p><p><span class="caption">▼リスト3.54: inst_is_newの実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2502d11a4a99f440dff09f63477e6a3dd021f5f3~1..2502d11a4a99f440dff09f63477e6a3dd021f5f3#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>命令フェッチ用のポートが<code>membus</code>から<code>i_membus</code>に変更されるため、 既存の<code>membus</code>を<code>i_membus</code>に置き換えてください(リスト56)。</p><p><span class="caption">▼リスト3.56: membusをi_membusに置き換える (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2502d11a4a99f440dff09f63477e6a3dd021f5f3~1..2502d11a4a99f440dff09f63477e6a3dd021f5f3#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// FIFOに2個以上空きがあるとき、命令をフェッチする</span>
...
</code></pre></div><p>ここからのテストは実行するクロック数が多くなります。 そこで、ログに何クロック目かを表示することでログを読みやすくします(リスト65)。</p><p><span class="caption">▼リスト3.65: 何クロック目かを出力する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2502d11a4a99f440dff09f63477e6a3dd021f5f3~1..2502d11a4a99f440dff09f63477e6a3dd021f5f3#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">var</span> clock_count: <span class="hljs-keyword">u64</span>;</span>
...
<span class="hljs-keyword">always_ff</span> {
    ...
    <span class="custom-hl-bold">} <span class="hljs-keyword">else</span> {</span>
        ...
        <span class="hljs-keyword">if</span> inst_valid {
            ...
            $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
            $display(<span class="hljs-string">&quot;  itype     : %b&quot;</span>, inst_ctrl.itype);
</code></pre></div><p>LW、SW命令のテストのために、 <code>src/sample.hex</code>を次のように変更します(リスト66)。</p><p><span class="caption">▼リスト3.66: テスト用のプログラムを記述する (sample.hex)</span> <a href="https://github.com/nananapo/bluecore/compare/2502d11a4a99f440dff09f63477e6a3dd021f5f3~1..2502d11a4a99f440dff09f63477e6a3dd021f5f3#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02002503 // lw x10, 0x20(x0)
...
</code></pre></div><p>funct3をcase文で分岐し、 アドレスの下位ビットを見ることで、 命令とアドレスに応じた値をrdataに設定します(リスト69)。</p><p><span class="caption">▼リスト3.69: rdataをアドレスと読み込みサイズに応じて変更する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e2ae462965570605a8aad8b9030104300a2d609e~1..e2ae462965570605a8aad8b9030104300a2d609e#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// loadの結果</span>
...
</code></pre></div><p><code>wmask</code>、wmask_expand関数をmodportに追加します ( リスト72 )。</p><p><span class="caption">▼リスト3.72: modport masterとslaveにwmask、wmask_expand関数を追加する (membus_if.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e2ae462965570605a8aad8b9030104300a2d609e~1..e2ae462965570605a8aad8b9030104300a2d609e#diff-5b8ff8b1b43eb415d367759fb46872980d16065c43c0a71a75fec85e987ad460">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">modport</span> master {
    valid       : <span class="hljs-keyword">output</span>,
    ready       : <span class="hljs-keyword">input</span> ,
    addr        : <span class="hljs-keyword">output</span>,
    wen         : <span class="hljs-keyword">output</span>,
...
</code></pre></div><p><code>wmask</code>には、書き込む部分を<code>1</code>、書き込まない部分を<code>0</code>で指定します。 このような挙動をする値を、書き込みマスクと呼びます。 バイト単位で指定するため、<code>wmask</code>の幅は<code>DATA_WIDTH / 8</code>ビットです。</p><p>次に、memoryモジュールで書き込みマスクをサポートします(リスト73)。</p><p><span class="caption">▼リスト3.73: 書き込みマスクをサポートするmemoryモジュール (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e2ae462965570605a8aad8b9030104300a2d609e~1..e2ae462965570605a8aad8b9030104300a2d609e#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
    <span class="hljs-keyword">param</span> FILEPATH_IS_ENV: <span class="hljs-keyword">logic</span>  = <span class="hljs-number">0</span> , <span class="hljs-comment">// FILEPATHが環境変数名かどうか</span>
    <span class="hljs-keyword">param</span> FILEPATH       : <span class="hljs-keyword">string</span> = <span class="hljs-string">&quot;&quot;</span>, <span class="hljs-comment">// メモリの初期化用ファイルのパス, または環境変数名</span>
) (
    clk   : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">clock</span>                                     ,
...
</code></pre></div><p><span class="caption">▼リスト3.78: メモリにアクセスする命令のとき、wmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/e2ae462965570605a8aad8b9030104300a2d609e~1..e2ae462965570605a8aad8b9030104300a2d609e#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>req_wmask = <span class="hljs-keyword">case</span> ctrl.funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
`)])])],-1),e('<p>ここで、新しく命令をフェッチしなおすようにしても、 ジャンプ命令によって実行されることがなくなった命令がFIFOに残っていることがあることに注意する必要があります(図4)。</p><p><img src="'+r+`" alt="ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図4: ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている</p><p>実行するべきではない命令を実行しないようにするために、 ジャンプ命令を実行するときに、FIFOをリセットします。</p><p>FIFOに、中身をリセットするための信号<code>flush</code>を実装します(リスト86)。</p><p><span class="caption">▼リスト3.86: ポートにflushを追加する (fifo.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614~1..cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> fifo #(
    ...
    wready    : <span class="hljs-keyword">output</span> <span class="hljs-keyword">logic</span>        ,
</code></pre></div><p><code>flush</code>が<code>1</code>のとき、 <code>head</code>と<code>tail</code>を<code>0</code>に初期化することでFIFOを空にします(リスト87、リスト88)。</p><p><span class="caption">▼リスト3.87: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH==1)</span> <a href="https://github.com/nananapo/bluecore/compare/cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614~1..cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        rdata  = <span class="hljs-number">0</span>;
        rvalid = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p><span class="caption">▼リスト3.88: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH!=1)</span> <a href="https://github.com/nananapo/bluecore/compare/cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614~1..cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        head = <span class="hljs-number">0</span>;
        tail = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p>coreモジュールで、 <code>control_hazard</code>と<code>flush</code>を接続し、 FIFOをリセットします(リスト89)。</p><p><span class="caption">▼リスト3.89: ジャンプ命令のとき、FIFOをリセットする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614~1..cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> if_fifo: fifo #(
    DATA_TYPE: if_fifo_type,
    WIDTH    : <span class="hljs-number">3</span>           ,
) (
    clk                           ,
...
</code></pre></div><h4 id="無条件ジャンプのテスト" tabindex="-1">無条件ジャンプのテスト <a class="header-anchor" href="#無条件ジャンプのテスト" aria-label="Permalink to “無条件ジャンプのテスト”">​</a></h4><p>簡単なテストを作成し、動作をテストします(リスト90、リスト91)。</p><p><span class="caption">▼リスト3.90: sample_jump.hex</span> <a href="https://github.com/nananapo/bluecore/compare/cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614~1..cbbb1fcfb5e6eeae1f7a3cfc51b0134936231614#diff-4b870828046f4065482146d5dd02dccb5d0523eabe5af669e953963404e810ac">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>0100006f //  0: jal x0, 0x10 : 0x10にジャンプする
deadbeef //  4:
deadbeef //  8:
deadbeef //  c:
...
</code></pre></div><p>命令が条件分岐命令で<code>brunit_take</code>が<code>1</code>のとき、 次のPCをPC + 即値にします ( リスト94、 リスト95 )。</p><p><span class="caption">▼リスト3.94: 命令が条件分岐命令か判定する関数 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d661903e7f788461d590969552f9f45f601d3888~1..d661903e7f788461d590969552f9f45f601d3888#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令が分岐命令かどうかを判定する</span>
...
</code></pre></div><p><span class="caption">▼リスト3.95: 分岐成立時のPCの設定 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d661903e7f788461d590969552f9f45f601d3888~1..d661903e7f788461d590969552f9f45f601d3888#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = inst_valid &amp;&amp; <span class="custom-hl-bold">(</span>inst_ctrl.is_jump <span class="custom-hl-bold">|| inst_is_br(inst_ctrl) &amp;&amp; brunit_take)</span>;
...
</code></pre></div><p>簡単なテストを作成し、動作をテストします(リスト97, リスト98)。</p><p><span class="caption">▼リスト3.97: sample_br.hex</span> <a href="https://github.com/nananapo/bluecore/compare/d661903e7f788461d590969552f9f45f601d3888~1..d661903e7f788461d590969552f9f45f601d3888#diff-ed70829e581bd3b76a3741500395b0a2d55950437f287a0db0b81ae8b589e4e0">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>00100093 //  0: addi x1, x0, 1
