...
</code></pre></div><p>ProcessタブのSynthesizeをクリックし、合成します(図21)。</p><p><img src="`+D+'" alt="Processタブ"></p><p class="caption">▲図21: Processタブ</p> そうすると、合成に失敗して図22のようなエラーが表示されます。 <p><img src="'+C+'" alt="合成するとエラーが発生する"></p><p class="caption">▲図22: 合成するとエラーが発生する</p> これは、Tang Nano 9Kに搭載されているメモリ用の部品の数が足りないために発生しているエラーです。 この問題を回避するために、eeiパッケージの`MEM_ADDR_WIDTH`の値を`10`に変更します[^23]。 メモリの幅を変更したら、Verylファイルをビルドしなおして、もう一度合成します。 <p><img src="'+v+'" alt="合成と配置配線に成功した"></p><p class="caption">▲図23: 合成と配置配線に成功した</p> 合成に成功したら、`Place &amp; Route`を押して、論理回路の配置配線情報を生成します(図23)。 それが終了したら、Tang Nano 9KをPCに接続して、Gowin Programmerを開いて設計をFPGAに書き込みます(図24)。 <p><img src="'+w+'" alt="Program / Configureボタンを押して書き込む"></p><p class="caption">▲図24: Program / Configureボタンを押して書き込む</p> Tang Nano 9Kの中央2つ以外のLEDが点灯していることを確認できます。 図5の左下のボタンを押すと全てのLEDが点灯します。 <p><img src="'+A+`" alt="LEDの制御用レジスタの値が12()なので中央2つのLEDが点灯せず、それ以外が点灯する"></p><p class="caption">▲図25: LEDの制御用レジスタの値が12(\`64&#39;b1100\`)なので中央2つのLEDが点灯せず、それ以外が点灯する</p><h4 id="ledの点滅を確認する" tabindex="-1">LEDの点滅を確認する <a class="header-anchor" href="#ledの点滅を確認する" aria-label="Permalink to “LEDの点滅を確認する”">​</a></h4><p><code>MEMORY_FILEPATH</code>パラメータの値を<code>test/led_counter.hex</code>のパスに設定します(リスト21)。</p><p><span class="caption">▼リスト8.21: 読み込むファイルを変更する (top_tang.sv)</span></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>core_top #(
    .MEMORY_FILEPATH_IS_ENV (0 ),
    .MEMORY_FILEPATH        (&quot;test/led_counter.hexへのパス&quot;)
) t (
</code></pre></div><p>合成、配置配線しなおして、設計をFPGAに書き込むとLEDが点滅します<sup class="footnote-ref"><a href="#fn3" id="fnref3">[3]</a></sup>。 図5の左下のボタンを押すと状態がリセットされます。</p><h2 id="fpgaへの合成2-pynq-z1" tabindex="-1">FPGAへの合成② (PYNQ-Z1) <a class="header-anchor" href="#fpgaへの合成2-pynq-z1" aria-label="Permalink to “FPGAへの合成② (PYNQ-Z1)”">​</a></h2><p>PYNQ-Z1をターゲットにCPUを合成します。 使用するEDAのバージョンは次の通りです。</p><ul><li>Vivado v2023.2</li></ul><p>初めてPYNQ-Z1を使う人は、<a href="https://www.pynq.io/boards.html" target="_blank" rel="noreferrer">PYNQのドキュメント</a>やACRiの記事を参考に起動方法を確認して、Vivadoにボードファイルを追加してください。</p><h3 id="合成用のモジュールを作成する-1" tabindex="-1">合成用のモジュールを作成する <a class="header-anchor" href="#合成用のモジュールを作成する-1" aria-label="Permalink to “合成用のモジュールを作成する”">​</a></h3><p><img src="`+x+`" alt="PYNQ-Z1のLED(6個)"></p><p class="caption">▲図26: PYNQ-Z1のLED(6個)</p> PYNQ-Z1にはLEDが6個実装されています(図26)。 本章ではボタンの上の横並びの4つのLED(図26右下)を使用します。 <p><a href="./05b-synth.html">「8.3.1 合成用のモジュールを作成する」</a>とおなじように、 <code>led</code>ポートのビット幅を一致させるためにPYNQ-Z1の合成のためのトップモジュールを作成します。</p><p><code>src/top_pynq_z1.veryl</code>を作成し、次のように記述します(リスト22)。 top_pynq_z1モジュールの<code>led</code>ポートは4ビットとして定義して、topモジュールの<code>led</code>ポートの下位4ビットを接続しています。</p><p><span class="caption">▼リスト8.22: PYNQ-Z1用の最上位モジュール (top_pynq_z1.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/70d12437720f90babf2a2228c8fe6b9eae6de17f~1..70d12437720f90babf2a2228c8fe6b9eae6de17f#diff-257ab0e195148813de1fa11a8132d7651c48ef788d6c85108bcd5ca3c44dce04">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
