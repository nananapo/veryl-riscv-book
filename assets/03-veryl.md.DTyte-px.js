import{_ as p,c as e,o as c,ah as a,j as n,a as l,t,ai as o,aj as r,ak as d,al as h,am as i,an as m}from"./chunks/framework.HhScKIQu.js";const f=JSON.parse('{"title":"ハードウェア記述言語 Veryl","description":"","frontmatter":{},"headers":[],"relativePath":"03-veryl.md","filePath":"03-veryl.md"}'),j={name:"03-veryl.md"};function y(g,s,u,b,k,w){return c(),e("div",null,[s[4]||(s[4]=a('<h1 id="ハードウェア記述言語-veryl" tabindex="-1">ハードウェア記述言語 Veryl <a class="header-anchor" href="#ハードウェア記述言語-veryl" aria-label="Permalink to “ハードウェア記述言語 Veryl”">​</a></h1><p>CPU (Central Proccessing Unit, 中央演算処理装置)は、 コンピュータを構成する主要な部品の1つであり、 電気で動くとても複雑な回路で構成されています。</p><p>本書では「ハードウェア記述言語」によってCPUをの回路を記述します。 回路を記述するといっても、いったい何をどうやって記述するのでしょうか?</p><p>まずは、論理回路を構成する方法から考えます。</p><h2 id="ハードウェア記述言語" tabindex="-1">ハードウェア記述言語 <a class="header-anchor" href="#ハードウェア記述言語" aria-label="Permalink to “ハードウェア記述言語”">​</a></h2><h3 id="論理回路の構成" tabindex="-1">論理回路の構成 <a class="header-anchor" href="#論理回路の構成" aria-label="Permalink to “論理回路の構成”">​</a></h3><p><strong>論理回路</strong>とは、 デジタル(例えば0と1だけ)なデータを利用して、データを加工、保持する回路のことです。 論理回路は、組み合わせ回路と順序回路に分類できます。</p><p><strong>組み合わせ回路</strong>とは、 入力に対して、一意に出力の決まる回路のことです。 例えば、1ビット同士の加算をする回路は図1、表1のように表されます。 この回路は半加算器と呼ばれていて、 1ビットのXとYを入力として受けとり、1ビットの和Sと桁上げCを出力します。 入力(X、Y)が決まると出力(C、S)が一意に決まるため、半加算器は組み合わせ回路です。</p><p><img src="'+o+'" alt="半加算器 (MIL記法の回路図)"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図1: 半加算器 (MIL記法の回路図)</p><div id="halfadder.truth" class="table"><p class="caption">表2.1: 半加算器 (真理値表)</p><table><tr class="hline"><th>X</th><th>Y</th><th>C</th><th>S</th></tr><tr class="hline"><td>0</td><td>0</td><td>0</td><td>0</td></tr><tr class="hline"><td>0</td><td>1</td><td>0</td><td>1</td></tr><tr class="hline"><td>1</td><td>0</td><td>0</td><td>1</td></tr><tr class="hline"><td>1</td><td>1</td><td>1</td><td>0</td></tr></table></div> **順序回路**とは、 入力と回路自身の状態によって一意に出力の決まる回路です。 例えば、入力が1になるたびにカウントアップして値を表示するカウンタを考えます(図2)。 カウントアップするためには、 今のカウンタの値(状態)を保持する必要があります。 そのため、このカウンタは入力と状態によって一意に出力の決まる順序回路です。 <p>1ビットの値はフリップフロップ(flip-flop, FF)という回路によって保持できます。 フリップフロップをN個並列に並べると、Nビットの値を保持できます。 フリップフロップを並列に並べた記憶装置のことを<strong>レジスタ</strong>(register, 置数器)と呼びます。 基本的に、レジスタの値は<strong>リセット信号</strong>(reset signal, reset)によって初期化し、 <strong>クロック信号</strong>(clock signal, clock)に同期したタイミングで変更します。</p><p><img src="'+r+`" alt="カウンタ (順序回路の例)"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図2: カウンタ (順序回路の例)</p><p>論理回路を設計するには、真理値表を作成し、 それを実現する論理演算を構成します。 入力数や状態数が数十個ならどうにか人力で設計できるかもしれませんが、 数千、数万の入力や状態があるとき、 手作業で設計するのはほとんど不可能です。 これを設計するために、ハードウェア記述言語を利用します。</p><h3 id="ハードウェア記述言語-1" tabindex="-1">ハードウェア記述言語 <a class="header-anchor" href="#ハードウェア記述言語-1" aria-label="Permalink to “ハードウェア記述言語”">​</a></h3><p><strong>ハードウェア記述言語</strong>(Hardware Description Language, HDL)とは、 デジタル回路を設計するための言語です。 例えばHDLであるSystemVerilogを利用すると、 半加算器はリスト1のように記述できます。</p><p><span class="caption">▼リスト2.1: SystemVerilogによる半加算器の記述</span></p><div class="language-sv"><button title="Copy Code" class="copy"></button><span class="lang">sv</span><pre class="hljs"><code><span class="hljs-keyword">module</span> HalfAdder(
...
</code></pre></div><p>本書ではレジスタのことを変数、 または変数のことをレジスタと呼ぶことがあります。</p><p>レジスタの値はクロック信号に同期したタイミングで変更し、 リセット信号に同期したタイミングで初期化します(図4)。 本書では、 クロック信号が<strong>立ち上がる</strong>(<code>0</code>から<code>1</code>に変わる)タイミングでレジスタの値を変更し、 リセット信号が<strong>立ち下がる</strong>(<code>1</code>から<code>0</code>に変わる)タイミングでレジスタの値を初期化することとします。</p><p><img src="`+h+`" alt="レジスタ(value)の値はクロック信号(clk)が立ち上がるタイミングで変わる"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図4: レジスタ(value)の値はクロック信号(clk)が立ち上がるタイミングで変わる</p><p>レジスタの値は、<strong>always_ff</strong>ブロックで初期化、変更します(リスト14)。 always_ffブロックには、値の変更タイミングのためのクロック信号とリセット信号を指定します。</p><p><span class="caption">▼リスト2.14: レジスタの値の初期化と変更</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタの定義</span>
<span class="hljs-keyword">var</span> value : <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">32</span>&gt;;
...
</code></pre></div><h4 id="演算子" tabindex="-1">演算子 <a class="header-anchor" href="#演算子" aria-label="Permalink to “演算子”">​</a></h4><p>Verylでは、表5の演算子を使用できます。 ほとんどの演算子と優先度は通常のプログラミング言語と同じですが、 ビット演算の種類が多かったり、 <code>x</code>と<code>z</code>を考慮した演算があるなどの違いがあります。</p>`,171)),n("p",null,[s[0]||(s[0]=a("SystemVerilogとの差異を説明すると、 <code>++</code>、<code>--</code>、<code>:=</code>、<code>:/</code>、<code>&lt;=</code>(代入)、 <code>&lt;</code>と<code>&gt;</code>がそれぞれ<code>&lt;:</code>と<code>&gt;:</code>に変更されています。 また、<code>inside</code>と",21)),n("code",null,t(),1),s[1]||(s[1]=l("、",-1)),s[2]||(s[2]=n("code",null,"?:",-1)),s[3]||(s[3]=l("(三項演算子)の形式が変更され、case式、switch式が追加されています。",-1))]),s[5]||(s[5]=a(`<p>単項、二項演算子の使用例は次の通りです(リスト31)。</p><p><span class="caption">▼リスト2.31: 単項、二項演算子 (Verylのドキュメントの例<sup class="footnote-ref"><a href="#fn2" id="fnref2">[2]</a></sup>を改変)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 単項算術演算</span>
...
<span class="hljs-meta">\`<span class="hljs-keyword">endif</span></span>
</code></pre></div><p><code>#[sv()]</code>は、宣言にSystemVerilogの属性を付けられます。 属性は使用するときに説明します。 <code>#[ifdef(マクロ名)]</code>をつけられた宣言は、 マクロが存在するときにのみ定義されるようになります。 <code>#[ifndef(マクロ名)]</code>はその逆で、 マクロが存在しないときにのみ定義されるようになります。</p><p>アトリビュートはポートやパラメータ、ブロック、モジュール、インターフェース、パッケージなど、どの宣言にも付けることができます。</p><h4 id="標準ライブラリ" tabindex="-1">標準ライブラリ <a class="header-anchor" href="#標準ライブラリ" aria-label="Permalink to “標準ライブラリ”">​</a></h4><p>Verylには、よく使うモジュールなどが標準ライブラリとして準備されています。 標準ライブラリは <a href="https://std.veryl-lang.org/" target="_blank" rel="noreferrer">https://std.veryl-lang.org/</a> で確認できます。</p><p>本書では標準ライブラリを使用していないため、説明は割愛します。</p><hr class="footnotes-sep"><section class="footnotes"><ol class="footnotes-list"><li id="fn1" class="footnote-item"><p>例えば、未定義の変数が1ビット幅の信号線として解釈される仕様があります <a href="#fnref1" class="footnote-backref">↩︎</a></p></li><li id="fn2" class="footnote-item"><p>演算子 - The Veryl Hardware Description Language, <a href="https://doc.veryl-lang.org/book/ja/05_language_reference/02_lexical_structure/01_operator.html" target="_blank" rel="noreferrer">https://doc.veryl-lang.org/book/ja/05_language_reference/02_lexical_structure/01_operator.html</a> <a href="#fnref2" class="footnote-backref">↩︎</a></p></li></ol></section>`,98))])}const _=p(j,[["render",y]]);export{f as __pageData,_ as default};
