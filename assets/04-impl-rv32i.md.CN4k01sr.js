...
</code></pre></div><p>すると、プロジェクト名のディレクトリと、その中に<code>Veryl.toml</code>、srcディレクトリが作成されます。 Verylのソースファイルはsrcディレクトリに作成します。</p><p><code>Veryl.toml</code>にはプロジェクトの設定を記述します。 デフォルトの状態だとソースマップファイルが生成されますが、使用しない場合は<code>Veryl.toml</code>を次のように変更してください(リスト2)。</p><p><span class="caption">▼リスト3.2: Veryl.toml</span> <a href="https://github.com/nananapo/bluecore/compare/21cec4f25fa5294ab95faaee39332f4f2fcf2c99~1..21cec4f25fa5294ab95faaee39332f4f2fcf2c99#diff-64bb648d464e211c90aeeb225a89aa505b27a33b890617e6303c9f92b1f43cf9">差分をみる</a></p><div class="language-toml"><button title="Copy Code" class="copy"></button><span class="lang">toml</span><pre class="hljs"><code><span class="hljs-section">[project]</span>
...
</code></pre></div><h2 id="定数の定義" tabindex="-1">定数の定義 <a class="header-anchor" href="#定数の定義" aria-label="Permalink to “定数の定義”">​</a></h2><p>いよいよコードを記述します。 まず、CPU内で何度も使用する定数や型を書いておくためのパッケージを作成します。</p><p><code>src/eei.veryl</code>を作成し、次のように記述します(リスト3)。</p><p><span class="caption">▼リスト3.3: eei.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/7e8f2ebc6dcb0d2ce30395d38f8bca3fd5eaab3c~1..7e8f2ebc6dcb0d2ce30395d38f8bca3fd5eaab3c#diff-11470b1e7d69c89ea3723d114583fe520f6d8482422a151ad706be8ab57ea6b7">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">package</span> eei {
...
</code></pre></div><p>membus_ifはジェネリックインターフェースです。 ジェネリックパラメータとして、 <code>ADDR_WIDTH</code>と<code>DATA_WIDTH</code>が定義されています。 <code>ADDR_WIDTH</code>はアドレスの幅、 <code>DATA_WIDTH</code>は1つのデータの幅です。</p><p>interfaceを利用することで変数の定義が不要になり、 ポートの相互接続を簡潔にできます。</p><h3 id="メモリモジュールを実装する" tabindex="-1">メモリモジュールを実装する <a class="header-anchor" href="#メモリモジュールを実装する" aria-label="Permalink to “メモリモジュールを実装する”">​</a></h3><p>メモリを作る準備が整いました。 <code>src/memory.veryl</code>を作成し、次のように記述します(リスト5)。</p><p><span class="caption">▼リスト3.5: メモリモジュールの定義 (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/408bdefd62bcc7fb94cfdbc9465e04788eb8cc78~1..408bdefd62bcc7fb94cfdbc9465e04788eb8cc78#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
...
</code></pre></div><p>それでは、最上位のモジュールを作成します。 <code>src/top.veryl</code>を作成し、次のように記述します(リスト8)。</p><p><span class="caption">▼リスト3.8: 最上位モジュールの定義 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/408bdefd62bcc7fb94cfdbc9465e04788eb8cc78~1..408bdefd62bcc7fb94cfdbc9465e04788eb8cc78#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>addr_to_memaddr関数は、 <code>MEM_DATA_WIDTH</code>(=32)をバイトに変換した値(=4)のlog2をとった値(=2)を使って、 <code>addr[17:2]</code>を切り取っています。 範囲の選択には<code>+:</code>を利用しています。</p><p>次に、coreモジュール用のmembus_ifインターフェースを作成します(リスト11)。 ジェネリックパラメータには、 coreモジュールのインターフェースのジェネリックパラメータと同じく、 ILENとXLENを割り当てます。</p><p><span class="caption">▼リスト3.11: coreモジュール用のmembus_ifインターフェースをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d85b7498f9ff270c50e0a399ce3722cf2ee458f9~1..d85b7498f9ff270c50e0a399ce3722cf2ee458f9#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus     : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>最後にcoreモジュールをインスタンス化します (リスト13)。 メモリとCPUが接続されました。</p><p><span class="caption">▼リスト3.13: coreモジュールをインスタンス化する (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/d85b7498f9ff270c50e0a399ce3722cf2ee458f9~1..d85b7498f9ff270c50e0a399ce3722cf2ee458f9#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk                ,
    rst                ,
    membus: membus_core,
);
...
</code></pre></div><p>fifoモジュールをインスタンス化したので、 メモリへデータを要求する処理を変更します(リスト25)。</p><p><span class="caption">▼リスト3.25: フェッチ処理の変更 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0f41d823fa71bbea417bafdec696c04346db8c00~1..0f41d823fa71bbea417bafdec696c04346db8c00#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令フェッチ処理</span>
<span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>リスト25では、 メモリに命令フェッチを要求する条件を FIFOに2つ以上空きがあるという条件に変更しています<sup class="footnote-ref"><a href="#fn4" id="fnref4">[4]</a></sup>。 これにより、FIFOがあふれてしまうことがなくなります。 また、FIFOから常にデータを取り出すようにしています。</p><p>命令をフェッチできたらFIFOに格納する処理をalways_ffブロックの中に追加します(リスト26)。</p><p><span class="caption">▼リスト3.26: FIFOへのデータの格納 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0f41d823fa71bbea417bafdec696c04346db8c00~1..0f41d823fa71bbea417bafdec696c04346db8c00#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// IFのFIFOの制御</span>
...
</code></pre></div><p><code>if_fifo_wvalid</code>と<code>if_fifo_wdata</code>を<code>0</code>に初期化します(リスト27)。</p><p><span class="caption">▼リスト3.27: 変数の初期化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/0f41d823fa71bbea417bafdec696c04346db8c00~1..0f41d823fa71bbea417bafdec696c04346db8c00#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    if_pc           = <span class="hljs-number">0</span>;
    if_is_requested = <span class="hljs-number">0</span>;
    if_pc_requested = <span class="hljs-number">0</span>;
...
</code></pre></div><p>これらの値とそれぞれの命令の対応は、仕様書を確認してください。</p><h3 id="制御フラグと即値を生成する" tabindex="-1">制御フラグと即値を生成する <a class="header-anchor" href="#制御フラグと即値を生成する" aria-label="Permalink to “制御フラグと即値を生成する”">​</a></h3><p>デコード処理を書く準備が整いました。 <code>src/inst_decoder.veryl</code>を作成し、 次のように記述します(リスト32)。</p><p><span class="caption">▼リスト3.32: inst_decoder.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/29a67adf0b8304ee8271640b093b779505caefe7~1..29a67adf0b8304ee8271640b093b779505caefe7#diff-0520643107a80d7dcd5de7ffc77443b16b7089aa9e14564e8201c524e6d0eec8">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
...
</code></pre></div><p>まず、デコーダとcoreモジュールを接続するために<code>inst_ctrl</code>と<code>inst_imm</code>を定義します。 次に、inst_decoderモジュールをインスタンス化します。 <code>bits</code>ポートに<code>inst_bits</code>を渡すことでフェッチした命令をデコードします。</p><p>デバッグ用のalways_ffブロックに、 デコードした結果をデバッグ表示するコードを記述します(リスト34)。</p><p><span class="caption">▼リスト3.34: デコード結果のデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/29a67adf0b8304ee8271640b093b779505caefe7~1..29a67adf0b8304ee8271640b093b779505caefe7#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>ifを使うことで、 0番目のレジスタが指定されたときは、 値が常に<code>0</code>になるようにします。</p><p>レジスタの値を読み込めていることを確認するために、 デバッグ表示にソースレジスタの値を追加します(リスト38)。 <code>$display</code>システムタスクで、 命令のレジスタ番号と値をデバッグ表示します。</p><p><span class="caption">▼リスト3.38: レジスタの値をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/a9a7259d1da03c6f1dece602cddb5f2fa3326ebb~1..a9a7259d1da03c6f1dece602cddb5f2fa3326ebb#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> if_fifo_rvalid {
        $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
...
</code></pre></div><p>次に、<code>src/alu.veryl</code>を作成し、次のように記述します(リスト42)。</p><p><span class="caption">▼リスト3.42: alu.veryl</span> <a href="https://github.com/nananapo/bluecore/compare/d92e59fec2b88ba0e94f73c3a02e465f2de55b38~1..d92e59fec2b88ba0e94f73c3a02e465f2de55b38#diff-b2410559b76182b1bc742cc3c956eb08e8bc1b4179316a0cebacd77cf94a4d7a">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">import</span> eei::*;
<span class="hljs-keyword">import</span> corectrl::*;
...
00000008 : 002081b3
  ...
  op2     : 00000002
  alu res : 00000003
</code></pre></div><p>まだ、結果をディスティネーションレジスタに格納する処理を作成していません。 そのため、命令を実行してもレジスタの値は変わらないことに注意してください</p><dl><dt>addi x1, x0, 32</dt><dd> \`op1\`は0番目のレジスタの値です。 0番目のレジスタの値は常に\`0\`であるため、\`32&#39;h00000000\`と表示されています。 \`op2\`は即値です。 即値は32であるため、\`32&#39;h00000020\`と表示されています。 ALUの計算結果として、0と32を足した結果\`32&#39;h00000020\`が表示されています。 </dd><dt>auipc x2, 256</dt><dd> \`op1\`はPCです。 \`op1\`には、命令のアドレス\`0x00000004\`が表示されています。 \`op2\`は即値です。 256を12bit左にシフトした値\`32&#39;h00100000\`が表示されています。 ALUの計算結果として、これを足した結果\`32&#39;h00100004\`が表示されています。 </dd><dt>add x3, x1, x2</dt><dd> \`op1\`、\`op2\`は1、2番目のレジスタの値です。 ALUの計算結果として、それぞれの初期値\`1\`と\`2\`を足した結果\`32&#39;h00000003\`が表示されています。 </dd></dl><h2 id="レジスタに結果を書き込む" tabindex="-1">レジスタに結果を書き込む <a class="header-anchor" href="#レジスタに結果を書き込む" aria-label="Permalink to “レジスタに結果を書き込む”">​</a></h2><p>CPUはレジスタから値を読み込み、計算して、 レジスタに結果の値を書き戻します。 レジスタに値を書き戻すことを、 値を<strong>ライトバック</strong>(write-back)すると呼びます。</p><p>ライトバックする値は、計算やメモリアクセスの結果です。 まだメモリにアクセスする処理を実装していませんが、 先にライトバック処理を実装します。</p><h3 id="ライトバック処理を実装する" tabindex="-1">ライトバック処理を実装する <a class="header-anchor" href="#ライトバック処理を実装する" aria-label="Permalink to “ライトバック処理を実装する”">​</a></h3><p>書き込む対象のレジスタ(デスティネーションレジスタ)は、 命令のrdフィールドによって番号で指定されます。 デコード時に、 レジスタに結果を書き込む命令かどうかを<code>InstCtrl.rwb_en</code>に格納しています(リスト32)。</p><p>LUI命令のときは即値をそのまま、 それ以外の命令のときはALUの結果をライトバックします(リスト48)。</p><p><span class="caption">▼リスト3.48: ライトバック処理の実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/c2ab4438bb7d6e582ccd2f7318c58c05caea3921~1..c2ab4438bb7d6e582ccd2f7318c58c05caea3921#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">let</span> rd_addr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">5</span>&gt; = inst_bits[<span class="hljs-number">11</span>:<span class="hljs-number">7</span>];</span>
...
</code></pre></div><h3 id="ライトバック処理をテストする" tabindex="-1">ライトバック処理をテストする <a class="header-anchor" href="#ライトバック処理をテストする" aria-label="Permalink to “ライトバック処理をテストする”">​</a></h3><p>デバッグ表示用のalways_ffブロックで、 ライトバック処理の概要をデバッグ表示します(リスト49)。 処理している命令がライトバックする命令のときにのみ、 <code>$display</code>システムタスクを呼び出します。</p><p><span class="caption">▼リスト3.49: ライトバックのデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/c2ab4438bb7d6e582ccd2f7318c58c05caea3921~1..c2ab4438bb7d6e582ccd2f7318c58c05caea3921#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> inst_ctrl.rwb_en {
...
</code></pre></div><p>次に、<code>inst_is_new</code>の値を更新します(リスト54)。 命令が現在のクロックで供給されたかどうかは、 FIFOの<code>rvalid</code>と<code>rready</code>を観測することでわかります。 <code>rvalid</code>が<code>1</code>のとき、 <code>rready</code>が<code>1</code>なら、 次のクロックで供給される命令は新しく供給される命令です。 <code>rready</code>が<code>0</code>なら、 次のクロックで供給されている命令は現在のクロックと同じ命令になります。 <code>rvalid</code>が<code>0</code>のとき、 次のクロックで供給される命令は常に新しく供給される命令になります (次のクロックで<code>rvalid</code>が<code>1</code>かどうかは考えません)。</p><p><span class="caption">▼リスト3.54: inst_is_newの実装 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>命令フェッチ用のポートが<code>membus</code>から<code>i_membus</code>に変更されるため、 既存の<code>membus</code>を<code>i_membus</code>に置き換えてください(リスト56)。</p><p><span class="caption">▼リスト3.56: membusをi_membusに置き換える (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// FIFOに2個以上空きがあるとき、命令をフェッチする</span>
...
</code></pre></div><p>次に、topモジュールでの調停を実装します(リスト57)。 新しく<code>i_membus</code>と<code>d_membus</code>をインスタンス化し、 それを<code>membus</code>と接続します。</p><p><span class="caption">▼リスト3.57: メモリへのアクセス要求の調停 (top.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-0c548fd82f89bdf97edffcd89dfccd2aab836eccf57f11b8e25c313abd0d0e6f">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> membus  : membus_if::&lt;MEM_DATA_WIDTH, MEM_ADDR_WIDTH&gt;;
...
</code></pre></div><p>ところで、現在のコードではmemunitの処理が終了していないときも値をライトバックし続けています。 レジスタへのライトバックは命令の実行が終了したときのみで良いため、 次のようにコードを変更します(リスト63)。</p><p><span class="caption">▼リスト3.63: 命令の実行が終了したときにのみライトバックする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if</span> inst_valid <span class="custom-hl-bold">&amp;&amp; if_fifo_rready</span> &amp;&amp; inst_ctrl.rwb_en {
        regfile[rd_addr] = wb_data;
    }
}
</code></pre></div><p>デバッグ表示も同様で、 ライトバックするときにのみデバッグ表示します(リスト64)。</p><p><span class="caption">▼リスト3.64: ライトバックするときにのみデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if</span> <span class="custom-hl-bold">if_fifo_rready &amp;&amp;</span> inst_ctrl.rwb_en {
    $display(<span class="hljs-string">&quot;  reg[%d] &lt;= %h&quot;</span>, rd_addr, wb_data);
}
</code></pre></div><h4 id="lw、sw命令のテスト" tabindex="-1">LW、SW命令のテスト <a class="header-anchor" href="#lw、sw命令のテスト" aria-label="Permalink to “LW、SW命令のテスト”">​</a></h4><p>LW命令とSW命令が正しく動作していることを確認するために、 デバッグ表示に次のコードを追加します(リスト65)。</p><p><span class="caption">▼リスト3.65: メモリモジュールの状態をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>$display(<span class="hljs-string">&quot;  mem stall : %b&quot;</span>, memu_stall);
$display(<span class="hljs-string">&quot;  mem rdata : %h&quot;</span>, memu_rdata);
</code></pre></div><p>ここからのテストは実行するクロック数が多くなります。 そこで、ログに何クロック目かを表示することでログを読みやすくします(リスト66)。</p><p><span class="caption">▼リスト3.66: 何クロック目かを出力する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-bold"><span class="hljs-keyword">var</span> clock_count: <span class="hljs-keyword">u64</span>;</span>
...
<span class="hljs-keyword">always_ff</span> {
    ...
    <span class="custom-hl-bold">} <span class="hljs-keyword">else</span> {</span>
        ...
        <span class="hljs-keyword">if</span> inst_valid {
            ...
            $display(<span class="hljs-string">&quot;%h : %h&quot;</span>, inst_pc, inst_bits);
            $display(<span class="hljs-string">&quot;  itype     : %b&quot;</span>, inst_ctrl.itype);
</code></pre></div><p>LW、SW命令のテストのために、 <code>src/sample.hex</code>を次のように変更します(リスト67)。</p><p><span class="caption">▼リスト3.67: テスト用のプログラムを記述する (sample.hex)</span> <a href="https://github.com/nananapo/bluecore/compare/6af292be6d0a314a0f88d42c2f5812e900d2a2b4~1..6af292be6d0a314a0f88d42c2f5812e900d2a2b4#diff-47e0566ba230be822ea6046444a3861b05b42992bceb7f989aead6e0e01c7451">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02002503 // lw x10, 0x20(x0)
...
</code></pre></div><p>funct3をcase文で分岐し、 アドレスの下位ビットを見ることで、 命令とアドレスに応じた値をrdataに設定します(リスト70)。</p><p><span class="caption">▼リスト3.70: rdataをアドレスと読み込みサイズに応じて変更する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// loadの結果</span>
...
</code></pre></div><p><code>wmask</code>、wmask_expand関数をmodportに追加します ( リスト73 )。</p><p><span class="caption">▼リスト3.73: modport masterとslaveにwmask、wmask_expand関数を追加する (membus_if.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-5b8ff8b1b43eb415d367759fb46872980d16065c43c0a71a75fec85e987ad460">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">modport</span> master {
    valid       : <span class="hljs-keyword">output</span>,
    ready       : <span class="hljs-keyword">input</span> ,
    addr        : <span class="hljs-keyword">output</span>,
    wen         : <span class="hljs-keyword">output</span>,
...
</code></pre></div><p><code>wmask</code>には、書き込む部分を<code>1</code>、書き込まない部分を<code>0</code>で指定します。 このような挙動をする値を、書き込みマスクと呼びます。 バイト単位で指定するため、<code>wmask</code>の幅は<code>DATA_WIDTH / 8</code>ビットです。</p><p>次に、memoryモジュールで書き込みマスクをサポートします(リスト74)。</p><p><span class="caption">▼リスト3.74: 書き込みマスクをサポートするmemoryモジュール (memory.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-f21911ca2ea9efd251c9040583fdaa56eff4e4f8451fc1aff3b3cca51277df90">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> memory::&lt;DATA_WIDTH: <span class="hljs-keyword">u32</span>, ADDR_WIDTH: <span class="hljs-keyword">u32</span>&gt; #(
    <span class="hljs-keyword">param</span> FILEPATH_IS_ENV: <span class="hljs-keyword">logic</span>  = <span class="hljs-number">0</span> , <span class="hljs-comment">// FILEPATHが環境変数名かどうか</span>
    <span class="hljs-keyword">param</span> FILEPATH       : <span class="hljs-keyword">string</span> = <span class="hljs-string">&quot;&quot;</span>, <span class="hljs-comment">// メモリの初期化用ファイルのパス, または環境変数名</span>
) (
    clk   : <span class="hljs-keyword">input</span>   <span class="hljs-keyword">clock</span>                                     ,
...
</code></pre></div><h4 id="memunitモジュールの実装" tabindex="-1">memunitモジュールの実装 <a class="header-anchor" href="#memunitモジュールの実装" aria-label="Permalink to “memunitモジュールの実装”">​</a></h4><p>memoryモジュールが書き込みマスクをサポートしたので、 memunitモジュールで<code>wmask</code>を設定します。</p><p><code>req_wmask</code>レジスタを作成し、 <code>membus.wmask</code>と接続します ( リスト76、 リスト77 )。</p><p><span class="caption">▼リスト3.76: req_wmaskの定義 (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> req_wmask: <span class="hljs-keyword">logic</span>&lt;MEM_DATA_WIDTH / <span class="hljs-number">8</span>&gt;;
</code></pre></div><p><span class="caption">▼リスト3.77: membusにwmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// メモリアクセス</span>
membus.valid = state == State::WaitReady;
membus.addr  = req_addr;
membus.wen   = req_wen;
membus.wdata = req_wdata;
<span class="custom-hl-bold">membus.wmask = req_wmask;</span>
</code></pre></div><p>always_ffの中で、<code>req_wmask</code>の値を設定します。 それぞれの命令のとき、<code>wmask</code>がどうなるかを確認してください( リスト78、 リスト79 )。</p><p><span class="caption">▼リスト3.78: if_resetでreq_wmaskを初期化する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
    state     = State::Init;
    req_wen   = <span class="hljs-number">0</span>;
    req_addr  = <span class="hljs-number">0</span>;
    req_wdata = <span class="hljs-number">0</span>;
...
</code></pre></div><p><span class="caption">▼リスト3.79: メモリにアクセスする命令のとき、wmaskを設定する (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>req_wmask = <span class="hljs-keyword">case</span> ctrl.funct3[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>] {
...
</code></pre></div><p>ソースレジスタの値はLSB側(右)に寄せられているため、アドレスを4で割った値が1, 2, 3のとき、アドレスに合わせてSB命令で書き込む値の左シフトが必要です ( リスト80 )。</p><p><span class="caption">▼リスト3.80: 書き込みデータをシフトする (memunit.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-d3618e86de098826661fabd111bec51452d943d3e486d18f9c3aa96b2dd9a89e">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>req_wdata = rs2 &lt;&lt; {addr[<span class="hljs-number">1</span>:<span class="hljs-number">0</span>], <span class="hljs-number">3&#39;b0</span>};
</code></pre></div><p>coreモジュールで、命令フェッチに使っているインターフェースの<code>wmask</code>を<code>&#39;x</code>に設定します ( リスト81 )。</p><p><span class="caption">▼リスト3.81: coreモジュールでi_membusのwmaskを&#39;xに設定する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令フェッチ処理</span>
<span class="hljs-keyword">always_comb</span> {
    <span class="hljs-comment">// FIFOに2個以上空きがあるとき、命令をフェッチする</span>
    i_membus.valid = if_fifo_wready_two;
    i_membus.addr  = if_pc;
    i_membus.wen   = <span class="hljs-number">0</span>;
    i_membus.wdata = &#39;x; <span class="hljs-comment">// wdataは使用しない</span>
    <span class="custom-hl-bold">i_membus.wmask = &#39;x; <span class="hljs-comment">// wmaskは使用しない</span></span>
</code></pre></div><h3 id="lb、lbu、lh、lhu、sb、sh命令をテストする" tabindex="-1">LB、LBU、LH、LHU、SB、SH命令をテストする <a class="header-anchor" href="#lb、lbu、lh、lhu、sb、sh命令をテストする" aria-label="Permalink to “LB、LBU、LH、LHU、SB、SH命令をテストする”">​</a></h3><p>簡単なテストを作成し、動作をテストします。 2つテストを記載するので、正しく動いているか確認してください。</p><p><span class="caption">▼リスト3.82: src/sample_lbh.hex</span> <a href="https://github.com/nananapo/bluecore/compare/63da5b773bd5815c1a10e5bc77a6fbaea5b9e545~1..63da5b773bd5815c1a10e5bc77a6fbaea5b9e545#diff-8ce901013258bdfae08e2cf52b70b3b6a3031d29401cf67466285ed2b6d689a7">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>02000083 // lb x1, 0x20(x0)  : x1 = ffffffef
...
</code></pre></div><p>次に、次にフェッチする命令をジャンプ先の命令に変更します。 フェッチ先の変更が発生を示す信号<code>control_hazard</code>と、 新しいフェッチ先を示す信号<code>control_hazard_pc_next</code>を作成します ( リスト85、 リスト86 )。</p><p><span class="caption">▼リスト3.85: control_hazardとcontrol_hazard_pc_nextの定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> control_hazard        : <span class="hljs-keyword">logic</span>;
<span class="hljs-keyword">var</span> control_hazard_pc_next: Addr ;
</code></pre></div><p><span class="caption">▼リスト3.86: control_hazardとcontrol_hazard_pc_nextの割り当て (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = inst_valid &amp;&amp; inst_ctrl.is_jump;
<span class="hljs-keyword">assign</span> control_hazard_pc_next = alu_result &amp; ~<span class="hljs-number">1</span>;
</code></pre></div><p><code>control_hazard</code>を利用して<code>if_pc</code>を更新し、 新しく命令をフェッチしなおすようにします(リスト87)。</p><p><span class="caption">▼リスト3.87: PCをジャンプ先に変更する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p>`,483),s("div",{class:"language-veryl"},[s("button",{title:"Copy Code",class:"copy"}),s("span",{class:"lang"},"veryl"),s("pre",{class:"hljs"},[s("code",null,[s("span",{class:"hljs-keyword"},"always_ff"),a(` {
...
`)])])],-1),e('<p>ここで、新しく命令をフェッチしなおすようにしても、 ジャンプ命令によって実行されることがなくなった命令がFIFOに残っていることがあることに注意する必要があります(図4)。</p><p><img src="'+r+`" alt="ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている"></p><p class="caption" style="text-align:center;font-weight:bold;">▲図4: ジャンプ命令とジャンプ先の間に余計な命令が入ってしまっている</p><p>実行するべきではない命令を実行しないようにするために、 ジャンプ命令を実行するときに、FIFOをリセットします。</p><p>FIFOに、中身をリセットするための信号<code>flush</code>を実装します(リスト88)。</p><p><span class="caption">▼リスト3.88: ポートにflushを追加する (fifo.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> fifo #(
    <span class="hljs-keyword">param</span> DATA_TYPE: <span class="hljs-keyword">type</span> = <span class="hljs-keyword">logic</span>,
    <span class="hljs-keyword">param</span> WIDTH    : <span class="hljs-keyword">u32</span>  = <span class="hljs-number">2</span>    ,
) (
    clk       : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>        ,
    ...
    wready    : <span class="hljs-keyword">output</span> <span class="hljs-keyword">logic</span>        ,
</code></pre></div><p><code>flush</code>が<code>1</code>のとき、 <code>head</code>と<code>tail</code>を<code>0</code>に初期化することでFIFOを空にします(リスト89、リスト90)。</p><p><span class="caption">▼リスト3.89: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH==1)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        rdata  = <span class="hljs-number">0</span>;
        rvalid = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p><span class="caption">▼リスト3.90: flushが1のとき、FIFOを空にする (fifo.veryl、WIDTH!=1)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-96f22bda69c26836ba4e1dbc83c35b9c6a63f889ea5f9f25cabc68d5110083a1">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        head = <span class="hljs-number">0</span>;
        tail = <span class="hljs-number">0</span>;
    } <span class="hljs-keyword">else</span> {
...
</code></pre></div><p>coreモジュールで、 <code>control_hazard</code>と<code>flush</code>を接続し、 FIFOをリセットします(リスト91)。</p><p><span class="caption">▼リスト3.91: ジャンプ命令のとき、FIFOをリセットする (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> if_fifo: fifo #(
    DATA_TYPE: if_fifo_type,
    WIDTH    : <span class="hljs-number">3</span>           ,
) (
    clk                           ,
...
</code></pre></div><h4 id="無条件ジャンプのテスト" tabindex="-1">無条件ジャンプのテスト <a class="header-anchor" href="#無条件ジャンプのテスト" aria-label="Permalink to “無条件ジャンプのテスト”">​</a></h4><p>簡単なテストを作成し、動作をテストします(リスト92、リスト93)。</p><p><span class="caption">▼リスト3.92: sample_jump.hex</span> <a href="https://github.com/nananapo/bluecore/compare/ccf4da568f058bcea8491110fe1da6e0787252e6~1..ccf4da568f058bcea8491110fe1da6e0787252e6#diff-4b870828046f4065482146d5dd02dccb5d0523eabe5af669e953963404e810ac">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>0100006f //  0: jal x0, 0x10 : 0x10にジャンプする
deadbeef //  4:
deadbeef //  8:
deadbeef //  c:
...
</code></pre></div><p><span class="caption">▼リスト3.93: テストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
...
</code></pre></div><p>brunitモジュールは、 <code>funct3</code>に応じて<code>take</code>の条件を切り替えます。 分岐が成立するときに<code>take</code>が<code>1</code>になります。</p><p>brunitモジュールを、 coreモジュールでインスタンス化します(リスト95)。 命令がB形式のとき、 <code>op1</code>は<code>rs1_data</code>、 <code>op2</code>は<code>rs2_data</code>になっていることを確認してください(リスト43)。</p><p><span class="caption">▼リスト3.95: brunitモジュールのインスタンス化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/a68fba03fabbdbe072263478a8ec0387fdbbf9d9~1..a68fba03fabbdbe072263478a8ec0387fdbbf9d9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> brunit_take: <span class="hljs-keyword">logic</span>;
...
</code></pre></div><p>命令が条件分岐命令で<code>brunit_take</code>が<code>1</code>のとき、 次のPCをPC + 即値にします ( リスト96、 リスト97 )。</p><p><span class="caption">▼リスト3.96: 命令が条件分岐命令か判定する関数 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/a68fba03fabbdbe072263478a8ec0387fdbbf9d9~1..a68fba03fabbdbe072263478a8ec0387fdbbf9d9#diff-6e03bfb8e3afdba1e0b88b561d8b8e5e4f2a7ffc6c4efb228c5797053b78742c">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// 命令が分岐命令かどうかを判定する</span>
...
</code></pre></div><p><span class="caption">▼リスト3.97: 分岐成立時のPCの設定 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/a68fba03fabbdbe072263478a8ec0387fdbbf9d9~1..a68fba03fabbdbe072263478a8ec0387fdbbf9d9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = inst_valid &amp;&amp; (inst_ctrl.is_jump || inst_is_br(inst_ctrl) &amp;&amp; brunit_take);
...
</code></pre></div><p><code>control_hazard</code>は、命令が無条件ジャンプ命令か、命令が条件分岐命令かつ分岐が成立するときに<code>1</code>になります。 <code>control_hazard_pc_next</code>は、無条件ジャンプ命令のときは<code>alu_result</code>、条件分岐命令のときはPC + 即値になります。</p><h4 id="条件分岐命令のテスト" tabindex="-1">条件分岐命令のテスト <a class="header-anchor" href="#条件分岐命令のテスト" aria-label="Permalink to “条件分岐命令のテスト”">​</a></h4><p>条件分岐命令を実行するとき、分岐の成否をデバッグ表示します。 デバッグ表示を行っているalways_ffブロック内に、次のコードを追加します(リスト98)。</p><p><span class="caption">▼リスト3.98: 分岐判定のデバッグ表示 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/a68fba03fabbdbe072263478a8ec0387fdbbf9d9~1..a68fba03fabbdbe072263478a8ec0387fdbbf9d9#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-very"><button title="Copy Code" class="copy"></button><span class="lang">very</span><pre class="hljs"><code>if inst_is_br(inst_ctrl) {
    $display(&quot;  br take   : %b&quot;, brunit_take);
}
</code></pre></div><p>簡単なテストを作成し、動作をテストします(リスト99, リスト100)。</p><p><span class="caption">▼リスト3.99: sample_br.hex</span> <a href="https://github.com/nananapo/bluecore/compare/a68fba03fabbdbe072263478a8ec0387fdbbf9d9~1..a68fba03fabbdbe072263478a8ec0387fdbbf9d9#diff-ed70829e581bd3b76a3741500395b0a2d55950437f287a0db0b81ae8b589e4e0">差分をみる</a></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>00100093 //  0: addi x1, x0, 1
...
</code></pre></div><p><span class="caption">▼リスト3.100: テストの実行</span></p><div class="language-terminal"><button title="Copy Code" class="copy"></button><span class="lang">terminal</span><pre class="hljs"><code><span class="hljs-meta prompt_">$ </span><span class="language-bash">make build</span>
<span class="hljs-meta prompt_">$ </span><span class="language-bash">make sim</span>
