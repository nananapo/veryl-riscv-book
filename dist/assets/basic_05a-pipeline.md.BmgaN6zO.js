...
</code></pre></div><p><span class="caption">▼リスト7.6: FIFOのインスタンス化 (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> id_ex_fifo: fifo #(
    DATA_TYPE: exq_type,
...
</code></pre></div><h3 id="exステージを実装する" tabindex="-1">EXステージを実装する <a class="header-anchor" href="#exステージを実装する" aria-label="Permalink to “EXステージを実装する”">​</a></h3><p>EXステージでは、 整数演算命令のときはALUで計算し、 分岐命令のときは分岐判定を行います。</p><p>まず、EXステージに存在する命令の情報を<code>exq_rdata</code>から取り出します(リスト10)。</p><p><span class="caption">▼リスト7.10: 変数の定義 (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> exs_valid    : <span class="hljs-keyword">logic</span>    = exq_rvalid;
...
</code></pre></div><p>次に、EXステージで扱う変数の名前を変更します。 変数の名前に<code>exs_</code>をつけます (リスト11)。</p><p><span class="caption">▼リスト7.11: 変数名を変更する (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタ番号</span>
...
</code></pre></div><p>最後に、MEMステージに命令とデータを渡します。 MEMステージにデータを渡すために、 <code>memq_wdata</code>にデータを割り当てます (リスト12)。</p><p><span class="caption">▼リスト7.12: MEMステージにデータを渡す (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p><code>mem_is_new</code>には、<code>id_is_new</code>の更新に利用していたコードを利用します(リスト14)。</p><p><span class="caption">▼リスト7.14: mem_is_newの更新 (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
...
</code></pre></div><p>次に、MEMモジュールで使う変数に合わせて、 memunitモジュールとcsrunitモジュールのポートに割り当てている変数名を変更します (リスト15)。</p><p><span class="caption">▼リスト7.15: 変数名を変更する (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> memu_rdata: UIntX;
...
</code></pre></div><p>MEMステージにある命令は、 memunitモジュールが処理中ではなく(<code>!memy_stall</code>)、 WBステージが命令を受け入れられるとき(<code>wbq_wready</code>)、 MEMステージを完了してWBステージに処理を進められます。 この仕組みは、<code>memq_rready</code>と<code>wbq_wvalid</code>を確認してください。</p><h3 id="wbステージを実装する" tabindex="-1">WBステージを実装する <a class="header-anchor" href="#wbステージを実装する" aria-label="Permalink to “WBステージを実装する”">​</a></h3><p>WBステージでは、命令の結果をレジスタにライトバックします。 WBステージが完了したら命令の処理は終わりなので、命令を破棄します。</p><p>まず、WBステージに存在する命令の情報を<code>wbq_rdata</code>から取り出します (リスト18)。</p><p><span class="caption">▼リスト7.18: 変数の定義 (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> wbs_valid    : <span class="hljs-keyword">logic</span>    = wbq_rvalid;
...
Test Result : 0 / 1
</code></pre></div><p>おや? テストに失敗してしまいました。 一体何が起きているのでしょうか?</p><h2 id="データ依存の対処" tabindex="-1">データ依存の対処 <a class="header-anchor" href="#データ依存の対処" aria-label="Permalink to “データ依存の対処”">​</a></h2><h3 id="正しく動かないプログラムを確認する" tabindex="-1">正しく動かないプログラムを確認する <a class="header-anchor" href="#正しく動かないプログラムを確認する" aria-label="Permalink to “正しく動かないプログラムを確認する”">​</a></h3><p>実は、ただIF、ID、EX、MEM、WBステージに処理を分割するだけでは、 正しく命令を実行できません。 例えば、リスト23のようなプログラムは正しく動きません。</p><p><code>test/sample_datahazard.hex</code>を作成し、次のように記述します (リスト23)。</p><p><span class="caption">▼リスト7.23: sample_datahazard.hex</span></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>0010811300100093 // 0:addi x1, x0, 1    4: addi x2, x1, 1
...
</code></pre></div><p>次に、データ依存があるときに、データハザードを発生させます (リスト26)。 データハザードを起こすためには、 EXステージのFIFOの<code>rready</code>とMEMステージの<code>wvalid</code>に、 データハザードが発生していないという条件を加えます。</p><p><span class="caption">▼リスト7.26: データ依存があるときにデータハザードを起こす (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
    <span class="hljs-comment">// EX -&gt; MEM</span>
...
</code></pre></div><p>最後に、データハザードが発生しているかどうかをデバッグ表示します (リスト27)。</p><p><span class="caption">▼リスト7.27: データハザードが発生しているかをデバッグ表示する (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>$display(<span class="hljs-string">&quot;EX -----&quot;</span>);
<span class="hljs-keyword">if</span> exs_valid {
    $display(<span class="hljs-string">&quot;  %h : %h&quot;</span>, exq_rdata.addr, exq_rdata.bits);
    $display(<span class="hljs-string">&quot;  op1     : %h&quot;</span>, exs_op1);
    $display(<span class="hljs-string">&quot;  op2     : %h&quot;</span>, exs_op2);
