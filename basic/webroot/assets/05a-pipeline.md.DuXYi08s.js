...
</code></pre></div><p><code>inst_valid</code>、<code>inst_is_new</code>、<code>inst_pc</code>、 <code>inst_bits</code>、<code>inst_ctrl</code>、<code>inst_imm</code>の名前をリスト1のように変更します。 定義だけではなく、変数を使用しているところもすべて変更してください。</p><h3 id="fifoを作成する" tabindex="-1">FIFOを作成する <a class="header-anchor" href="#fifoを作成する" aria-label="Permalink to “FIFOを作成する”">​</a></h3><p>命令フェッチ処理とそれ以降の処理は、それぞれ独立して動作しています。 実は既にCPUは、IFとIDステージ(命令フェッチ以外の処理を行うステージ)の2ステージのパイプライン処理を行っています。</p><p>IFステージとIDステージはFIFOで区切られており、 FIFOのレジスタを経由して命令の受け渡しを行います。 これと同様に、 5ステージのパイプライン処理の実装では、 それぞれのステージをFIFOで接続します(図5)。 ただし、FIFOのサイズは1とします。 この場合、FIFOはただの1つのレジスタです。</p><p><img src="`+r+'" alt="FIFOを利用したパイプライン処理"> IFからIDへのFIFOは存在するため、 IDからEX、EXからMEM、MEMからWBへのFIFOを作成します。</p><h4 id="構造体の定義" tabindex="-1">構造体の定義 <a class="header-anchor" href="#構造体の定義" aria-label="Permalink to “構造体の定義”">​</a></h4><p><img src="'+o+`" alt="構造体のフィールドの生存区間"> まず、FIFOに格納するデータの型を定義します。 それぞれのフィールドが存在する区間は図6の通りです。</p><p><span class="caption">▼リスト7.2: ID → EXの間のFIFOのデータ型 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ce0d9f4631056598a118d0a3b72b77828e274f8f~1..ce0d9f4631056598a118d0a3b72b77828e274f8f#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">struct</span> exq_type {
    addr: Addr    ,
    bits: Inst    ,
    ctrl: InstCtrl,
    imm : UIntX   ,
}
</code></pre></div><p>IDステージは、IFステージから命令のアドレスと命令のビット列を受け取ります。 命令のビット列をデコードして、制御フラグと即値を生成し、EXステージに渡します(リスト2)。</p><p><span class="caption">▼リスト7.3: EX → MEMの間のFIFOのデータ型 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ce0d9f4631056598a118d0a3b72b77828e274f8f~1..ce0d9f4631056598a118d0a3b72b77828e274f8f#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">struct</span> memq_type {
    addr      : Addr       ,
    bits      : Inst       ,
    ctrl      : InstCtrl   ,
    imm       : UIntX      ,
...
</code></pre></div><p><span class="caption">▼リスト7.6: FIFOのインスタンス化 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/ce0d9f4631056598a118d0a3b72b77828e274f8f~1..ce0d9f4631056598a118d0a3b72b77828e274f8f#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> id_ex_fifo: fifo #(
    DATA_TYPE: exq_type,
...
</code></pre></div><h3 id="ifステージを実装する" tabindex="-1">IFステージを実装する <a class="header-anchor" href="#ifステージを実装する" aria-label="Permalink to “IFステージを実装する”">​</a></h3><p>まず、IFステージを実装します。 ...といっても、 既にIFステージ(=命令フェッチ処理)は独立に動くものとして実装されているため、 手を加える必要はありません。</p><p>リスト7のようなコメントを挿入すると、 ステージの処理を書いている区間が分かりやすくなります。 ID、EX、MEM、WBステージを実装するときにも同様のコメントを挿入し、 ステージの処理のコードをまとまった場所に配置しましょう。</p><p><span class="caption">▼リスト7.7: IFステージが始まることを示すコメントを挿入する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">///////////////////////////////// IF Stage /////////////////////////////////</span>

<span class="hljs-keyword">var</span> if_pc          : Addr ;
...
</code></pre></div><h3 id="idステージを実装する" tabindex="-1">IDステージを実装する <a class="header-anchor" href="#idステージを実装する" aria-label="Permalink to “IDステージを実装する”">​</a></h3><p>IDステージでは、命令をデコードします。 既に<code>ids_ctrl</code>と<code>ids_imm</code>には、 デコード結果の制御フラグと即値が割り当てられているため、 既存のコードの変更は必要ありません。</p><p>デコード結果はEXステージに渡します。 EXステージにデータを渡すには、 <code>exq_wdata</code>にデータを割り当てます (リスト8)。</p><p><span class="caption">▼リスト7.8: EXステージに値を渡す (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>IDステージにある命令は、 EXステージが命令を受け入れられるとき(<code>exq_wready</code>)、 IDステージを完了してEXステージに処理を進められます。 この仕組みは、 <code>if_fifo_rready</code>に<code>exq_wready</code>を割り当てることで実現できます。</p><p>最後に、命令が現在のクロックで供給されたかどうかを示す変数<code>id_is_new</code>は必要ないため削除します (リスト9)。</p><p><span class="caption">▼リスト7.9: ids_is_newを削除する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="custom-hl-del"><span class="hljs-keyword">var</span> ids_is_new   : <span class="hljs-keyword">logic</span>   ;</span>
</code></pre></div><h3 id="exステージを実装する" tabindex="-1">EXステージを実装する <a class="header-anchor" href="#exステージを実装する" aria-label="Permalink to “EXステージを実装する”">​</a></h3><p>EXステージでは、 整数演算命令のときはALUで計算し、 分岐命令のときは分岐判定を行います。</p><p>まず、EXステージに存在する命令の情報を<code>exq_rdata</code>から取り出します(リスト10)。</p><p><span class="caption">▼リスト7.10: 変数の定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> exs_valid    : <span class="hljs-keyword">logic</span>    = exq_rvalid;
...
</code></pre></div><p>次に、EXステージで扱う変数の名前を変更します。 変数の名前に<code>exs_</code>をつけます (リスト11)。</p><p><span class="caption">▼リスト7.11: 変数名を変更する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// レジスタ番号</span>
...
</code></pre></div><p>最後に、MEMステージに命令とデータを渡します。 MEMステージにデータを渡すために、 <code>memq_wdata</code>にデータを割り当てます (リスト12)。</p><p><span class="caption">▼リスト7.12: MEMステージにデータを渡す (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p><code>mem_is_new</code>には、<code>id_is_new</code>の更新に利用していたコードを利用します(リスト14)。</p><p><span class="caption">▼リスト7.14: mem_is_newの更新 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
...
</code></pre></div><p>次に、MEMモジュールで使う変数に合わせて、 memunitモジュールとcsrunitモジュールのポートに割り当てている変数名を変更します (リスト15)。</p><p><span class="caption">▼リスト7.15: 変数名を変更する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> memu_rdata: UIntX;
...
</code></pre></div><p>フェッチ先が変わったことを表す変数<code>control_hazard</code>と、 新しいフェッチ先を示す信号<code>control_hazard_pc_next</code>では、 EXステージで計算したデータとCSRステージのトラップ情報を利用します (リスト16)。</p><p><span class="caption">▼リスト7.16: ジャンプの判定処理 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">assign</span> control_hazard         = <span class="custom-hl-bold">mems</span>_valid &amp;&amp; (csru_raise_trap || <span class="custom-hl-bold">mems_</span>ctrl.is_jump || <span class="custom-hl-bold">memq_rdata.</span>br_taken);
...
</code></pre></div><p>ジャンプ命令の後ろの余計な命令を実行しないために、 <code>control_hazard</code>が<code>1</code>になったとき、 ID、EX、MEMステージに命令を供給するFIFOをフラッシュします。 <code>control_hazard</code>が<code>1</code>になるとき、 MEMステージの処理は完了しています。 後述しますが、WBステージの処理は必ず1クロックで終了します。 そのため、フラッシュするとき、 MEMステージにある命令は必ずWBステージに移動します。</p><p>最後に、WBステージに命令とデータを渡します(リスト17)。 WBステージにデータを渡すために、 <code>wbq_wdata</code>にデータを割り当てます</p><p><span class="caption">▼リスト7.17: WBステージにデータを渡す (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
...
</code></pre></div><p>MEMステージにある命令は、 memunitモジュールが処理中ではなく(<code>!memy_stall</code>)、 WBステージが命令を受け入れられるとき(<code>wbq_wready</code>)、 MEMステージを完了してWBステージに処理を進められます。 この仕組みは、<code>memq_rready</code>と<code>wbq_wvalid</code>を確認してください。</p><h3 id="wbステージを実装する" tabindex="-1">WBステージを実装する <a class="header-anchor" href="#wbステージを実装する" aria-label="Permalink to “WBステージを実装する”">​</a></h3><p>WBステージでは、命令の結果をレジスタにライトバックします。 WBステージが完了したら命令の処理は終わりなので、命令を破棄します。</p><p>まず、WBステージに存在する命令の情報を<code>wbq_rdata</code>から取り出します (リスト18)。</p><p><span class="caption">▼リスト7.18: 変数の定義 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">let</span> wbs_valid    : <span class="hljs-keyword">logic</span>    = wbq_rvalid;
...
</code></pre></div><p>最後に、命令をFIFOから取り出します。 WBステージでは命令を複数クロックで処理することはなく、 WBステージの次のステージを待つ必要もありません。 <code>wbq_rready</code>に<code>1</code>を割り当てることで、 常にFIFOから命令を取り出します(リスト20)。</p><p><span class="caption">▼リスト7.20: 命令をFIFOから取り出す (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
    <span class="hljs-comment">// WB -&gt; END</span>
    wbq_rready = <span class="hljs-number">1</span>;
}
</code></pre></div><p>これで、IF、ID、EX、MEM、WBステージを作成できました。</p><h3 id="デバッグのために情報を表示する" tabindex="-1">デバッグのために情報を表示する <a class="header-anchor" href="#デバッグのために情報を表示する" aria-label="Permalink to “デバッグのために情報を表示する”">​</a></h3><p>今までは同時に1つの命令しか処理していませんでしたが、 これからは全てのステージで別の命令を処理することになります。 デバッグ表示を変更しておきましょう。</p><p>リスト21のように、デバッグ表示のalways_ffブロックを変更します。</p><p><span class="caption">▼リスト7.21: 各ステージの情報をデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e~1..25aa0d6d869faf3acb21c3fa12b35b50f2b28c1e#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">///////////////////////////////// DEBUG /////////////////////////////////</span>
...
MEM -----
  0000000000000000 : 00100093
  ...
</code></pre></div><p>ログを確認すると、 アドレス0の命令でx1が1になっているはずですが、 アドレス4の命令でx1を読み込むときにx1は0になっています。</p><p>この問題は、 まだアドレス0の命令の結果がレジスタファイルに書き込まれていないのに、 アドレス4の命令でレジスタファイルで結果を読み出しているために発生しています。</p><h3 id="データ依存とは何か" tabindex="-1">データ依存とは何か？ <a class="header-anchor" href="#データ依存とは何か" aria-label="Permalink to “データ依存とは何か？”">​</a></h3><p>ある命令Aの実行結果の値を利用する命令Bが存在するとき、 命令Aと命令Bの間には<strong>データ依存</strong>(data dependence)があると呼びます。 データ依存に対処するためには、 命令Aの結果がレジスタに書き込まれるのを待つ必要があります。 データ依存があることにより発生するパイプラインハザードのことを <strong>データハザード</strong>(data hazard)と呼びます。</p><p><img src="`+b+`" alt="データ依存関係のあるプログラム"></p><h3 id="データ依存に対処する" tabindex="-1">データ依存に対処する <a class="header-anchor" href="#データ依存に対処する" aria-label="Permalink to “データ依存に対処する”">​</a></h3><p>レジスタの値を読み出すのはEXステージです。 データ依存に対処するために、 データ依存関係があるときにEXステージをストールさせます。</p><p>まず、MEMとEXか、WBとEXステージにある命令の間にデータ依存があることを検知します (リスト25)。 例えばMEMステージとデータ依存の関係にあるとき、 MEMステージの命令はライトバックする命令で、 rdがEXステージのrs1、またはrs2と一致しています。</p><p><span class="caption">▼リスト7.25: データ依存の検知 (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2231149cc2eb45298dcd2a5b5cec9bae26c017d5~1..2231149cc2eb45298dcd2a5b5cec9bae26c017d5#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// データハザード</span>
...
</code></pre></div><p>次に、データ依存があるときに、データハザードを発生させます (リスト26)。 データハザードを起こすためには、 EXステージのFIFOの<code>rready</code>とMEMステージの<code>wvalid</code>に、 データハザードが発生していないという条件を加えます。</p><p><span class="caption">▼リスト7.26: データ依存があるときにデータハザードを起こす (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2231149cc2eb45298dcd2a5b5cec9bae26c017d5~1..2231149cc2eb45298dcd2a5b5cec9bae26c017d5#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_comb</span> {
    <span class="hljs-comment">// EX -&gt; MEM</span>
...
</code></pre></div><p>最後に、データハザードが発生しているかどうかをデバッグ表示します (リスト27)。</p><p><span class="caption">▼リスト7.27: データハザードが発生しているかをデバッグ表示する (core.veryl)</span> <a href="https://github.com/nananapo/bluecore/compare/2231149cc2eb45298dcd2a5b5cec9bae26c017d5~1..2231149cc2eb45298dcd2a5b5cec9bae26c017d5#diff-bdad1723f95a5423ff5ab8ba69bb572aabe1c8def0cda1748f6f980f61b57510">差分をみる</a></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>$display(<span class="hljs-string">&quot;EX -----&quot;</span>);
<span class="hljs-keyword">if</span> exs_valid {
    $display(<span class="hljs-string">&quot;  %h : %h&quot;</span>, exq_rdata.addr, exq_rdata.bits);
    $display(<span class="hljs-string">&quot;  op1     : %h&quot;</span>, exs_op1);
    $display(<span class="hljs-string">&quot;  op2     : %h&quot;</span>, exs_op2);
