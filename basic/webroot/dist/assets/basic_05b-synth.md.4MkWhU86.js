import{_ as a,c as e,o as n,af as l,aF as p,aG as o,aH as t,aI as c,aJ as r,aK as d,aL as i,aM as h,aN as _,aO as m,aP as u,aQ as g,aR as b,aS as y,aT as f,aU as j,aV as E,aW as k,aX as P,aY as L,aZ as v,a_ as C,a$ as D,b0 as w,b1 as A,b2 as x,b3 as M,b4 as O,b5 as T,b6 as I,b7 as N,b8 as R,b9 as q,ba as S,bb as V,bc as F,bd as U,be as Y,bf as G,bg as H,bh as K,bi as W,bj as Q,bk as Z,bl as X,bm as z,bn as $,bo as B,bp as J,bq as ss,br as as,bs as es,bt as ns,bu as ls,bv as ps,bw as os,bx as ts}from"./chunks/framework.D5l_65jD.js";const gs=JSON.parse('{"title":"CPUの合成","description":"","frontmatter":{},"headers":[],"relativePath":"basic/05b-synth.md","filePath":"basic/05b-synth.md"}'),cs={name:"basic/05b-synth.md"};function rs(ds,s,is,hs,_s,ms){return n(),e("div",null,[...s[0]||(s[0]=[l('<h1 id="cpuの合成" tabindex="-1">CPUの合成 <a class="header-anchor" href="#cpuの合成" aria-label="Permalink to “CPUの合成”">​</a></h1><p>これまでの章では、RV64IのCPUを作成してパイプライン化しました。 今までは動作確認とテストはシミュレータで行っていましたが、 本章では実機(FPGA)でCPUを動かします。</p><p><img src="'+p+'" alt="PYNQ-Z1"></p><h2 id="fpgaとは何か" tabindex="-1">FPGAとは何か？ <a class="header-anchor" href="#fpgaとは何か" aria-label="Permalink to “FPGAとは何か？”">​</a></h2><p>集積回路を製造するには手間と時間とお金が必要です。 FPGAを使うと、少しの手間と少しの時間、安価に集積回路の実現をお試しできます。</p><p><strong>FPGA</strong>(Field Programmable Gate Array)は、 任意の論理回路を実現できる集積回路のことです。 ハードウェア記述言語で設計した論理回路をFPGA上に設定することで、 実際に集積回路を製造しなくても実機で論理回路を再現できます。</p><p>「任意の論理回路を実現できる集積回路」は、 主にプロダクトターム方式、またはルックアップ・テーブル方式で構成されています。 本書では<strong>ルックアップ・テーブル</strong>(Lookup Table, <strong>LUT</strong>)方式のFPGAを利用します。</p><div id="lut_sample_truth" class="table"><p class="caption">表8.1: 真理値表の例</p><table><tr class="hline"><th>X</th><th>Y</th><th>A</th></tr><tr class="hline"><td>0</td><td>0</td><td>0</td></tr><tr class="hline"><td>0</td><td>1</td><td>1</td></tr><tr class="hline"><td>1</td><td>0</td><td>1</td></tr><tr class="hline"><td>1</td><td>1</td><td>0</td></tr></table></div> ![表1を実現するLUT](./images/05b-synth/lut.png) LUTとは、真理値表を記憶素子に保存しておいて、 入力によって記憶された真理値を選択して出力する回路のことです。 例えば、2つの入力`X`と`Y`を受け取って`A`を出力する論理回路(表1)は、図2の回路で実現できます。 ここでマルチプレクサ(multiplexer, MUX)とは、複数の入力を選択信号によって選択して出力する回路のことです。 <p>図2では、記憶素子のデータを<code>Y</code>によって選択し、さらに<code>X</code>によって選択することで2入力1出力の真理値表の論理回路を実現しています。 入力がN個で出力がM個のLUTのことをN入力M出力LUTと呼びます。</p><p>ルックアップ・テーブル方式のFPGAは、多数のLUT、入出力装置、これらを相互接続するための配線によって構成されています。 また、乗算回路やメモリなどの部品はFPGAよりも専用の回路で実現した方が良い<sup class="footnote-ref"><a href="#fn1" id="fnref1">[1]</a></sup>ので、 メモリや乗算回路の部品が内蔵されていることがあります。</p><p>本書では2つのFPGA(Tang Nano 9K、PYNQ-Z1)を使用して実機でCPUを動作させます。 2024年11月12日時点ではどちらも秋月電子通商で入手できて、 Tang Nano 9Kは3000円くらい、 PYNQ-Z1は50000円くらいで入手できます。</p><h2 id="ledの制御" tabindex="-1">LEDの制御 <a class="header-anchor" href="#ledの制御" aria-label="Permalink to “LEDの制御”">​</a></h2><p><img src="'+o+'" alt="Tang Mega 138K ProのLED(6個)"> 大抵のFPGAボードにはLEDがついています。 本章では2つのテストプログラム(LEDを点灯させる、LEDを点滅させる)によって、CPUの動作確認を行います。</p><p>LEDはトップモジュールのポートを経由して制御します(図4)。 ポートとLEDの接続方法は合成系によって異なるため、それらの接続方法は後で考えます。 CPUからLEDを制御するには、メモリ経由で制御する、CSRによって制御するなどの方法が考えられます。 本書ではLEDを制御するためのCSRを実装して、CSRをトップモジュールのポートに接続することでLEDを制御します。</p><p><img src="'+t+`" alt="CSRのLED制御用レジスタがLEDに接続される"></p><h3 id="csrにled制御用レジスタを実装する" tabindex="-1">CSRにLED制御用レジスタを実装する <a class="header-anchor" href="#csrにled制御用レジスタを実装する" aria-label="Permalink to “CSRにLED制御用レジスタを実装する”">​</a></h3><p>RISC-VのCSRのアドレス空間には、読み込みと書き込みができるCSRを自由に定義できる場所(<code>0x800</code>から<code>0x8FF</code>)が用意されています<sup class="footnote-ref"><a href="#fn2" id="fnref2">[2]</a></sup>。 これの先頭アドレス<code>0x800</code>をLEDの制御用レジスタのアドレスとして実装を進めます。</p><p>まず、<code>CsrAddr</code>型にLED制御用レジスタのアドレスを追加します(リスト1)。</p><p><span class="caption">▼リスト8.1: LEDの制御用レジスタのアドレスを追加する (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">enum</span> CsrAddr: <span class="hljs-keyword">logic</span>&lt;<span class="hljs-number">12</span>&gt; {
...
</code></pre></div><p>書き込みマスクはすべて書き込み可にします(リスト2)。</p><p><span class="caption">▼リスト8.2: LEDの制御用レジスタの書き込みマスク (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">const</span> LED_WMASK   : UIntX = <span class="hljs-number">&#39;hffff_ffff_ffff_ffff</span>;
</code></pre></div><p>LEDの制御用レジスタをcsrunitモジュールのポートに定義します。CSRの幅はUIntXです(リスト3)。</p><p><span class="caption">▼リスト8.3: LEDの制御用レジスタを定義する (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> csrunit (
    ...
...
</code></pre></div><p><code>rdata</code>と<code>wmask</code>にLEDの制御用レジスタの値を割り当てます(リスト4)。</p><p><span class="caption">▼リスト8.4: rdataとwmaskに値を割り当てる (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// read</span>
...
wmask = <span class="hljs-keyword">case</span> csr_addr {
...
</code></pre></div><p>リセット時にLEDの制御用レジスタの値を0に設定します(リスト5)。</p><p><span class="caption">▼リスト8.5: リセット値の設定 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">if_reset</span> {
...
</code></pre></div><p>LEDの制御用レジスタへの書き込み処理を実装します(リスト6)。</p><p><span class="caption">▼リスト8.6: LEDの制御用レジスタへの書き込み (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">case</span> csr_addr {
...
</code></pre></div><h3 id="トップモジュールにledを制御するポートを実装する" tabindex="-1">トップモジュールにLEDを制御するポートを実装する <a class="header-anchor" href="#トップモジュールにledを制御するポートを実装する" aria-label="Permalink to “トップモジュールにLEDを制御するポートを実装する”">​</a></h3><p>LEDはトップモジュールのポートを経由して制御します(図4)。 そのため、トップモジュールにLEDを制御するポートを作成して、csrunitのLEDの制御用レジスタの値を接続します (リスト7、リスト8、リスト9、リスト10)。 LEDの個数はFPGAによって異なるため、とりあえずXLEN(=64)ビットのポートを定義します。</p><p><span class="caption">▼リスト8.7: coreモジュールにポートを追加する (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> core (
    ...
    <span class="custom-hl-bold">led     : <span class="hljs-keyword">output</span>  UIntX                                    ,</span>
</code></pre></div><p><span class="caption">▼リスト8.8: csrunitモジュールのledポートと接続する (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> csru: csrunit (
    clk                            ,
    rst                            ,
    valid   : mems_valid           ,
    pc      : mems_pc              ,
...
</code></pre></div><p><span class="caption">▼リスト8.9: topモジュールにポートを追加する (top.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> top #(
...
</code></pre></div><p><span class="caption">▼リスト8.10: coreモジュールのledポートと接続する (top.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> c: core (
    clk       ,
    rst       ,
    i_membus  ,
    d_membus  ,
...
</code></pre></div><h3 id="プロジェクトを作成する" tabindex="-1">プロジェクトを作成する <a class="header-anchor" href="#プロジェクトを作成する" aria-label="Permalink to “プロジェクトを作成する”">​</a></h3><p>新規プロジェクトを作成します。 GOWIN FPGA Designerを開いて、Quick Startの<code>New Project...</code>を選択します。 選択したら表示されるウィンドウでは、FPGA Design Projectを選択してOKを押してください(図6)。</p><p><img src="`+r+'" alt="FPGA Design Projectを選択する"> プロジェクト名と場所を指定します。 プロジェクト名はtangnano9k、場所は好きな場所に指定してください(図7)。</p><p><img src="'+d+'" alt="プロジェクト名と場所の指定"> ターゲットのFPGAを選択します。 <code>GW1NR-LV9QN88PC6/I5</code>を選択して、Nextを押してください(図8)。</p><p><img src="'+i+'" alt="ターゲットを選択する"> プロジェクトが作成されました(図9)。</p><p><img src="'+h+'" alt="プロジェクトが作成された"></p><h3 id="設定を変更する" tabindex="-1">設定を変更する <a class="header-anchor" href="#設定を変更する" aria-label="Permalink to “設定を変更する”">​</a></h3><p>プロジェクトのデフォルト設定ではSystemVerilogを利用できないため、設定を変更します。</p><p>ProjectのConfigurationから、設定画面を開きます(図10)。</p><p><img src="'+_+'" alt="設定画面を開く"> SynthesizeのVerilog Languageを<code>System Verilog 2017</code>に設定します。 同じ画面でトップモジュール(Top Module/Entity)を設定できるため、<code>core_top_tang</code>を指定します(図11)。</p><p><img src="'+m+`" alt="設定を変更する"></p><h3 id="設計ファイルを追加する" tabindex="-1">設計ファイルを追加する <a class="header-anchor" href="#設計ファイルを追加する" aria-label="Permalink to “設計ファイルを追加する”">​</a></h3><p>Verylのソースファイルをビルドして、 生成されるファイルリスト(<code>core.f</code>)を利用して、 生成されたSystemVerilogソースファイルをプロジェクトに追加します。</p><p>Gowin FPGA Programmerでファイルを追加するには、 ウィンドウ下部のConsole画面で<code>add_file</code>を実行します。 しかし、<code>add_file</code>はファイルリストの読み込みに対応していないので、 ファイルリストを読み込んで<code>add_file</code>を実行するスクリプトを作成します(リスト16)。</p><p><span class="caption">▼リスト8.16: add_files.tcl</span></p><div class="language-tcl"><button title="Copy Code" class="copy"></button><span class="lang">tcl</span><pre class="hljs"><code><span class="hljs-keyword">set</span> file_list [<span class="hljs-keyword">open</span> <span class="hljs-string">&quot;ファイルリストのパス&quot;</span> r]
...
</code></pre></div><p><code>IO_LOC</code>で接続する場所の名前を指定します。</p><p>場所の名前はTang Nano 9Kのデータシートで確認できます。 例えば図17と図18から、 LEDは<code>10</code>、<code>11</code>、<code>13</code>、<code>14</code>、<code>15</code>、<code>16</code>に割り当てられていることが分かります。 また、LEDが負論理(<code>1</code>で消灯、<code>0</code>で点灯)であることが分かります。 水晶発信器とボタンについても、データシートを見て確認してください。</p><p><img src="`+j+'" alt="LED"><img src="'+E+'" alt="PIN10_IOL～の接続先"></p><h4 id="タイミング制約" tabindex="-1">タイミング制約 <a class="header-anchor" href="#タイミング制約" aria-label="Permalink to “タイミング制約”">​</a></h4><p>FPGAが何MHzで動くかをタイミング制約ファイルに記述します。</p><p>物理制約ファイルと同じようにAdd Fileを選択して、 タイミング制約ファイルを作成します(図19)。</p><p><img src="'+k+'" alt="タイミング制約ファイルを選択する"> 名前は<code>timing.sdc</code>にします(図20)。</p><p><img src="'+P+`" alt="名前を設定する"> タイミング制約ファイルには、次のように記述します(リスト19)。</p><p><span class="caption">▼リスト8.19: タイミング制約ファイル (timing.sdc)</span></p><div class="language-sdc"><button title="Copy Code" class="copy"></button><span class="lang">sdc</span><pre class="hljs"><code>create_clock -name clk -period 37.037 -waveform {0 18.518} [get_ports {clk}]
