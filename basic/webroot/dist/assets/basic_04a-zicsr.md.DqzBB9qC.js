...
</code></pre></div><p>これでデコード処理を書く準備が整いました。 inst_decoderモジュールの<code>InstCtrl</code>を生成している部分を変更します(リスト3)。</p><p><span class="caption">▼リスト4.3: OP_SYSTEMとis_csrを追加する (inst_decoder.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>                                        ↓ is_csrを追加
ctrl = {
...
</code></pre></div><p>次に、mtvecレジスタを作成します。 MXLEN=XLENとしているので、 型は<code>UIntX</code>にします (リスト9)。</p><p><span class="caption">▼リスト4.9: mtvecレジスタの定義 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// CSR</span>
<span class="hljs-keyword">var</span> mtvec: UIntX;
</code></pre></div><p>MODEはDirectモード(<code>2&#39;b00</code>)しか対応していません。 mtvecはWARLなレジスタなので、 MODEフィールドには書き込めないようにする必要があります。 これを制御するためにmtvecレジスタの書き込みマスク用の定数を定義します (リスト10)。</p><p><span class="caption">▼リスト4.10: mtvecレジスタの書き込みマスクの定義 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// wmasks</span>
<span class="hljs-keyword">const</span> MTVEC_WMASK: UIntX = <span class="hljs-number">&#39;hffff_fffc</span>;
</code></pre></div><p>次に、書き込むデータ<code>wdata</code>の生成と、 mtvecレジスタの読み込みを実装します(リスト11)。</p><p><span class="caption">▼リスト4.11: レジスタの読み込みと書き込むデータの作成 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">var</span> wmask: UIntX; <span class="hljs-comment">// write mask</span>
...
</code></pre></div><p>mtvecの初期値は<code>0</code>です。 mtvecに<code>wdata</code>を書き込むとき、 MODEが常に<code>2&#39;b00</code>になります。</p><h3 id="csrunitモジュールをテストする" tabindex="-1">csrunitモジュールをテストする <a class="header-anchor" href="#csrunitモジュールをテストする" aria-label="Permalink to “csrunitモジュールをテストする”">​</a></h3><p>mtvecレジスタの書き込み、 読み込みができることを確認します。</p><p><code>test/sample_csr.hex</code>を作成し、 次のように記述します(リスト13)。</p><p><span class="caption">▼リスト4.13: sample_csr.hex</span></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>305bd0f3 // 0: csrrwi x1, mtvec, 0b10111
...
</code></pre></div><p>次に、トラップの発生原因を表現する型<code>CsrCause</code>を定義します。 今のところ、発生原因はECALL命令によるEnvironment Call From M-mode例外しかありません (リスト16)。</p><p><span class="caption">▼リスト4.16: CsrCause型の定義 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">enum</span> CsrCause: UIntX {
...
</code></pre></div><p>次に、mepcとmcauseの読み込み処理と、書き込みマスクの割り当てを実装します。 どちらもcase文にアドレスと値のペアを追加するだけです ( リスト19、 リスト20 )。</p><p><span class="caption">▼リスト4.19: mepcとmcauseの読み込み (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>rdata = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : mtvec,
    <span class="custom-hl-bold">CsrAddr::MEPC  : mepc,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: mcause,</span>
    <span class="hljs-keyword">default</span>        : &#39;x,
};
</code></pre></div><p><span class="caption">▼リスト4.20: mepcとmcauseの書き込みマスクの設定 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code>wmask = <span class="hljs-keyword">case</span> csr_addr {
    CsrAddr::MTVEC : MTVEC_WMASK,
    <span class="custom-hl-bold">CsrAddr::MEPC  : MEPC_WMASK,</span>
    <span class="custom-hl-bold">CsrAddr::MCAUSE: MCAUSE_WMASK,</span>
    <span class="hljs-keyword">default</span>        : <span class="hljs-number">0</span>,
};
</code></pre></div><p>最後に、mepcとmcauseの書き込みを実装します。 if_resetで値を<code>0</code>に初期化し、case文にmepcとmcauseの場合を実装します (リスト21)。</p><p><span class="caption">▼リスト4.21: mepcとmcauseの書き込み (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">always_ff</span> {
    <span class="hljs-keyword">if_reset</span> {
        mtvec  = <span class="hljs-number">0</span>;
...
</code></pre></div><h4 id="例外の実装" tabindex="-1">例外の実装 <a class="header-anchor" href="#例外の実装" aria-label="Permalink to “例外の実装”">​</a></h4><p>ECALL命令と、それによって発生するトラップを実装します。 まず、csrunitモジュールにポートを追加します (リスト22)。</p><p><span class="caption">▼リスト4.22: csrunitモジュールにポートを追加する (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">module</span> csrunit (
    clk        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">clock</span>       ,
    rst        : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">reset</span>       ,
    valid      : <span class="hljs-keyword">input</span>  <span class="hljs-keyword">logic</span>       ,
...
</code></pre></div><p><span class="caption">▼リスト4.24: csrunitモジュールのポートの定義を変更する ② (core.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-keyword">inst</span> csru: csrunit (
    clk                       ,
    rst                       ,
    valid   : inst_valid      ,
...
</code></pre></div><p><img src="`+e+`" alt="ECALL命令のエンコーディング"> それでは、csrunitモジュールにトラップの処理を実装します。</p><p>ECALL命令は、 I形式、 即値は<code>0</code>、 rs1とrdは<code>0</code>、 funct3は<code>0</code>、 opcodeは<code>SYSTEM</code>な命令です(図2)。 これを判定するための変数を作成します(リスト26)。</p><p><span class="caption">▼リスト4.26: ECALL命令かどうかの判定 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// ECALL命令かどうか</span>
...
</code></pre></div><p>次に、例外が発生するかどうかを示す<code>raise_expt</code>と、 例外の発生の原因を示す<code>expt_cause</code>を作成します。 今のところ、例外はECALL命令によってのみ発生するため、 <code>expt_cause</code>は実質的に定数になっています (リスト27)。</p><p><span class="caption">▼リスト4.27: 例外とトラップの判定 (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// Exception</span>
...
</code></pre></div><p><code>test/sample_ecall.hex</code>を作成し、 次のように記述します (リスト30)。</p><p><span class="caption">▼リスト4.30: sample_ecall.hex</span></p><div class="language-hex"><button title="Copy Code" class="copy"></button><span class="lang">hex</span><pre class="hljs"><code>30585073 //  0: csrrwi x0, mtvec, 0x10
00000073 //  4: ecall
00000000 //  8:
00000000 //  c:
...
</code></pre></div><p>次に、 csrunitモジュールにMRET命令が供給されているときにmepcにジャンプする仕組みを実装します。 ジャンプするための仕組みには、トラップによってジャンプする仕組みを利用します (リスト33)。 <code>raise_trap</code>に<code>is_mret</code>を追加し、トラップ先も変更します。</p><p><span class="caption">▼リスト4.33: MRET命令によってジャンプさせる (csrunit.veryl)</span></p><div class="language-veryl"><button title="Copy Code" class="copy"></button><span class="lang">veryl</span><pre class="hljs"><code><span class="hljs-comment">// Trap</span>
