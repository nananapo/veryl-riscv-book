...
//footnote[muldiv.sufficient][「効率」は、計算に要する時間や回路面積などの効率のことです。高速に計算する方法については応用編で検討します。]
...
絶対値で計算することで乗算の実行途中で符号ビットを考慮する必要がなくなり、
既に実装してある符号無しの乗算器だけで符号付きの乗算を実現できます。
...
絶対値で計算してから符号を戻すことで符号無し除算器だけで符号付きの剰余演算を実現できます。
