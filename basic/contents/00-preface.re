...
@<large>{本書で利用するソースコードは、以下のリポジトリから入手できます。}

 * @<large>{@<href>{https://github.com/nananapo/bluecore}}

@<large>{本書は次のリポジトリで管理されています。質問や報告があるときは、以下のリポジトリでissueを作成してください。}

 * @<large>{@<href>{https://github.com/nananapo/veryl-riscv-book}}
...
プログラムコードの差分は次のように修飾されます。
ただし、リスト内のコードが全て新しく追加されるときは修飾しません。
削除されたコードを示さない場合もあります。

//list[][]{
@<del>|print("Hello, world!\n");|       @<balloon>{削除したコード}
@<b>|print("Hello, "+name+"!\n");|    @<balloon>{追加、変更したコード}
//}
