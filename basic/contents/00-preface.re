...
こんにちは!
あなたはCPUを自作したことがありますか?
自作したことがあってもなくても大歓迎、
この本はCPU自作の面白さを世に広めるために執筆されました。

パソコンの主要な部品であるCPUは、
とても大きくて複雑な電子回路で構成されています。
現代的で、高速で、ゲームができるようなCPUを作るのは非常に難しいですが、
ちょっと遅くて機能が少ないCPUなら、誰でも簡単に作ることができます。

本書では、@<b>{Veryl}という「ハードウェアを記述するための言語」でCPUを自作する方法を解説しています。
Verylを使うと、例えば32ビットの足し算をする回路を次のように記述できます。
...
@<code>{x}と@<code>{y}を受け取って、
@<code>{sum}に@<code>{x + y}を割り当てるだけ....簡単ですね。
これと同様に、入出力を書いて、足し算やAND、OR、NOT演算などを書くだけで、CPUを書くことができます。

CPUは大きな論理回路です。
でも、Verylで書いた小さな部品を組み合わせていけば、
簡単に実現することができます。

@<b>{あなたもCPUを自作してみませんか？}
自分好みのCPUを作る第一歩を踏み出しましょう。
 ...
 * 基本的な論理演算
 * C、C++、JavaScript、Python、Ruby、Rustのような一般的なプログラミング言語の経験

本書では、Verylの他にC++, Python, Makefile, シェルスクリプトを使用します。
Verylについては詳細を解説しています。
他の言語については、動作とどのような機能を持つかは解説しますが、
言語の仕様や書き方、ライブラリなどは説明しません。

===[notoc] 本書のソースコード / 問い合わせ先

本書で利用するソースコードは、以下のサポートページから入手することができます。
質問やお問い合わせ方法についてもサポートページを確認してください。
...
本文に「後の章」と書かれていても、本書には含まれない場合があります。
完全版および電子版は無料で配布されており、
...
//footnote[vlsi][小さいチップなら安く(数万 ～ 数百万円で)製造できます。efablessやTinyTapeout、OpenMPWで検索してください]
