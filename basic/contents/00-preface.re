...
本書は、Verylという言語で@<b>{CPUを自作する}方法を解説した本です。
...
本書に関する質問やお問い合わせは、以下のサポートページにお願いします。
...
CPUの自作を難しくしているのは、
論理設計と物理的な製造です。
論理設計は、論理設計に使う言語やシミュレータ、
回路に落とし込むツールなどが無料で公開されており、
簡単に
物理的な製造のハードルは高いですが、
...
ただし、実機がなくても実装を進められるので所有していなくても構いません。
...
VerylのソースコードはSystemVerilogに変換できます。
そのため、SystemVerilogを利用できる環境でVerylを使用できます。
...
他には、回路のシミュレーションやテストのためにC++とPythonを利用します。
プログラムがどのような意味や機能を持つかは解説しますが、
