...
1ビットの値はフリップフロップ(flip-flop, FF)という回路によって保持することができます。
フロップフロップをN個並列に並べると、
Nビットの値を保持することができます。
...
その結果をレジスタに格納する操作の記述によって行うことができます。
このような、レジスタからレジスタに、
...
//caution[注意]{
本書は2024/11/3時点のVeryl(バージョン0.13.2)について、
本書で利用する範囲の文法, 機能を解説しています。
Verylはまだ開発途上(正式版, 安定版がリリースされていない)状態の言語であるため、
破壊的変更が入り、記載しているコードが使えなくなる可能性があります。
最新のVerylに対応した解説はweb版, pdf版を確認してください。
//}
...
VerylのSystemVerilogへのトランスパイルをウェブブラウザ上で試すことができます。
...
物理的なハードウェア上では、全ての値は@<code>{0}か@<code>{1}の二値として解釈されます。
不定値とハイインピーダンスはシミュレーションのときにのみ利用されます。

1ビットの四値を表現するための型は@<b>{logic}です。
Nビットのlogic型は@<code>{logic<N>}と記述することができます。
...
変数への代入は@<code>{assign 変数名 = 式;}で行います。

通常のプログラミング言語での代入とは、
スタック領域やレジスタに存在する変数に値を格納することです。
これに対して、assign文による代入は変数に式の結果が@<b>{継続的代入}(continuous assignment)されます。
変数に式が継続的代入されると、
...
yの値が時間経過により0, 1, 0, 1, 0と変化したとします。
このとき、xの値はyが変わるのと同時に変化します(@<img>{assign_wave})。
@<img>{assign_wave}は、横軸が時間で、xとyの値を線の高低で表しています。
@<img>{assign_wave}のような図を@<b>{波形図}(waveform)、または単に@<b>{波形}と呼びます。

xにyではなくa + bを継続的代入すると、aかbの変化をトリガーにxの値が変化します。

//image[assign_wave][xはyの値の変化に追従する][width=60%]

assign文による代入の代わりに、
@<b>{always_comb}ブロックでも代入することができます
(@<list>{always_comb.assign})。
always_comb内での代入のことを、@<b>{ブロッキング代入}(blocking assignment)と呼びます。

//list[always_comb.assign][always_combブロック内での代入]{
always_comb {
	a = b + 100;
}
//}
...
assign文による代入と同じように、
@<list>{always_comb.assign}ではbの変化をトリガーにaの値が変化します。

always_combブロックには複数のブロッキング代入文を記述することができます。
このとき、代入文は上から順番に実行されます。
...
aにはXが代入されますが、bにはYが代入されます。
a, b, sは、変数X, Yの変化をトリガーに値が更新されます。

assign文やブロッキング代入文は論理回路の状態(レジスタ)を変更しません。
そのため、assign文やブロッキング代入文は組み合わせ回路になります。
...
var文によって宣言した変数は、
assign文による継続的代入, またはalways_comb内でのブロッキング代入が可能です。
...
全ての代入は同時に行われます(@<list>{always_ff.nonblocking})。
この代入は後続の文の代入をブロックしないため、
ブロッキング代入の対として@<b>{ノンブロッキング代入}(non-blocking assignment)と呼びます。

//list[always_ff.nonblocking][ノンブロッキング代入のタイミングは同じ]{
always_ff {
	if_reset {
		...
	} else {
...
@<list>{always_ff.nonblocking}のAとBの代入文は、
2つのalways_ffブロックに分けて記述することができます(@<list>{multi.always_ff.nonblocking})。
同じタイミングで更新されるalways_ffブロックは、全て同時に実行されます。

2つ以上のalways_ffブロックで、
1つの同じレジスタの値を変更することはできません。

//list[multi.always_ff.nonblocking][同じタイミングで更新されるalways_ffは同時に実行される]{
// AとBの値を入れ替える
always_ff(clk, rst) {
	A = B;
}
...
本書では、
継続的代入, ブロッキング代入, ノンブロッキング代入を区別せず、
どれも代入と呼ぶことがあります。
...
代入方法				値の更新タイミング
==========================================================================================================
assign文@<br>{}let文	式に使用されている変数が更新されたとき。@<br>{}継続的代入される。
always_combでの代入		always_combブロック内の式に使用されている変数が更新されたとき。@<br>{}ブロッキング代入になるので、上から順に実行される。
always_ffでの代入		クロック信号, またはリセット信号のタイミング。@<br>{}ノンブロッキング代入になるので、同じタイミングで更新されるalways_ff@<br>{}での代入は全て同時に実行される。
//}
...
モジュールには、インスタンス化するときに変更可能な定数(@<b>{パラメータ})を用意することができます。
...
最下位ビット(least significant bit, LSB)は@<b>{lsb}キーワードで指定することができます。
...
ビット列や文字列を連結したいとき、@<code>{{\}}を使用することができます(@<list>{renketu})。
...
for文は@<list>{for.code}のように記述することができます。
...
モジュールでのポートの宣言は、@<code>{ポート名 : modport インターフェース名::modport名}と記述することができます。
