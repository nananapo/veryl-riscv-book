...
半加算器とは1ビットの加算を行う回路で、
入力(X、Y)が決まると、出力(C、S)が一意に決まります(@<table>{halfadder.truth})。
...
これに対して、always_combブロック内での代入は、
式が評価(計算)された値が変数に1度だけ代入されるのではなく、
変数の値は常に式の計算結果になります。

具体例で考えてみます。
例えば、1ビットの変数xに1ビットの変数yを代入します
(@<list>{assign.wave})。
...
xにyではなくa + bを代入すると、aかbの変化をトリガーにxの値が変化します。

//image[assign_wave][xはyの値の変化に追従する][width=60%]

always_combブロックには複数の代入文を記述することができます。
...
ブロッキング代入文やassign文は論理回路の状態(レジスタ)を変更しません。
そのため、ブロッキング代入文やassign文は組み合わせ回路になります。
...
var文で宣言した変数に対してブロッキング代入することができます。

@<b>{let}文を使うと、
変数の宣言とブロッキング代入を同時に行うことができます。
...
何度も記述する操作や計算は、関数(@<b>{function})を使うことでまとめて記述することができます(@<list>{function.first})。
...
インターフェース内には、関数の定義やassignブロック、always_combブロック、always_ffブロックなどの文を記述することができます。
