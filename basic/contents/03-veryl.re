...
半加算器は、入力X, Yが決まると、出力C, Sが一意に決まります(@<table>{halfadder.truth})。

//image[halfadder][半加算器 (MIL記法)][width=50%]
...
例えば、入力が1になるたびにカウントダウンするカウンタを考えます。
カウントダウンするためには、
今のカウンタの値(状態)を保持する必要があります。
よって、このカウンタは、
入力と状態によって一意に出力の決まる回路です。

1ビットの値はフリップフロップという回路によって保持することができます。
フロップフロップをN個並列に並べると、
Nビットの値を保持することができます。
フリップフロップを並列に並べた記憶装置のことを、@<b>{レジスタ}(置数器)といいます。
...
真理値表を作成し、
それを実現する論理演算を構成します。
入力数や状態数が数十個ならどうにか設計できるかもしれませんが、
数千, 数万の入力や状態があるとき、
手作業で設計するのはほとんど不可能です。
...
デジタル回路を設計するための言語です。
有名なHDLとしては、SystemVerilogが挙げられます。

SystemVerilogを利用すると、半加算器は@<list>{halfadder.sv}のように記述することができます。
...
@<b>{レジスタ転送レベル}(Register Transfer Level, RTL)といいます。

HDLで記述された論理回路は、
@<b>{合成系}によって、
RTLから実際の回路のデータに変換されます。

=== Veryl

メジャーなHDLといえば、Verilog, SystemVerilogなどが挙げられます@<fn>{vhdl}。

//footnote[vhdl][VHDLが無いじゃないかと思った方、すみません。VHDLのことを私はよく知らないので無いことにしました。]

Verilogは1980年代に開発された言語であり、
