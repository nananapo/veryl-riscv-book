...
しかし、継続的代入と違い、ブロッキング代入には、
代入が完了するまで後続の文の代入をブロックするという違いがあります。

//list[always_comb.order][ブロッキング代入は後続の文の代入をブロックする]{
always_comb {
	s = 0;
	a = s; // a = 0
	s = 1;
...
例えば、@<code>{always_comb.order}では、
aには0が代入されますが、bには1が代入されます。
これは、
@<code>{s = 0}が完了するまで@<code>{a = s}がブロックされることでaに0が代入され、
@<code>{s = 1}が完了するまで@<code>{b = a}がブロックされることでbに1が代入されるためです。
最終的に、aは0, bは1, sは1になります。
簡単に言ってしまうと、ブロッキング代入文は上から順番に実行されます。

assign文やalways_combブロック内でブロッキング代入される式は、
組み合わせ回路になります。
...
//table[table.assign][変数への代入方法と動作]{
代入方法				例								値の更新タイミング
==========================================================================================================
継続的代入				assign文@<br>{}let文			式に使用されている変数が更新されたとき
ブロッキング代入		always_comb@<br>{}内の代入		always_combブロック内の式に使用されている変数が更新@<br>{}されたとき。後続の文の代入をブロックすることに注意
ノンブロッキング代入	always_ff@<br>{}内の代入		クロック信号, またはリセット信号のタイミング。@<br>{}全てのノンブロッキング代入は同時に代入されることに注意
//}
