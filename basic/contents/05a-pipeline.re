...
プログラムの実行時間は、簡単に、次のような式で表せます(@<eq>{cpu-seino-houteisiki})。
...
この問題は、命令の処理を、まるで車の組立のように流れ作業で行うことで緩和できます。
...
整数演算命令の時、レジスタの値を読み取り、ALUで計算します。
分岐命令のとき、分岐判定を行います。
...
IDステージにある命令は、
EXステージが命令を受け付けられるとき(@<code>{exq_wready})、
...
EXステージにある命令は、
MEMステージが命令を受け付けられるとき(@<code>{memq_wready})、
...
WBステージが命令を受け付けられるとき(@<code>{wbq_wready})、
