...
それぞれの用語の定義は次の通りです。
 ...
 : クロック周波数 (clock rate)
    クロック・サイクル時間(clock cycle time)の逆数@<br>{}
    クロック・サイクル時間は、クロックが@<code>{0}→@<code>{1}→@<code>{0}になる周期のこと
...
クリティカルパスとは、組み合わせ回路の中で最も大きな遅延を持つ経路のことです。
...
//footnote[other.runinst][他の方法として、関数呼び出しやループをCPU側で検知して結果を保存して利用することで実行命令数を減らす手法があります。この手法はずっと後の章で検討します。]

==== CPIに注目する

CPIを減らすためには、
例えばどの命令も1クロックで実行してしまうという方法が考えられます。
...
命令のビット列をもとにレジスタの値を選択し、
ALUで足し算を実行し、
その結果をレジスタにライトバックします。
これらを1クロックで実行するということは、
命令が保存されている32ビットのレジスタと32*64ビットのレジスタファイルを入力に、
...
この問題は、命令の処理を、まるで車の組立のように流れ作業で行うことで緩和できます(@<img>{pipeline})。
...
具体的に処理をどのようなステージに分割してパイプライン処理を実現すればいいでしょうか?
...
EXステージは、IDステージで生成された制御フラグと即値を受け取ります。
...
IDステージにある命令は、
EXステージが命令を受け入れられるとき(@<code>{exq_wready})、
IDステージを完了してEXステージに処理を進められます。
この仕組みは、
...
EXステージにある命令は、
MEMステージが命令を受け入れられるとき(@<code>{memq_wready})、
EXステージを完了してMEMステージに処理を進められます。
この仕組みは、
...
また、ジャンプ命令、分岐命令かつ分岐が成立、またはトラップが発生するとき、
次に実行する命令のアドレスを変更します。
...
WBステージが命令を受け入れられるとき(@<code>{wbq_wready})、
MEMステージを完了してWBステージに処理を進められます。
この仕組みは、@<code>{memq_rready}と@<code>{wbq_wvalid}を確認してください。
...
最後に、データハザードが発生しているかどうかをデバッグ表示します
