...
CPUがソフトウェアを実行するとき、
処理を中断したり終了したりしなければならないような異常な状態@<fn>{unusual-condition}が発生することがあります。
...
具体的にはPCをトラップベクタ(trap vector)に移動したり、CSRを変更したりします。
...
mcauseレジスタを読むことでどの例外が発生したかを判別することができますが、
その例外の詳しい情報を知りたいことがあります。

//image[mtval][mtvalレジスタ][width=100%]

RISC-Vには、例外が発生した時のソフトウェアによるハンドリングを補助するために、
