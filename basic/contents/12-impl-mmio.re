...
//image[mmio.drawio][メモリマップ][width=40%]
...
デバイスとメモリ空間の対応は@<img>{mmio.drawio}のように設定します。
@<img>{mmio.drawio}のようにメモリがどのように配置されているかを示す図のことを@<b>{メモリマップ}(Memory map)と呼びます。
...
//list[membus_if.veryl.memtoram.Membus][別名の定義 (membus_if.veryl)]{
